...
Current slice override: `f453cd9a` completed the AchievementToast unused cleanup; the active working-tree slice is the Onboarding residual compiler/source-proven unused cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; after the current Onboarding residual cleanup, prefer a low-risk noUnused slice such as dormant canvas support cleanup or pure image utility cleanup before entering `fileSystemService.ts`, `ApiSettingsView.tsx`, `geminiService.ts`, `useImageGeneration.ts`, or another larger hotspot.
