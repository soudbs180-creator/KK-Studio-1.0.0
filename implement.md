...
Current slice override: M107 extracts only the OpenAI-compatible post-surface image dispatch plan into a pure helper. Keep `resolveChannelRuntime`, `resolveImageSurface`, endpoint construction, auth, fetch behavior, logging, and billing-safe fallback throws in their existing owners.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files. After M107, choose the next seam from a fresh map; do not chain endpoint/auth, fetch behavior, fallback-order, keyManager, storage, release metadata, UI, or broad debt cleanup into the same slice.
