...
Current slice override: `5dac56e8` completed the import-only unused cleanup; the active working-tree slice is the live canvas residual cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current live canvas residual slice, preserve `InfiniteCanvasProps` compatibility and only remove compiler-proven unused destructured values.
