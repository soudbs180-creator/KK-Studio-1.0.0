...
3. For Stage Two, split or prune one responsibility at a time from the largest files. M123 followed the M122 security hardening by moving the task-token implementation behind a focused helper. The current settings UI closure resolves the previously excluded UI WIP that blocked full unit verification, then the next ordinary step is a fresh seam map or quality scan. Do not chain keyManager secrets/cloud sync, provider fetches, endpoint/auth behavior, provider persistence, route selection, storage, release metadata, unrelated UI redesign, or broad debt cleanup into the same slice.
