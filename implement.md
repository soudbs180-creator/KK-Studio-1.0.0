...
This is a long-running execution. Plain `.git` still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. Do not use plain `.git` for commit readiness. The development fact source is `git --git-dir=node_modules/.codex-git-full --work-tree=.`; the latest clean baseline before the Stage Two M18 Canvas workflow update helper extraction was `c46a4c49 refactor: extract canvas prompt image link helpers`.
...
3. For Stage Two, split one responsibility at a time from the largest files; the next slice must be selected from a fresh `CanvasContext.tsx` source map after the workflow update helper extraction commit.
