...
Current slice override: M108 extracts only OpenAI-compatible image URL/base64 payload parsing into a pure helper. Keep endpoint selection, auth, fetch behavior, task polling, provider compatibility classification, logging, billing-safe fallback throws, and UI behavior in their existing owners.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files. After M108, choose the next seam from a fresh map; do not chain endpoint/auth, fetch behavior, fallback-order, keyManager, storage, release metadata, UI, or broad debt cleanup into the same slice.
