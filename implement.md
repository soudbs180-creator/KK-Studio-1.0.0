...
Current slice override: `eeb377d5` completed the dormant Pixi canvas renderer cleanup; the active working-tree slice is the dormant canvas residual cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; after the current dormant canvas residual cleanup, continue only with source-proven cleanup before entering `fileSystemService.ts`, `ApiSettingsView.tsx`, `geminiService.ts`, `useImageGeneration.ts`, or another larger hotspot.
