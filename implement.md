...
This is a long-running execution. Plain `.git` still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. Do not use plain `.git` for commit readiness. The development fact source is `git --git-dir=node_modules/.codex-git-full --work-tree=.`; the latest clean baseline before the active canvas merge helper slice is `0a5c2339 refactor: extract canvas persisted image recovery helpers`.
...
3. For Stage Two, split one responsibility at a time from the largest files; the current CanvasContext slice is the canvas merge helper boundary.
