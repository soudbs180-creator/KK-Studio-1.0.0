...
Current slice override: `63386046` completed the user API payload unused secret-constant cleanup; the active working-tree slice is cost service import-only cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current cost service import-only cleanup, preserve all cost estimation and recording logic while removing only compiler-proven unused imports.
