...
Current slice override: M109 extracts only OpenAI-compatible image profile, aspect orientation, image count, generation size, and edit size resolution into a pure helper. Keep endpoint selection, auth, fetch behavior, task polling, provider compatibility classification, logging, billing-safe fallback throws, edit endpoint routing, and UI behavior in their existing owners.
