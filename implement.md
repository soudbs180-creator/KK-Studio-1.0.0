...
Current slice override: Stage Two M126 OpenAI-compatible image payload security hardening is the active code slice. The selected production boundary is URL and MIME allowlisting inside `src/services/llm/openAICompatibleImagePayload.ts`. Do not change provider endpoint selection, auth/header assembly, route config persistence, fetch execution, polling loops, billing, release metadata, key storage, provider routing, UI, or unrelated runtime/server behavior in this slice.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files. M123 followed the M122 security hardening by moving the task-token implementation behind a focused helper. M124 continued the OpenAI-compatible adapter seam by extracting pure AceData route/reference/size helpers. M125 extracted 12AI async route helpers and chat-image response selection. M126 closes the security follow-up by allowlisting image URL schemes and raster MIME types in the payload helper. After M126, the next ordinary step is a fresh seam map or quality scan. Do not chain keyManager secrets/cloud sync, provider fetches, endpoint/auth behavior, provider persistence, route selection, storage, release metadata, unrelated UI redesign, or broad debt cleanup into the same slice.
