...
Current slice override: Stage Two M127 OpenAI-compatible image reference cleanup is the active code slice. The selected production boundary is repeated reference-image formatting and unreachable commented delegate bodies inside `src/services/llm/OpenAICompatibleAdapter.ts` and `src/services/llm/openAICompatibleImageReferences.ts`. Do not change provider endpoint selection, auth/header assembly, route config persistence, fetch execution, polling loops, billing, release metadata, key storage, provider routing, UI, or unrelated runtime/server behavior in this slice.
