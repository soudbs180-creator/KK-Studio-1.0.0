...
Current slice override: `37540efc` completed the model display-name provider-parameter cleanup; the active working-tree slice is video-service unused cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current video-service cleanup, preserve video generation behavior and only remove or mark compiler-proven unused locals/imports/parameters.
