...
Current slice override: `bdb082d7` completed the live canvas residual cleanup; the active working-tree slice is workflow-actions import cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current workflow-actions import slice, preserve `App.tsx` template-list ownership and only remove the compiler-proven unused hook import.
