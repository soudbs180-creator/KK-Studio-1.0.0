...
Current slice override: `6c624909` completed the UserProfileModal billing alias cleanup; the active working-tree slice is user API cloud helper cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current user API cloud helper cleanup, remove only the uncalled private helper while preserving typed auth payload loading, local fallback, cache behavior, payload compaction, and secret redaction.
