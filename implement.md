...
Current slice override: `426a0a0c` completed secureModelProxy route-gate wiring; the next slice should start with an OpenAI-compatible image-dispatch seam map before code movement.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; the next candidate is `src/services/llm/OpenAICompatibleAdapter.ts` image dispatch, but only after a source-contract map protects endpoint selection, auth, fetch behavior, and fallback ordering.
