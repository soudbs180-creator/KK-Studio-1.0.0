...
This is a long-running execution. Plain `.git` still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. Do not use plain `.git` for commit readiness. The development fact source is `git --git-dir=node_modules/.codex-git-full --work-tree=.`; the latest working-tree code line is M112 local user-route auth helper extraction, with exact commits tracked by alternate git.
...
Current slice override: M112 extracts only local user-route auth/header/query-key and route-surface pure helpers into `apps/api/src/modules/model-proxy/application/local-user-route-auth.ts`. Keep endpoint selection, fetch behavior, task polling requests, provider branch execution, credential storage, keyManager/cloud sync, fallback ordering, logging, billing metadata, release metadata, and UI behavior in their existing owners.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files. After M112, choose the next seam from a fresh map; do not chain endpoint selection, fetch behavior, fallback-order, keyManager, storage, release metadata, UI, or broad debt cleanup into the same slice.
