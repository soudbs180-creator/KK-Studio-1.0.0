...
3. For Stage Two, split or prune one responsibility at a time from the largest files; after the current OpenAI-compatible unused cleanup, the next candidate is `LLMService.ts`, `ChatSidebar.tsx`, `PromptNodeComponent.tsx`, or another file-specific noUnused cleanup slice.
