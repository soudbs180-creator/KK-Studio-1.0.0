...
3. For Stage Two, split or prune one responsibility at a time from the largest files; the next candidate must be selected by a fresh seam map, with current options tracked in `status.md`.
