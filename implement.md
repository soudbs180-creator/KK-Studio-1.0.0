...
Current slice override: no active code slice after `f2de4377`. The next likely OpenAI-compatible seam is generic task payload/status/result parsing, but it must get a fresh RED contract first and must keep endpoint selection, auth, fetch behavior, task polling requests, provider compatibility classification, logging, billing-safe fallback throws, edit endpoint routing, and UI behavior in their existing owners.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files. After M109, choose the next seam from a fresh map; do not chain endpoint/auth, fetch behavior, fallback-order, keyManager, storage, release metadata, UI, or broad debt cleanup into the same slice.
