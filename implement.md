...
Current slice override: `d49398c9` completed the storage preference unused cleanup; the active working-tree slice is image storage unused cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current image storage unused cleanup, remove only the compiler-proven unused local in `cleanupOriginals()` while preserving batch image pagination, compression, save, and error behavior.
