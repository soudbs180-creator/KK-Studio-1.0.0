...
Current slice override: `efe1bdf9` completed the image storage unused cleanup; the active working-tree slice is Google adapter import cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current Google adapter import cleanup, remove only compiler-proven unused imports while preserving Google/Gemini request behavior and inline video generation type references.
