...
This is a long-running execution. Plain `.git` still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. Do not use plain `.git` for commit readiness. The development fact source is `git --git-dir=node_modules/.codex-git-full --work-tree=.`; the latest working-tree code line is M113 local user-route endpoint helper extraction, with exact commits tracked by alternate git.
...
Current slice override: M113 extracts only local user-route direct endpoint URL normalization into `apps/api/src/modules/model-proxy/application/local-user-route-endpoints.ts`. Keep auth/header/query-key behavior, endpoint call sites, fetch behavior, task polling requests, provider branch execution, credential storage, keyManager/cloud sync, fallback ordering, logging, billing metadata, release metadata, and UI behavior in their existing owners.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files. After M113, choose the next seam from a fresh map; do not chain auth/header behavior, endpoint call-site behavior, fetch behavior, fallback-order, keyManager, storage, release metadata, UI, or broad debt cleanup into the same slice.
