...
Current slice override: `574b3f41` completed the Gemini service unused cleanup; the active working-tree slice is image generation hook cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current image generation hook cleanup, remove only compiler-proven unused imports/destructures/locals while preserving generation recovery, billing, storage persistence, and pending-task state transitions.
