...
Current slice override: `d4291729` completed the generation runtime import cleanup; the active working-tree slice is CanvasContext type-import cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current CanvasContext type-import cleanup, preserve the public type re-export boundary and remove only the compiler-proven unused imports.
