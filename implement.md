...
This is a long-running execution. Plain `.git` still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. Do not use plain `.git` for commit readiness. The development fact source is `git --git-dir=node_modules/.codex-git-full --work-tree=.`; the latest clean baseline before the active prompt recovery slice is `bfd56ddc docs: tighten final audit ledger`.
...
3. For Stage Two, split one responsibility at a time from the largest files; the current CanvasContext slice is the startup prompt recovery normalization boundary.
