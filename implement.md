...
Current slice override: `cedb3718` completed the image generation hook cleanup; the active working-tree slice is API settings unused cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current API settings cleanup, remove only compiler-proven unused imports/constants/helpers while preserving API settings route visibility, readonly snapshot behavior, BYOK persistence, and settings workbench UI contracts.
