...
Current slice override: Stage Two M128 dead Gemini response cache cleanup is the active code slice. The selected production boundary is deleting the source-proven unreferenced `src/services/storage/cache.ts` module, removing prompt text from the surviving prompt-node insertion diagnostic, and extending the storage unused-cleanup source contract. Do not change active storage persistence, image storage, file-system service behavior, UI, release metadata, provider routing, auth/header behavior, or unrelated runtime/server behavior in this slice.
