...
Current slice override: `95c5fc16` completed the user API cloud helper cleanup; the active working-tree slice is NewAPI management unused cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current NewAPI management unused cleanup, remove only compiler-proven unused locals while preserving the `getAllChannels()` preflight side effect, `/api/channel/balance` request, `channels` cache update, and returned `updatedChannels` shape.
