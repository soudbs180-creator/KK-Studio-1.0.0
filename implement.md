...
This is a long-running execution. Plain `.git` still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. Do not use plain `.git` for commit readiness. The development fact source is `git --git-dir=node_modules/.codex-git-full --work-tree=.`; the latest clean baseline before the current slice is `56debf21 refactor: extract key manager provider merge helper`.
...
3. For Stage Two, split one responsibility at a time from the largest files; the active candidate is the provider linked-slot matching helper in `src/services/auth/keyManager.ts`.
