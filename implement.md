...
Current slice override: `0cc6c77c` completed the KK API client unused DTO cleanup; the active working-tree slice is user API payload unused secret-constant cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current user API payload cleanup, preserve the live client-visible secret placeholder and redaction policy in cloud record storage while removing only duplicate compiler-proven unused constants from the payload sanitizer.
