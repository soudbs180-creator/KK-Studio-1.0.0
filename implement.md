...
Current slice override: `9ce70e96` completed the dormant canvas residual cleanup; the active working-tree slice is the legacy dashboard unused-icon cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; after the current legacy dashboard unused-icon cleanup, continue only with source-proven cleanup before entering `fileSystemService.ts`, `ApiSettingsView.tsx`, `geminiService.ts`, `useImageGeneration.ts`, or another larger hotspot.
