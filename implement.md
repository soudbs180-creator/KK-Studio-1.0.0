...
Current slice override: `a4032085` completed the generation-runtime contract test cleanup; the active working-tree slice is prompt optimizer duplicate-tab cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current prompt optimizer duplicate-tab cleanup, preserve prompt optimizer route/UI payload behavior and only remove the compiler-proven unused duplicate tab constant.
