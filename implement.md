...
Current slice override: `27f03e62` completed the user API profile import-only cleanup; the active working-tree slice is UserProfileModal billing alias cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current UserProfileModal billing alias cleanup, remove only the duplicate stale alias while preserving the live `billingUiEnabled` feature gate and billing UI behavior.
