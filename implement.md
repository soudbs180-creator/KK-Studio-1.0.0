...
Current slice override: `c4526e6b` completed the ProjectManager prop-destructure cleanup; the active working-tree slice is ecommerce taskMerger parameter cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current ecommerce taskMerger cleanup, preserve merge behavior and only rename the compiler-proven unused private parameter.
