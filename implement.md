...
Current slice override: `324b42a6` completed the video-service unused cleanup; the active working-tree slice is image-service unused cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current image-service cleanup, preserve image priority/LOD behavior and only remove compiler-proven unused locals/imports/fields/helpers.
