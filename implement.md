...
Current slice override: Stage Two M130 OpenAI-compatible diagnostics prompt redaction is the active code slice. The selected production boundary is redacting prompt-like fields in `src/services/llm/openAICompatibleDiagnostics.ts` and ensuring OpenAI-compatible python snippets use redacted request previews instead of raw prompt/body values. Do not change endpoint selection, auth/header behavior, fetch execution, polling, billing, fallback ordering, key storage, provider routing, UI, release metadata, or unrelated runtime/server behavior in this slice.
