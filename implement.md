...
Current slice override: `9db2fe41` completed the Google adapter import cleanup; the active working-tree slice is Gemini service import cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current Gemini service import cleanup, remove only compiler-proven unused imports/helpers while preserving Gemini request behavior, error classification, and the `llmService.generateImage(llmOptions)` / `calculateCost(...)` call shapes.
