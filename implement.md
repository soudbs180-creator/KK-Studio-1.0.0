...
Current slice override: `d5e6a809` completed the NewAPI management unused cleanup; the active working-tree slice is billing service unused cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current billing service unused cleanup, remove only the uncalled private recharge helper while preserving the default payment-channel config builder, `qrDisplay` mapping, and recharge channel list fallback behavior.
