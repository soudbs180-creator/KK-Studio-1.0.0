...
Current slice override: `c465146b` completed the recharge submission unused cleanup; the active working-tree slice is storage adapter unused cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current storage adapter unused cleanup, remove only compiler-proven unused locals while preserving OPFS import behavior, native/indexeddb import behavior, and image dimension fallback semantics.
