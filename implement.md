...
Current slice override: `318e6b1e` completed the secure model proxy unused-helper cleanup; the active working-tree slice is user API profile import-only cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current user API profile cleanup, remove only the unused named import while preserving typed auth API fallback and cloud merge behavior.
