...
Current slice override: `7d2c2584` completed the file-system compatibility stub cleanup; the active working-tree slice is the import-only unused cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current import-only slice, remove only compiler-proven unused imports/types without changing implementation code.
