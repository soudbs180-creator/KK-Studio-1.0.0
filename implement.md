...
Stage One Backfill M2 completed in `8a458cd4` by hardening `src/app/usePromptGroupLayout.ts` without re-creating the hook. Stage One Backfill M3 completed in `ab719c4a` by semantically checking the public `useGenerationRuntime` boundary. The generation billing follow-up completed in `083db7f8`. Stage One Backfill M5 completed in `569383aa` by semantically checking `usePptRuntime` and the PPT helper boundary. Stage Two M1 completed in `92a9dc41` by extracting state/default/context boundary ownership into `src/context/canvasContextState.ts` and compatibility syncing into `src/context/canvasCompatibility.ts`. Stage Two M2 completed in `e0f1b583` by extracting the pure Canvas selection reducer into `src/context/canvasSelection.ts`. Stage Two M3 completed in `83cc8d7f` by extracting the pure prompt child image resolver into `src/context/canvasPromptChildImages.ts`. Stage Two M4 completed in `9ec4dbb1` by extracting the pure workflow source node ID resolver into `src/context/canvasWorkflowSourceNodeIds.ts`. Stage Two M5 completed in `002ee6fe` by extracting media recovery helpers into `src/context/canvasMediaRecovery.ts`. Stage Two M6 completed in `53f80d80` by extracting startup prompt recovery normalization into `src/context/canvasPromptRecovery.ts`. Stage Two M7 completed in `0a5c2339` by extracting persisted image recovery helpers into `src/context/canvasPersistedImageRecovery.ts`. Stage Two M8 completed in `b68867dd` by extracting canvas merge helpers into `src/context/canvasMerge.ts`. Stage Two M9 completed in `7d8a4331` by extracting invalid-card cleanup helpers into `src/context/canvasCleanup.ts`. Stage Two M10 completed in `3abdd250` by extracting canvas placement helpers into `src/context/canvasPlacement.ts`. Stage Two M11 completed in `c171de38` by extracting canvas layering helpers into `src/context/canvasLayering.ts`. The latest security/release cleanup line is `4cdbf4cf` protobuf override, `567f85aa` portable metadata refresh, `0c5cadde` Nutrient OCR server-key hardening, `333f2551` PostCSS security patch, `b6620ef2` dead AI12 service pruning, `0603547a` legacy Netlify raw-key endpoint removal, `5994c34b` Canvas group helper extraction, `4722acbe` Canvas movement helper extraction, `d7a9d0a7` Canvas tag helper extraction, `c9d39bb2` Canvas node update helper extraction, `c80ffa70` Canvas position update helper extraction, `c46a4c49` Canvas prompt-image link helper extraction, `fbe26764` Canvas workflow update helper extraction, `16d805cb` Canvas image delete helper extraction, `943af671` Canvas merge-into helper extraction, `c0c37736` Canvas unused-code cleanup, `bfc6fb8a` Canvas arrange-selection helper extraction, `9769ea7f` Canvas duplicate selected-arrange cleanup, `c1a76a43` Canvas selected-root arrange helper extraction, `1318d84d` Canvas selected-group arrange helper extraction, `7cbd7346` Canvas auto-arrange helper extraction, `28356b4f` ledger alignment, `b16843ee` Canvas prompt node update helper extraction, `a174b557` keyManager model helper extraction, `3ce7ae59` keyManager key type helper extraction, `08eb89d8` model normalization facade consolidation, `56debf21` provider runtime-state merge extraction, `615b7969` provider linked-slot matching extraction, `dd3ad358` provider usage helper extraction, `a598312d` route ID helper extraction, `531eae6b` credential sanitizer extraction, `7b54fd1a` channel config secret boundary extraction, `4658d947` keyManager dead helper pruning, `afe89b71` browser diagnostics wrapper pruning, `1214ab98` legacy Google model constant pruning, `031f3ec7` pricing model ID helper extraction, `46045a80` model category helper extraction, `ea2ad869` model type inference extraction, `54cd8312` pricing URL helper extraction, `c435de27` deprecated-model helper extraction, `6cada9ad` 12AI base URL wrapper pruning, `eb3ac527` Google official model predicate extraction, `8b40e892` channel capabilities helper extraction, `f812b66c` API type detector helper extraction, `7005bd50` default model constants helper extraction, `549a2422` provider presets helper extraction, `81ba2a24` documented static model helper extraction, `f5153811` model-list normalization helper extraction, `74d6345a` global-model dead-code cleanup, `dd2295d3` effective provider model fallback extraction, `6902b79b` provider limit delegator pruning, `ff419de9` final-gate fixture repair, `58be183d` payment sidecar dependency audit coverage, `0edb13f5` OpenAI-compatible diagnostics preview extraction, `05fadc4f` ledger alignment, `8f878b3a` OpenAI-compatible image-routing error classifier extraction, `96b94e5e` unreachable OpenAI image fallback cleanup, and `fbf5178e` ledger alignment. The current slice closes compiler/source-proven unused cleanup narrowed to `src/components/layout/PromptBar.tsx` and `src/components/image/ImageCard2.tsx`; the next App cleanup, if selected, must be a separate proof.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; after the current PromptBar/ImageCard cleanup, the next candidate is either an App-specific compiler/source-proven unused cleanup slice or a freshly mapped OpenAI/keyManager/PromptBar seam.
