...
Current slice override: `6028e59a` completed the API settings unused cleanup; the active working-tree slice is keyManager unused cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current keyManager unused cleanup, remove only compiler-proven dead bindings while preserving compatibility re-exports, key storage, provider persistence, cloud payload shape, and route selection behavior.
