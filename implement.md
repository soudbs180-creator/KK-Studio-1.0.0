...
Current slice override: `56ffe696` completed the small LLM-adapter unused cleanup; the active working-tree slice is chat-service unused cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current chat-service cleanup, preserve request/storage/keyManager behavior and only remove compiler-proven unused imports/locals.
