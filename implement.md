...
3. For Stage Two, split or prune one responsibility at a time from the largest files; the active candidate is compiler/source-proven unused cleanup in `src/components/layout/PromptBar.tsx` and `src/components/image/ImageCard2.tsx`.
