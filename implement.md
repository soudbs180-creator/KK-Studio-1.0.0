...
Current slice override: `651b54c5` completed the cost service import-only cleanup; the active working-tree slice is secure model proxy unused-helper cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current secure model proxy cleanup, remove only the uncalled private `buildInvocationError` helper and preserve source-contracted local route gate helpers.
