...
This is a long-running execution. Plain `.git` still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. Do not use plain `.git` for commit readiness. The development fact source is `git --git-dir=node_modules/.codex-git-full --work-tree=.`; the latest clean baseline is `7cbd7346 refactor: extract canvas auto arrange helper`.
...
3. For Stage Two, split one responsibility at a time from the largest files; the next candidate is the pure model parsing/normalization boundary in `src/services/auth/keyManager.ts`, unless a safer remaining `CanvasContext.tsx` seam is proven first.
