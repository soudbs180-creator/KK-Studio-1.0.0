...
Current slice override: `8dfa152a` completed the chat-service unused cleanup; the active working-tree slice is generation-runtime contract test cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current generation-runtime contract test cleanup, preserve all runtime boundary assertions and only remove compiler-proven unused test locals.
