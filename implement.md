...
This is a long-running execution. Plain `.git` still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. Do not use plain `.git` for commit readiness. The development fact source is `git --git-dir=node_modules/.codex-git-full --work-tree=.`; the alternate-git HEAD before the current M110 working-tree slice was `cb0d23c9 docs: record remaining refactor assessment`.
...
Current slice override: M110 extracts only OpenAI-compatible generic task ID/status/list payload parsing into `src/services/llm/openAICompatibleTaskPayload.ts`. Keep `extractProviderMessage`, `buildPolledTaskResult`, endpoint selection, auth, fetch behavior, task polling requests, provider compatibility classification, logging, billing-safe fallback throws, edit endpoint routing, and UI behavior in their existing owners.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files. After M110, choose the next seam from a fresh map; do not chain endpoint/auth, fetch behavior, fallback-order, keyManager, storage, release metadata, UI, or broad debt cleanup into the same slice.
