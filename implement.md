...
Current slice override: `05394f83` completed the pure image orphan cleanup; the active working-tree slice is the dormant Pixi canvas renderer cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; after the current dormant Pixi canvas renderer cleanup, continue only with source-proven dormant canvas support cleanup before entering `fileSystemService.ts`, `ApiSettingsView.tsx`, `geminiService.ts`, `useImageGeneration.ts`, or another larger hotspot.
