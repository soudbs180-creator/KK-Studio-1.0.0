...
Current slice override: `c29effe5` completed the CanvasContext type-import cleanup; the active working-tree slice is ProjectManager prop-destructure cleanup.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files; for the current ProjectManager cleanup, preserve the public props interface and remove only compiler-proven unused destructured props.
