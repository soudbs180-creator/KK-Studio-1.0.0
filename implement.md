...
This is a long-running execution. Plain `.git` may show stale historical state. Do not use plain `.git` for commit readiness. The development fact source is `git --git-dir=node_modules/.codex-git-full --work-tree=.`. The latest committed slice before the current working tree is `2dbb402e fix: harden legacy payment server callbacks`; the current working-tree scope is M119 OpenAI-compatible Google extra-body helper extraction.
...
Current slice override: M119 moves only New API Google `extra_body` merge/build helpers from `OpenAICompatibleAdapter.ts` into `openAICompatibleGoogleExtraBody.ts`. Do not change endpoint selection, auth, fetch behavior, provider routing, fallback ordering, billing, UI, release metadata, or broad type/log cleanup in this slice.
