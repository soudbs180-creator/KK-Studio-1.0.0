...
This is a long-running execution. Plain `.git` may show stale historical state. Do not use plain `.git` for commit readiness. The development fact source is `git --git-dir=node_modules/.codex-git-full --work-tree=.`. The latest stable code baseline after the settings UI closure is `5dda8972 style: close settings workbench shell flattening`; docs-only ledger sync commits may sit above it. The latest code slices before that closure are `74dbdbf1 refactor: extract wuyin route helper`, `dcf38e87 fix: harden local user-route task signing`, `9e4b409e refactor: extract local user-route task token helper`, and `5dda8972 style: close settings workbench shell flattening`.
...
Current slice override: Stage Two M125 OpenAI-compatible adapter helper extraction is the active code slice. The selected production boundaries are pure 12AI async route helpers in `src/services/llm/openAICompatible12AIAsyncRoute.ts` and chat-image response candidate extraction in `src/services/llm/openAICompatibleImagePayload.ts`. Do not change provider endpoint selection, auth/header assembly, route config persistence, fetch execution, polling loops, billing, release metadata, key storage, provider routing, UI, or unrelated runtime/server behavior in this slice.
...
3. For Stage Two, split or prune one responsibility at a time from the largest files. M123 followed the M122 security hardening by moving the task-token implementation behind a focused helper. M124 continued the OpenAI-compatible adapter seam by extracting pure AceData route/reference/size helpers. M125 continues the same adapter seam by extracting 12AI async route helpers and chat-image response selection. After M125, the next ordinary step is a fresh seam map or quality scan. Do not chain keyManager secrets/cloud sync, provider fetches, endpoint/auth behavior, provider persistence, route selection, storage, release metadata, unrelated UI redesign, or broad debt cleanup into the same slice.
