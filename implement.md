...
Current slice override: Stage Two M129 keyManager update diagnostic redaction is the active code slice. The selected production boundary is replacing the raw `updateKey` diagnostic `updates` object with a focused redacted payload helper in `src/services/auth/keyManagerUpdateDiagnostics.ts` and extending the existing key-manager secrets contract. Do not change key persistence, provider persistence, cloud sync, model discovery, route selection, auth/header behavior, storage, UI, release metadata, or unrelated runtime/server behavior in this slice.
