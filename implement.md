...
Current slice override: user-reported ecommerce framework card closure after the auth logout hotfix is the active slice. The selected production boundary is limited to the ecommerce framework prompt card header remark/tag surface and full-canvas ecommerce framework cohort auto-arrange ordering. Do not change endpoint selection, auth/header behavior, fetch execution, polling, billing, fallback ordering, key storage, provider routing, release metadata, payment/server behavior, PromptBar controls, settings UI, or unrelated runtime extraction work in this slice.
