...
async function handleRegister(event: HandlerEvent) {
  try {
    ...
    if (!result.success) {
      return makeErrorResponse(400, "Invalid email format or password must be at least 8 characters.");
    }
    ...
    if (checkUser.rows.length > 0) {
      return makeErrorResponse(400, "Email has already been registered.");
    }
    ...
    return makeErrorResponse(500, "Registration failed due to server error.");
  }
}
...
async function handleLogin(event: HandlerEvent) {
  try {
    ...
    const result = UserAuthSchema.safeParse(payload);
    if (!result.success) {
      return makeErrorResponse(400, "Incorrect email or password.");
    }
    const { email, password } = result.data;
    ...
    return makeErrorResponse(500, "Login failed due to server error.");
  }
}
...
async function handleRefresh(event: HandlerEvent) {
  try {
    ...
    if (userRes.rows.length === 0) {
      return makeErrorResponse(401, "User no longer exists.");
    }
    ...
    return makeErrorResponse(500, "Token refresh failed.");
  }
}
...
export const handler: Handler = async (event, context) => {
  ...
  return makeErrorResponse(404, "Endpoint not found.");
};
