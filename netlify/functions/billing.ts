...
async function handleGetPlans() {
  ...
  return makeResponse(200, { plans: plansArray });
}

/**
 * 创建 Stripe 支付 Checkout 会话并在 DB 记录 pending 订单
 */
async function handleCreateCheckout(event: HandlerEvent) {
  ...
  try {
    ...
    if (!parsed.success) {
      return makeErrorResponse(400, "Invalid plan ID.");
    }
    ...
    if (!plan) {
      return makeErrorResponse(400, "Billing plan not found.");
    }
    ...
    return makeErrorResponse(500, "Failed to create payment session. Server error.");
  }
}
...
export const handler: Handler = async (event) => {
  ...
  return makeErrorResponse(404, "Billing endpoint not found.");
};
