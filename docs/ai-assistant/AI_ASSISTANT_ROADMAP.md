# AI Assistant Roadmap — KK Studio v1.5.5
...
1. 确认 `AGENTS.md` 和本文件存在，且都写明 **KK Studio v1.5.5**。
2. 修改 `scripts/governance/check-agent-docs.mjs`，把本文件加入必检。
3. 检查 token：`KK Studio v1.5.5`、`ToolRegistry`、`CanvasRuntimeState`。
...
```text
请严格读取并遵守 AGENTS.md 与 AI_ASSISTANT_CAPABILITY_OPTIMIZATION.md。当前项目版本必须保持 KK Studio v1.5.5，不要改错版本。

本轮只执行 Sprint 0：
1. 检查 AGENTS.md 与 AI_ASSISTANT_CAPABILITY_OPTIMIZATION.md 是否存在。
2. 修改 scripts/governance/check-agent-docs.mjs，把 AI_ASSISTANT_CAPABILITY_OPTIMIZATION.md 加入必检文件，并检查 token：KK Studio v1.5.5、ToolRegistry、CanvasRuntimeState。
3. 搜索旧文档中明显错误的 1.4.x、1.5.0、1.5.1 或与 v1.5.5 冲突的描述，只修正文档，不改业务代码。
...
```text
请继续严格遵守 AGENTS.md 与 AI_ASSISTANT_CAPABILITY_OPTIMIZATION.md。当前项目版本仍为 KK Studio v1.5.5。
...
7. 不要把版本写错；本项目当前是 **KK Studio v1.5.5**。
