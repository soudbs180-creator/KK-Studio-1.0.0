...
| 4.4 | Agent Run 中断恢复 | 部分 | `AgentRunStore.ts:173-199,344-377` 对认证 owner 保留 active Run，并按时间戳合并服务端权威投影；`agentRunHydration.ts:42-67` 校验 owner-scoped list，`agentRunEventRecovery.ts:150-274` 只对最近 20 个 active + synced Run 做 bounded cursor invalidation，并在详情成功合并后推进 owner-qualified sequence；`agentSessionProjection.ts` 已提供不接触 Chat storage 的 owner-scoped Session list/detail 只读投影。migration 022 与 `agent-run-write-store.js` 已提供同 owner、首次绑定后不可改绑/解除的可选 Run/Session binding；`chatAgentSessionProjection.ts` 以显式 Asset/summary/token/owner/time 证据守住写入资格，`chatCanonicalAssetResolver.ts` 复用 owner-scoped Asset API 生成内容寻址 canonical id。远端独有计划仍是不可执行 projection，Chat 安全写投影、summary/TokenBudget 权威生产、binding 激活和语义事件 replay 仍未实现。 | upgrade |
