...
| 4.4 | Agent Run 中断恢复 | 部分 | `AgentRunStore.ts:173-199,344-377` 对认证 owner 保留 active Run，并按时间戳合并服务端权威投影；`agentRunHydration.ts:42-67` 校验 owner-scoped list，`agentRunEventRecovery.ts:150-274` 只对最近 20 个 active + synced Run 做 bounded cursor invalidation，并在详情成功合并后推进 owner-qualified sequence；`agentSessionProjection.ts` 已提供 owner-scoped Session list/detail 投影并接收校验后的 write response。migration 022 与 `agent-run-write-store.js` 已提供同 owner、首次绑定后不可改绑/解除的可选 Run/Session binding；`chatAgentSessionWriteCoordinator.ts` 以 captured owner 和 expected auth subject 组合 detail/404、canonical Asset、summary、TokenBudget、strict mapper 与 upsert，保留非 Chat 权威状态且把 stale 回包视为服务端权威。远端独有计划仍是不可执行 projection；ChatSidebar 尚未激活协调器，binding 和语义事件 replay 仍未实现。 | upgrade |
| 4.5 | 跨设备续跑 | 部分 | 服务端 owner-scoped Run list/get、event query、Session list/get、Context Snapshot、可选 Run/Session owner binding 与 typed client 已在位，Web Run projection hydration + event cursor 已能发现并刷新第二个浏览器中的 active Run，Session list/detail 也在 startup、认证恢复与 online 时按 owner 严格刷新；owner 变化、跨 Run/乱序事件、陈旧详情和本地较新 pending snapshot 均 fail closed。Session write coordinator 已能安全 upsert 并 hydrate 权威响应，但 `AITakeoverContext.tsx` 仍不会执行远端计划，本地 Chat 默认不调用协调器或传 `sessionId`。binding 激活、跨设备执行接管和真实 E2E 仍缺失。 | upgrade |
