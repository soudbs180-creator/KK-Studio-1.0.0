...
| Chat 结构化 summary 与 TokenBudget 裁剪 | `apps/web/src/components/layout/chat-sidebar/session/chatContextCompression.ts` / `apps/web/src/components/layout/chat-sidebar/session/chatAgentContextBudget.ts` / `tests/unit/chat-context-compression.test.ts` / `tests/unit/chat-agent-context-budget.test.ts` |
