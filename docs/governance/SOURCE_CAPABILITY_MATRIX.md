...
| 4.2 | 多轮对话历史 | 部分 | `agentPlannerSessionContext.ts` 只从 exact owner-scoped Session detail 构造历史投影；`agentPlannerContext.ts` 排除已被 summary 覆盖的消息、历史 system/tool message 和执行授权字段，再把 bounded summary、最近 user/assistant 消息、工具结果、知识引用及通过 freshness/surface/canvas 门禁的 metadata-only Snapshot 交给 Planner。`agentPlannerReferencePolicy.ts` 只解析仍存在于当前画布的唯一历史选区，普通“继续”、模糊/多候选指代、历史 Job ID 和目标偷换均 fail closed。Run event 已有 strict metadata-only `step_outcome`，但完整 semantic replay、replan event 与真实 LLM 多轮验证仍缺失。 | upgrade |
...
| 4.4 | Agent Run 中断恢复 | 部分 | `AgentRunStore.ts:173-199,344-377` 对认证 owner 保留 active Run，并按时间戳合并服务端权威投影；`agentRunHydration.ts:42-67` 校验 owner-scoped list，`agentRunEventRecovery.ts:150-274` 只对最近 20 个 active + synced Run 做 bounded cursor invalidation，并在详情成功合并后推进 owner-qualified sequence；migration 023 与 `agent-run-event-store.js` 已让同一 owner-scoped 查询返回 strict `run_snapshot | step_outcome`，其中 step variant 只有关系型 verification metadata，Web mixed page 仍只触发权威 detail 重读。`agentSessionProjection.ts` 提供 owner-scoped Session/detail/latest Snapshot 投影并在 owner 切换时统一清空。migration 022 与 `agent-run-write-store.js` 提供同 owner、首次绑定后不可改绑/解除的可选 Run/Session binding；`chatAgentSessionWriteCoordinator.ts` 和 `chatAgentRunSessionBinding.ts` 只在完整证据通过后返回 binding。`AgentRuntime.ts` 又要求 exact detail 能生成 bounded authority-free context，并以 1.5 秒上限恢复 latest Snapshot、异步追加当前安全 capture；Snapshot 网络失败保留 Session 与 Run。远端独有计划仍是不可执行 projection，完整语义 replay、replan event 和真实跨设备 E2E 仍未实现。 | upgrade |
