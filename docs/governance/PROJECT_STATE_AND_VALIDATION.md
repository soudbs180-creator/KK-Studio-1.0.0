...
3. 上述 gate 通过后再扩展视频/音频 Worker；Phase 3 下一纵向切片让 `llmBrain.ts` / `localBrain.ts` 只消费已经通过预算和权威 Session 门禁的结构化上下文计划，不把本地 binding 扩张为语义 replay、远端执行授权或 ChatSidebar 全量切流。
