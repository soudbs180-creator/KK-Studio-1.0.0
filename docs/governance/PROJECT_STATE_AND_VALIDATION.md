...
3. 上述 gate 通过后再扩展视频/音频 Worker；Phase 3 下一纵向切片在已验证的 owner-stable Session write coordinator 之后，只为成功写入并 hydrate 权威 detail 的本地 Run 激活现有 Run/Session binding；不与 ChatSidebar 全量切流、语义 replay 或跨设备执行接管混入同一提交。
