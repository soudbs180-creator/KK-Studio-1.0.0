...
| Local Runner | `local-runner/` | 当前仅为 Browser/OpenCLI experimental runtime；typecheck/build/独立测试已进入验证链，已移除共享 fallback token 与启动 token 输出，收紧 loopback/JSON body 边界，并以 strict Zod command envelope 和递归脱敏审计限制本地命令输入与落盘元数据。ACL、轮换/配对、路径和媒体处理门禁仍未通过，仍不是生产媒体运行时。 |
