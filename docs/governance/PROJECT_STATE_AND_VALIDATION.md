...
| Capability Graph / Image Worker | `packages/shared/src/capability-graph/`、`packages/shared/src/generation-worker/`、`services/api/lib/capability-graph/`、`services/api/lib/generation-v3/worker/` | Capability Graph、Provider Connection、Google 安全迁移桥、generation/dispatcher dual-read 首段、image-slice 数据面准入、server image Worker 与 owner-scoped pending Job discovery/hydration 的代码基础已落地；Worker 新任务 admission 与存量 drain/cancel 已解耦，profile 全覆盖、全 Provider 切流、真实 migration rehearsal、灰度和恢复 E2E 仍待完成。 |
