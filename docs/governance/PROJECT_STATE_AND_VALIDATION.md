...
> 2026-07-24 当前事实：metadata-only `replan` 事件底座已由 migration 024 完成，只从 accepted plan replacement 推导 1–3 次计数与固定 reason/trigger code；Web 仅作 detail invalidation。下一 Phase 3 独立切片是 confirmation expiry，完整执行与接管仍未完成。
...
3. 上述 Phase 2 gate 通过后再扩展视频/音频 Worker。Phase 3 下一纵向切片独立实现 confirmation expiry：只消费 owner-bound Session confirmation metadata，并继续禁止 event metadata、本地 binding、历史 Snapshot 或过期 grant 获得远端执行授权；真实 replan executor 保持后续独立任务。
