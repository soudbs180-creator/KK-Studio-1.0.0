...
3. 上述 gate 通过后再扩展视频/音频 Worker；Phase 3 下一纵向切片应为已经预算化的 Session + Context Snapshot 补多轮指代语义回归，再单独设计 discriminated semantic event replay；不得把本地 binding 或历史 Snapshot 扩张为远端执行授权或 ChatSidebar 全量切流。
