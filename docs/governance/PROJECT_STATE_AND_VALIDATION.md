...
| Local Runner | `local-runner/` | 当前仅为 Browser/OpenCLI experimental runtime；typecheck/build/独立测试已进入验证链，首段安全切片已移除共享 fallback token 与启动 token 输出、收紧 loopback/JSON body 边界。ACL、轮换/配对、Zod、路径和媒体处理门禁仍未通过，仍不是生产媒体运行时。 |
...
> 2026-07-24 当前事实：metadata-only `replan` 事件底座与 confirmation expiry foundation 均已完成。bound Run confirmation 现在要求 owner-stable Session metadata proof，并绑定 plan、tool、target、expiry 及可用时的 quote/cost；真实 Quote 来源、semantic replay、replan executor 与跨设备执行接管仍未完成。
...
3. 上述 Phase 2 gate 通过后再扩展视频/音频 Worker。Phase 3 下一纵向切片是把 Planner 失败接入真实 bounded replan executor；confirmation expiry foundation 已完成，但真实 Quote 来源和确认卡/账本一致性须随 cost-bearing 链路独立闭环。event metadata、本地 binding、历史 Snapshot、远端 projection 或过期 grant 均不得获得执行授权。

### Required PR evidence

Phase 3 下一独立纵切应基于 migration 024 接入真实 bounded replan executor；不得把 metadata event、历史 plan、Session binding、Snapshot 或 Session confirmation record 直接升级为执行授权。migration 024 与既有 019/022/023 一样仍需受控 PostgreSQL 的空库、存量库、重复执行与 deploy probe 演练。
