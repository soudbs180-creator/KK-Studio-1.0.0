...
- 顶部三段切换必须由真实运行状态控制，不允许维护只改变高亮、不改变 Surface 的本地副本状态。
- `画布` 对应 Canvas；`Copilot` 对应现有 Chat/Agent Runtime；`创作` 对应现有 Asset Library 与生成能力。
- 创作模式复用资产能力并使用左侧 `262px` 资产面板，不新增参考站时间线，也不与项目选择器、竖向工具栏同时显示。
...
- 桌面竖向项目轨道是 Canvas 最左侧的一级入口，固定为
  `x=12px`、`top=50%`、`transform: translateY(-50%)`、`width=30px`；必须保持视口垂直居中，
  且不得因项目面板展开而横向移动。
