...
- **修改范围**：修复 `WorkflowGraph['metadata']` 缺乏 `largeCanvasLegacyNodesStripped` 等字段的类型检查错误，修复 `prompt-group-regroup-behavior`、`prompt-group-drag-layout`、`frontend-key-boundary-hardening` 与 `canvas-persisted-image-hydration-guard` 测试里对已弃用 regroup 逻辑、重构 Billing 启动和水合缓存恢复字段的过时正则断言，使本地测试能全部通过。
- **修改文件**：
