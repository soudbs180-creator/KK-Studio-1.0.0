...
import { AlertTriangle, CheckSquare, Square, Upload, X } from 'lucide-react';
...
const EcommerceAnalysisReviewPanel: React.FC<EcommerceAnalysisReviewPanelProps> = ({
  ...
  const activePromptPreview = activeReviewItem
    ? (activeTaskState?.resolvedPromptPreview || activeReviewItem.promptDraft || '暂无提示词')
    : '暂无提示词';
  ...
  const renderReferenceGallery = (
    ...
    <div className="rounded-xl border p-3" style={cardStyle}>
      ...
      <div
        ...
        className="mt-3 grid max-h-56 min-w-0 grid-cols-2 gap-2 overflow-y-auto pr-1 sm:grid-cols-3"
      >
  ...
  const renderActiveDetail = (reviewItem: ReviewItemDescriptor, compact = false) => (
    <div
      ...
      className="flex min-h-0 min-w-0 flex-col gap-3 overflow-hidden rounded-xl border p-3"
      style={cardStyle}
    >
  ...
  return (
    <div className="mb-2 overflow-hidden rounded-xl border p-3" style={containerStyle}>
      ...
      <div className="grid min-w-0 gap-3 md:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <div className="min-w-0 overflow-hidden">
          <div
            ...
            className="min-w-0 overflow-y-auto pr-1 max-h-[min(72vh,680px)]"
          >
