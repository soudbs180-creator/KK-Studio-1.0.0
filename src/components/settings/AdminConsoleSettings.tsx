...
const AdminConsoleSettings: React.FC = () => {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      if (error || data !== true) throw error || new Error('管理员密码修改失败');
      setOldPassword('');
      setNewPassword('');
      setConfirmPassword('');
  ...
  const handleRecharge = async () => {
    ...
    try {
      ...
      if (!row?.success) throw new Error(row?.message || '充值失败');
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard value="至少 8 位" label="密码策略" helper="建议尽快替换默认密码" tone="amber" />
        <MetricCard value={amountLabel} label="本次充值" helper="提交前可再次核对" tone="emerald" />
        <MetricCard value="管理员作用域" label="权限范围" helper="影响后台权限和用户资产" tone="rose" />
        <MetricCard value="3 项" label="可执行操作" helper="密码、充值、授予权限" tone="neutral" />
      </div>
        ...
        <SettingCard title="修改管理员密码">
          <div className="space-y-4">
            <div className="grid gap-3 md:grid-cols-3">
              <SettingInput label="旧密码" type="password" value={oldPassword} onChange={setOldPassword} placeholder="输入当前密码" />
              <SettingInput label="新密码" type="password" value={newPassword} onChange={setNewPassword} placeholder="至少 8 位" />
              <SettingInput label="确认新密码" type="password" value={confirmPassword} onChange={setConfirmPassword} placeholder="再次输入新密码" />
            </div>
            ...
            <div className="rounded-xl border border-[var(--border-light)] p-3 text-[13px] leading-6 text-[var(--text-secondary)]">
              推荐使用字母、数字和符号组合。修改后，后续重新解锁后台需要使用新密码。
            </div>
          </div>
        </SettingCard>
        ...
        <SettingCard title="给用户充值积分">
          <div className="space-y-4">
            <div className="grid gap-3 md:grid-cols-2">
              <SettingInput label="用户 ID 或邮箱" value={identity} onChange={setIdentity} placeholder="例如 user@example.com" />
              <SettingInput label="备注" value={rechargeRemark} onChange={setRechargeRemark} placeholder="可选备注" />
            </div>
            ...
            <div className="rounded-xl border border-[var(--border-light)] p-3 text-[13px] leading-6 text-[var(--text-secondary)]">
              目标用户：{identity.trim() || '尚未填写'}
              <br />
              充值额度：{amountLabel}
              <br />
              备注信息：{rechargeRemark.trim() || '管理员手动充值'}
            </div>
            <div className="flex gap-2">
      ...
      <SettingCard title="授予管理员权限">
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr),320px]">
          ...
          <div className="rounded-xl border p-3" style={{ backgroundColor: 'color-mix(in srgb, var(--warning) 12%, transparent)', borderColor: 'color-mix(in srgb, var(--warning) 28%, transparent)' }}>
