...
## 自动化测试与校验 (2026-05-25)
- [x] `npm run typecheck`：通过 (语义校验与 payment-server 语法检查全数通过)
...
- [x] `npm run build`：通过 (Vite 桌面端前端构建成功)
- [x] `npm run check:encoding`：通过 (编码校验无乱码字符)
- [ ] `npm run test:unit`：单元测试因部分遗留 src/ 路径仍有报错，将在里程碑 6 (测试路径迁移) 中统一重构与适配。已修复核心关联测试的类型及环境路径定义。

## 安全扫描
- [ ] 前端敏感 API 密钥扫描 (sk-, AIza, 密钥环境变量)：待运行 (里程碑 5 中将进行清除与二次硬化扫描)
- [ ] 直连官方域名过滤检测：待运行 (里程碑 5 中安全重构后进行)
