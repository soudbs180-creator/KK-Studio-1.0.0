...
## Active Runtime / PPT Gate

Use this gate for the active runtime/PPT work in this thread:
...
Runtime/PPT commits also require `npm.cmd run typecheck`, `npm.cmd run test:unit`, `npm.cmd run build`, `npm.cmd run governance:agent-docs`, and `npm.cmd run check:encoding` unless `status.md` records a specific blocker.
...
Run the Clay UI contract suite for every UI-lane change in the parallel UI thread:
