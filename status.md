...
- Alternate-git worktree was clean at `cedb3718` before this API settings unused cleanup pass.
...
- Current focus: M104 API settings unused cleanup has RED/GREEN targeted, filtered noUnused, full validation, and in-app browser smoke evidence; it is ready for path-limited staging/commit.
...
- Current commit scope: `src/components/settings/ApiSettingsView.tsx`, `tests/unit/api-settings-unused-cleanup-contract.test.ts`, `tests/unit/api-settings-view-source-guard.test.ts`, `tests/unit/api-settings-encoding-guard.test.ts`, `tsconfig.tests.json`, and the four ledger files. Provider routing behavior, keyManager secret storage, storage persistence behavior, release metadata, generation/PPT runtime behavior, and unrelated architecture helpers remain excluded.
- Browser QA: passed for M104 through the Codex in-app Browser on `http://127.0.0.1:3100/settings/api-management?qa=api-settings-unused-cleanup-m104`; after using the local temporary-user entry, the API settings route rendered `API 配置` / `API 供应商`, the title was `KK Studio - AI Image Workspace`, and current-tab console error count was `0`.
...
1. Remaining noUnused cleanup after M104 is 8 diagnostics: `src/services/auth/keyManager.ts` 6 and `src/services/model/secureModelProxy.ts` 2 source-contracted local route-gate helpers. The two `secureModelProxy.ts` helpers are intentional route-gate exceptions; do not remove them blindly.
2. Next cleanup candidate should be chosen from a fresh TS6133 cluster scan after the M104 commit. Keep the next slice separate from keyManager secrets/cloud sync, provider routing, storage persistence, release metadata, and route-gate helper cleanup unless explicitly scoped.
