...
- Current hotfix override (2026-05-07): close the user-reported local-account logout regression. Explicit logout must return to the initial login page instead of remaining on the startup/session-check surface, and the startup warning surface must use readable app-owned tokens outside settings scope.
- Current commit scope: `src/context/AuthContext.tsx`, `src/components/common/AppStartupScreen.tsx`, `src/index.css`, `tests/unit/workspace-auth-gate.test.ts`, `tests/unit/settings-entry-surface-style-regression.test.ts`, and ledger files only. Endpoint selection, auth/header behavior outside session recovery, fetch execution, polling, billing, fallback ordering, key storage, provider routing, release metadata, payment/server behavior, ecommerce behavior, and unrelated runtime extraction work are excluded.
- Root cause: explicit logout cleared runtime auth, then hosted/no-token session recovery could re-enter the startup loading path and keep `AuthenticatedAppShell` on `AppStartupScreen` before the login gate. The warning text also borrowed settings-scope/fallback warning colors, which made the startup warning hard to read outside the settings panel.
- Current targeted validation: RED was confirmed first by extending `tests/unit/workspace-auth-gate.test.ts` for explicit logout blocking hosted/session recovery until a new runtime user appears, and by extending `tests/unit/settings-entry-surface-style-regression.test.ts` for app-owned startup screen tokens/test id. The tests failed before the fix because `sessionRecoveryBlockedBySignOut` and `data-testid="app-startup-screen"` were missing. GREEN passed after `AuthContext` blocked session recovery on explicit logout and `AppStartupScreen` stopped borrowing settings-scope card/warning tokens.
- Browser QA for the current auth logout hotfix: completed through the Codex in-app Browser against the live local Vite app at `http://127.0.0.1:4313/?qa=auth-clean-login-1778130000003`. Clean local storage first showed the startup check briefly, then the initial login page with `app-startup-screen` count 0. A local temporary user entered the workspace, storage selection was saved, the tutorial overlay was closed, the desktop user menu exposed `退出登录`, and clicking it returned to the login page with `app-startup-screen` count 0, login button count 1, temporary local login button count 1, workspace prompt count 0, `.theme-transitioning` count 0, stale chunk text count 0, and page console error count 0.

- Active lane in this thread: user-reported UI regression closure after M130, with the current hotfix scoped to the ecommerce main-image ratio selector. Stage One M6 and Stage One Backfill are complete; the post-M120 UI closure line is split and committed in `da4ffc79`, `485a6bef`, and `1ca080eb`; M121-M130 are committed through `b37cf885`.
- Current slice override: close the user's visible ecommerce main-image ratio selector regression. The `自适应`, `1:1`, and `3:4` choices must render in one equal-width row. Endpoint selection, auth/header behavior, fetch execution, polling, billing, fallback ordering, key storage, provider routing, release metadata, payment/server behavior, and unrelated runtime extraction work are excluded.
- Clay UI audit closure landed in `9e7ae2b5` and is no longer the active lane.
- Current branch: `main`.
- Plain `.git` still reports a stale historical view. The writable full Git metadata copy at `node_modules/.codex-git-full` is the only development fact source; the latest committed code baseline before M130 is `740042c1 fix: redact key update diagnostics`. Use `git --git-dir=node_modules/.codex-git-full --work-tree=.` for status/staging/commits in this session.
...
- Current focus: finish validation, browser QA, ledger sync, and commit for the ecommerce ratio row hotfix.
...
- Current commit scope: `src/components/image/ImageOptionsPanel.tsx`, `tests/unit/prompt-bar-layout-regression.test.ts`, `tests/unit/prompt-bar-ecommerce-footer-controls.test.ts`, and ledger files only. Existing dirty files outside this scope remain excluded: `src/index.css`, `tests/unit/clay-frosted-surface-contract.test.ts`, and `tests/unit/settings-desktop-workbench-regression.test.ts`.
- Current targeted validation: RED was confirmed first by adding `ecommerce main-image ratio choices stay in one equal three-column row` to `tests/unit/prompt-bar-layout-regression.test.ts`; it failed against the old `autoInGrid = hasAuto && isOddCount` layout. GREEN passed after the `resolveRatioLayout()` fix. The expanded focused UI/ecommerce gate passed 30/30: `tests/unit/prompt-bar-layout-regression.test.ts`, `tests/unit/settings-desktop-workbench-regression.test.ts`, `tests/unit/prompt-bar-ecommerce-framework-companion.test.ts`, `tests/unit/ecommerce-build-runtime-contract.test.ts`, `tests/unit/ecommerce-canvas-contract.test.ts`, `tests/unit/clay-frosted-surface-contract.test.ts`, `tests/unit/prompt-bar-ecommerce-footer-controls.test.ts`, and `tests/unit/ecommerce-sheet-settings-runtime-contract.test.ts`.
- Browser QA for the current ratio hotfix: completed through the Codex in-app Browser against the live local Vite app at `http://127.0.0.1:4310/?qa=ecommerce-ratio-row-1778129221556`. Dark theme was active (`body.dark-mode` count 1). The ecommerce PromptBar options panel opened from the `主图 A+` control, and the `主图比例` row rendered exactly one `自适应`, one `1:1`, and one `3:4` button in a single horizontal row with consistent visual widths. `.theme-transitioning` count was 0, stale chunk text count was 0, and page console error count was 0.

## Current Finalization Gate

- Current auth logout/startup readability hotfix gate results: focused RED/GREEN passed, expanded auth/startup focused suite passed 20/20, `npm.cmd run governance:agent-docs` passed before and after ledger sync, `npm.cmd run check:encoding` passed before and after ledger sync, `npm.cmd run typecheck` passed, `npm.cmd run build` passed, `npm.cmd run test:unit` passed 1401/1401, path-limited alternate-git `diff --check` passed with LF/CRLF normalization warnings only, and browser QA was completed through the Codex in-app Browser as recorded in Active State.
...
Fresh remaining-work assessment after `740042c1`, the M129 keyManager update diagnostic redaction, and the current M130 OpenAI-compatible diagnostics prompt redaction:

- Current fact source: the latest committed code baseline before M130 is `740042c1 fix: redact key update diagnostics`; exact HEAD after this slice must be read from alternate git.
- Current finalization gate: `governance:check`, `architecture:check`, `governance:security`, `spec:check`, `audit:dependencies`, `typecheck`, `test:unit` 1373/1373, `build`, `check:encoding`, the strict no-unused probe, and the settings browser/manual smoke checks passed before M124. M124 separately passed strict TS, architecture, typecheck, `test:unit` 1377/1377, build, agent-docs, and encoding. M125 separately passed targeted/adjacent gates, strict TS, architecture, typecheck, `test:unit` 1381/1381, build, agent-docs, encoding, and path-limited diff check. M126 separately passed targeted/adjacent gates, strict TS, architecture, security governance, typecheck, `test:unit` 1384/1384, build, agent-docs, and encoding. M127 separately passed targeted/adjacent gates, strict TS, architecture, typecheck, `test:unit` 1389/1389, build, agent-docs, encoding, and path-limited diff check. M128 separately passed focused 4/4, source-proof scan, strict TS, architecture, typecheck, `test:unit` 1390/1390, build, agent-docs, encoding, and path-limited diff check. M129 separately passed RED/GREEN, targeted 14/14, strict TS, architecture, security governance, typecheck, `test:unit` 1393/1393, build, agent-docs, encoding, and path-limited diff check. M130 focused RED/GREEN is complete; full gate is pending before commit.
...
1. The current noUnused filter is clean; the route-gate helpers are wired into live entrypoints; M107-M130 moved the latest OpenAI-compatible, server, storage/logging, key diagnostic, and prompt diagnostics cleanup seams into focused modules or source contracts; M118 and M122 closed the latest confirmed security blockers.
2. M130 removes prompt/message/content values from OpenAI-compatible diagnostics previews and snippets while preserving actual request execution behavior.
3. The next ordinary cleanup candidate should be chosen from the read-only candidates: OpenAI-compatible error helpers, prompt optimizer cache redaction, or shared user-route auth inference. `PromptBar.tsx` attachment/drag/reference-image ingestion remains available but requires browser QA because it touches file-input UI behavior.
