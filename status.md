...
- Active lane in this thread: user-reported UI regression closure after M130. Stage One M6 and Stage One Backfill are complete; the post-M120 UI closure line is split and committed in `da4ffc79`, `485a6bef`, and `1ca080eb`; M121-M130 are committed through `b37cf885`.
- Current slice override: close the user's visible PromptBar, ecommerce canvas card, settings chrome, and mobile footer regressions. Endpoint selection, auth/header behavior, fetch execution, polling, billing, fallback ordering, key storage, provider routing, release metadata, payment/server behavior, and unrelated runtime extraction work are excluded.
...
- Current focus: finish validation, browser QA, ledger sync, and commit for the user-reported UI regressions.
...
- Current commit scope: `src/app/useEcommerceBuildRuntime.ts`, `src/components/canvas/PromptNodeComponent.tsx`, `src/components/ecommerce/EcommerceCanvasWorkbenchCard.tsx`, `src/components/layout/PromptBar.tsx`, `src/components/layout/prompt-bar/DesktopComposerModePanel.tsx`, `src/index.css`, `src/types.ts`, focused UI/ecommerce source-contract tests, and ledger files only.
- Current targeted validation: `node --import ./scripts/test/set-log-level.mjs --test --test-isolation=none tests/unit/prompt-bar-layout-regression.test.ts tests/unit/settings-desktop-workbench-regression.test.ts tests/unit/prompt-bar-ecommerce-framework-companion.test.ts tests/unit/ecommerce-build-runtime-contract.test.ts tests/unit/ecommerce-canvas-contract.test.ts` passed 17/17 after the UI fixes. The related Clay/PromptBar/settings contract set including `tests/unit/clay-frosted-surface-contract.test.ts` passed 19/19 after the old heavy-shadow Clay assertions were aligned with the user's requested flat-shadow UI.
- Browser QA for the current UI slice: completed through the Codex in-app Browser against the freshly built `dist/` served at `http://127.0.0.1:4310/` by a temporary read-only Node static server. Dark theme was active (`body.dark-mode`). Workspace check at `http://127.0.0.1:4310/?ui-regression-qa=1778100000001` rendered the ecommerce input surface and one PromptBar footer with `data-mobile-action-overflow-policy="single-row-primary-secondary-drawer"`; `.theme-transitioning` count was 0, stale chunk text count was 0, and page console error count was 0. Settings check after local temporary-user entry at `http://127.0.0.1:4310/settings/api-management` rendered the mobile settings shell (`settings-shell-mobile` count 1) with 3 reference cards and 1 section card; `.theme-transitioning` count was 0, stale chunk text count was 0, and page console error count was 0. A follow-up root-route reload on the same static server paused on the startup overlay without console errors, so the live repeated workspace re-entry is recorded as a static QA limitation rather than a production UI verdict; source contracts and the first workspace browser pass cover the PromptBar/ecommerce visible surface.

## Current Finalization Gate

- Current UI regression gate results: focused UI/ecommerce gate passed 17/17; related Clay/PromptBar/settings gate passed 19/19; `npx.cmd tsc --noEmit --noUnusedLocals true --noUnusedParameters true --pretty false` passed; `npm.cmd run typecheck` passed; `npm.cmd run test:unit` passed 1394/1394; `npm.cmd run build` passed; `npm.cmd run governance:agent-docs` passed; `npm.cmd run check:encoding` passed; path-limited alternate-git `diff --check` passed with LF/CRLF normalization warnings only. Browser QA was completed through the Codex in-app Browser as recorded in Active State.
