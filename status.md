...
- Alternate-git worktree was clean at `96b94e5e` before this ledger alignment pass.
...
- Current focus: start the compiler/source-proven unused cleanup seam narrowed to `src/components/layout/PromptBar.tsx` and `src/components/image/ImageCard2.tsx`. Do not touch `src/App.tsx` unless a fresh diagnostic produces App-specific proof.
...
- Current commit scope: ledger-only alignment after `96b94e5e`. UI components, release metadata, payment business logic, provider routing changes, endpoint/auth changes, diagnostics preview helpers, and unrelated architecture helpers remain excluded from this docs commit.
...
1. Recommended next slice from read-only seam review: compiler/source-proven unused locals/imports narrowed to `src/components/layout/PromptBar.tsx` and `src/components/image/ImageCard2.tsx`; do not touch `src/App.tsx` unless a fresh diagnostic produces App-specific proof.
2. Run a fresh filtered TypeScript `noUnusedLocals` probe for `PromptBar.tsx`, `ImageCard2.tsx`, and `App.tsx` before deleting anything.
...
- The alternate-git worktree was clean at `96b94e5e` before this ledger-only alignment pass, but any staging must still be explicit path-based and reviewed before commit.
