...
- Alternate-git worktree was clean at `d8845775` before this model display-name provider-parameter cleanup pass.
...
- Current focus: finish model display-name TS6133 cleanup by renaming the unused `getModelDisplayName` compatibility provider parameter while preserving display-name behavior and third-argument call shape.
...
- Current commit scope: `src/services/model/modelCapabilities.ts`, `tests/unit/model-display-name-regression.test.ts`, `tsconfig.tests.json`, and the four ledger files. Model/provider routing behavior, admin model lookup behavior, keyManager/model-list behavior, API/settings UI files, endpoint/auth changes, release metadata, payment business logic, PPT behavior, storage behavior, and unrelated architecture helpers remain excluded.
- Browser QA: skipped for this M82 slice because it only renames an unused service compatibility parameter and updates tests/docs; it does not change JSX/CSS or browser-visible behavior.
...
- Current giant tracked files after the M82 working tree: `src/index.css` 13552 physical lines, `src/App.tsx` 4812, `src/services/auth/keyManager.ts` 4338, `src/services/llm/OpenAICompatibleAdapter.ts` 4337, `src/components/layout/PromptBar.tsx` 3989, `src/components/settings/ApiSettingsView.tsx` 3385, `src/components/layout/ChatSidebar.tsx` 2743, `src/app/useGenerationRuntime.ts` 2603, `src/context/CanvasContext.tsx` 2516, `src/components/canvas/PromptNodeComponent.tsx` 2244, `src/services/model/modelCapabilities.ts` 1806, `src/services/storage/fileSystemService.ts` 1577, `src/app/useWorkflowActions.ts` 498, `src/components/settings/ProjectManager.tsx` 842, `src/components/Onboarding/OnboardingManager.tsx` 262, `src/components/Onboarding/OnboardingOverlay.tsx` 200, `src/components/Onboarding/useOnboardingProgress.ts` 137, and `src/components/Onboarding/AchievementToast.tsx` 60. Current SystemLogs line counts: `SystemLogsView.tsx` 421 and `SystemLogsView.localized.tsx` 450.
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 423, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The filtered noUnused probe now shows 61 TS6133 diagnostics plus 4 TS619x diagnostics. These are refactor debt indicators, not release blockers by themselves.
...
## Active M82 Slice (Model Display Name Provider Parameter Cleanup)
...
1. Close the current model display-name provider-parameter cleanup slice first; do not claim final completion while additional Stage Two seams and remaining TS6133/TS619x hotspots remain open.
...
1. Remaining noUnused cleanup hotspots are now outside App, ChatSidebar, PromptNode, SystemLogs, Onboarding, deleted imageCompression, deleted PixiCanvas, dormant/live canvas residual files, workflow-actions import cleanup, common ErrorBoundary cleanup, generation runtime import cleanup, CanvasContext type-import cleanup, ProjectManager prop-destructure cleanup, ecommerce taskMerger parameter cleanup, and model display-name provider-parameter cleanup. The largest clusters are `useImageGeneration.ts`, `geminiService.ts`, `ApiSettingsView.tsx`, `keyManager.ts`, and `GoogleAdapter.ts`. Handle each as a separate minimal slice with a source contract.
2. Next cleanup candidate should be chosen from a fresh TS6133 cluster scan after M82. Current lower-risk candidates include video-service import/parameter cleanup or isolated storage/image helper locals. Keep the next slice separate from storage persistence, API/key settings, Gemini/provider routing, billing/payment, and keyManager work unless explicitly scoped.
...
- The alternate-git worktree was clean at `e714380b` before this common error-boundary cleanup pass, but any staging must still be explicit path-based and reviewed before commit.
