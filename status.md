...
- Active lane: make user BYOK/API settings local-first through the backend/local API payload path, with browser-side raw-secret persistence disabled for both signed-in and sessionless local users.
- Included files for this slice: `src/services/auth/keyManager.ts`, `src/services/auth/keyManagerProviders.ts`, `src/components/settings/ApiSettingsView.tsx`, `src/services/api/userApiViewState.ts`, `src/vite-env.d.ts`, `scripts/governance/check-sensitive-boundaries.mjs`, `tests/unit/frontend-key-boundary-hardening.test.ts`, `tests/unit/key-manager-cloud-sync.test.ts`, `tests/unit/key-manager-runtime-fallback.test.ts`, `tests/unit/key-manager-dead-code-pruning-contract.test.ts`, `tests/unit/user-api-view-state.test.ts`, `plans.md`, `implement.md`, `status.md`, and `validation.md`.
- Excluded files/artifacts: provider secrets, user API keys, `.tmp/`, generated `dist/`, generated `apps/admin/dist/`, screenshots/output artifacts, deployment credentials, unrelated canvas/PromptBar/payment files, and production DB rows.
- Root cause evidence: the system/provider credit-key path was already backend-held and sanitized on reads, but the user BYOK path still had an old sessionless local branch that could write `KeyManagerState` and providers to browser `localStorage` when loopback/local runtime was active. The API settings UI also exposed a "browser session draft" editing mode when the local API was offline or degraded.
- Implemented fix: `keyManager` now purges anonymous sensitive caches, never hydrates user/temp-user secrets from scoped browser `localStorage`, never writes user/temp-user slots to browser `localStorage`, and sends pending slot/provider changes through the local API payload bridge for every `userId`, including temp/local users. Provider persistence now removes scoped browser provider caches and returns `"none"` instead of storing raw provider configs. The API settings UI disables sessionless browser draft mode and requires a reachable, persistent local API bridge before local/temp BYOK edits. `resolveUserApiViewState` now fails closed when the local API is unreachable even if a caller accidentally passes sessionless actions as enabled. `VITE_API_KEY` was removed from frontend env typing, and `governance:security` now rejects unallowlisted `VITE_*KEY/SECRET/TOKEN` names in runtime source.
- Security audit evidence: read-only subagent audits found no current system provider key exposure in main frontend bundles, localStorage, normal contracts, public active-model responses, or admin list responses. The remaining material issue was user BYOK/browser-local persistence, which this slice closes for the API settings surface.
- Fresh validation evidence: targeted security boundary suite passed 50/50; `npm.cmd run governance:security` passed; `npm.cmd run typecheck` passed; full `npm.cmd run test:unit` passed 1471/1471; `npm.cmd run build` passed and produced `dist/assets/index-DMW4Ppn9.js`; `npm.cmd run governance:agent-docs` passed; `npm.cmd run check:encoding` passed; path-limited alternate-git `diff --check` passed with LF/CRLF normalization warnings only.
- Production deploy evidence for this slice: `cmd /c npx vercel deploy --prod --yes` created `https://kk-studio-bikxw95np-yykks-projects-727e9560.vercel.app`; the upload command ended with a transient Vercel API `socket hang up`, but follow-up `vercel inspect` showed deployment `dpl_D8tgr52sz26mshnG34Dd3LoJiJPW` reached Production / Ready. `vercel alias set` successfully pointed both `https://kkai.plus` and `https://www.kkai.plus` to that deployment. Live `https://kkai.plus/app-version.json` returns build time `2026-05-22T06:58:20.303Z`, and live `https://kkai.plus/` returns `200` HTML.
- Production API smoke for this slice: `https://kkai.plus/healthz`, `https://kkai.plus/api/healthz`, and direct VPS `https://172-245-156-16.sslip.io/healthz` return `200` JSON with `selfHostedCoreReady=true`, valid PostgreSQL config, and `hasUserApiEncryptionSecret=true`. `https://kkai.plus/api/v1/model-catalog/models?kind=image` returns public `Nano Banana`; `/api/v1/model-catalog/active` and `/active-credit-models` still return `200` with `items: []`, confirming the remaining no-model condition is missing production provider data, not a frontend/backend/VPS connection failure.
- Bundle boundary smoke: local built `dist/assets/index-DMW4Ppn9.js` contains no `VITE_API_KEY` or `kk_studio_key_manager` literal matches. A live bundle fetch was flaky from this Windows network path, but the deployed `app-version.json` build time matches the local post-fix production build.
- VPS/admin deploy evidence: the admin static build from commit `89e5dd81` was packaged to ignored `.tmp/kk-admin-dist-89e5dd81.tar` and deployed to `/var/www/kk-admin` on `172.245.156.16`; remote `nginx -t` and reload passed. `http://172.245.156.16:4174/login`, `/providers`, and `/assets/index-DYGx7sqD.js` return `200`, while direct VPS `https://172-245-156-16.sslip.io/healthz` returns `200` with `selfHostedCoreReady=true`. Direct VPS `/api/healthz` remains `404` because that route is a Vercel wrapper route, not the VPS API route.
- Remaining risk/follow-up: broader non-secret browser persistence still exists for canvas, image cache, UI preferences, chat/task history, and compatibility tokens. That is a separate storage-ownership milestone if the product requirement is "all user data only under the initially selected local path, with cloud backup as explicit recovery." This slice closes raw user API secret persistence in browser storage and keeps system keys backend-side.
- Next step: stage and commit only the security-boundary slice, then save a real provider key/model row through the admin provider UI and re-smoke `/api/v1/model-catalog/active-credit-models`.

## Current Admin Credit-Provider Bootstrap Fix

- Active lane: admin model/provider configuration unblock for the production "no available model" issue after the UI density deployment.
- Included files for this slice: `apps/admin/src/features/providers/providerEditorModel.ts`, `apps/admin/src/pages/AdminProvidersPage.tsx`, `apps/admin/src/styles/admin.css`, `apps/api/src/modules/model-catalog/infrastructure/postgres-credit-provider-repository.ts`, `apps/api/src/modules/model-catalog/infrastructure/in-memory-credit-provider-repository.ts`, `packages/contracts/src/dto/model-catalog.ts`, `tests/unit/admin-providers-page.test.ts`, `plans.md`, `implement.md`, `status.md`, and `validation.md`.
- Excluded files/artifacts: generated `dist/`, `apps/admin/dist/`, `output/`, temp browser/static-server state, production DB rows, provider secrets, unrelated PromptBar/canvas/runtime files, payment code, and deployment credentials.
- Root cause evidence: live `https://kkai.plus/api/healthz` returns `200` with PostgreSQL-backed `creditProviders`, and `/api/v1/model-catalog/models?kind=image` returns public `gemini-2.5-flash-image` / `Nano Banana`; however, `/api/v1/model-catalog/active` and `/api/v1/model-catalog/active-credit-models` return `200` with `items: []`. Code inspection and two subagent reports agree that active/system-credit models come from `admin_credit_models where is_active = true`; the public catalog is not an executable route source. Ecommerce mode further filters to ecommerce-approved models such as `gemini-3.1-flash-image-preview`, so public `gemini-2.5-flash-image` alone would still not populate the ecommerce picker.
- Implemented fix: the admin provider editor can create a first `system-image-provider` draft with base URL `https://future-api.vodeshop.com`, active `gemini-3.1-flash-image-preview` / `Nano Banana 2`, 1K/2K/4K credit pricing, priority/weight `100`, and a write-only new-key textarea. Existing providers still retain key fingerprints without exposing raw key values. Admin DTOs now carry route `priority` and `weight`, and save payload construction preserves those values instead of flattening them to zero.
- RED/GREEN evidence: `tests/unit/admin-providers-page.test.ts` first failed because `createDefaultProviderEditorState` did not exist. A second RED confirmed the missing `hasProviderApiKeysForSave` guard. After the fix the focused admin provider test passed 5/5.
- Related validation evidence: `tests/unit/credit-provider-routes.test.ts` passed 3/3; `tests/unit/postgres-credit-provider-repository.test.ts` passed 5/5; `tests/unit/admin-model-service-credit-routes.test.ts` passed 2/2; `npm.cmd run admin:build` passed and produced `apps/admin/dist/assets/index-DYGx7sqD.js`; `npm.cmd run typecheck` passed with 133 semantic test files; `npm.cmd run governance:agent-docs` passed; `npm.cmd run check:encoding` passed; path-limited alternate-git `diff --check` passed with LF/CRLF normalization warnings only. Full `npm.cmd run build` passed and produced `dist/assets/index-CpTmd36I.js`; full `npm.cmd run test:unit` passed 1471/1471.
- Browser QA evidence: a local static server served the built `apps/admin/dist` at `http://127.0.0.1:4176/providers` with an injected admin browser session for inspection only. The in-app Browser DOM showed `System Providers`, `Create system provider`, and after clicking it showed `system-image-provider`, `https://future-api.vodeshop.com`, API key input, `gemini-3.1-flash-image-preview`, `Nano Banana 2`, `Save provider`, credits `12`, priority `100`, weight `100`, and checked `Active`. In-app screenshot capture timed out on `Page.captureScreenshot`, so the browser evidence is DOM state rather than a saved image.
- Production deploy evidence after code fix: `cmd /c npx vercel deploy --prod --yes` completed on 2026-05-22 and produced `https://kk-studio-ge7pnllju-yykks-projects-727e9560.vercel.app`. `vercel alias set` then successfully pointed both `https://kkai.plus` and `https://www.kkai.plus` at that deployment. Live `https://kkai.plus/app-version.json` now returns build time `2026-05-22T01:16:49.789Z`.
- Production API smoke after the alias update: live `https://kkai.plus/healthz` and `/api/healthz` return `200` with `selfHostedCoreReady=true`, valid PostgreSQL config, and `creditProviders=postgres`. `https://kkai.plus/api/v1/model-catalog/models?kind=image` returns public `gemini-2.5-flash-image` / `Nano Banana`. Live `/api/v1/model-catalog/active` and `/active-credit-models` still return `200` with `items: []` because no real production admin provider row/key has been saved yet. This commit unblocks the admin configuration path; it does not inject production secrets or mutate production DB rows.
- VPS admin static deploy status: completed after the user supplied root access for this deploy. The ignored `.tmp/kk-admin-dist-89e5dd81.tar` package was uploaded to the VPS, extracted to `/var/www/kk-admin`, and Nginx reloaded after `nginx -t` passed. `http://172.245.156.16:4174/login`, `/providers`, and `/assets/index-DYGx7sqD.js` return `200`.
- Next step: use the admin provider UI to save the real provider key/model row and re-smoke `/api/v1/model-catalog/active-credit-models` until it returns at least one active model.
...
- Included files for this slice: `index.html`, `src/components/common/AppStartupScreen.tsx`, `src/components/ecommerce/EcommerceImportPanel.tsx`, `src/index.css`, `tests/unit/settings-entry-surface-style-regression.test.ts`, `tests/unit/app-startup-screen-localization.test.ts`, `tests/unit/ecommerce-import-panel-density-contract.test.ts`, `tests/unit/tailwind-utility-cascade-contract.test.ts`, `status.md`, and `validation.md`.
...
- Root cause evidence: `AppStartupScreen` had been reduced from the full restore launch hall to a `420px` prompt card, removing the title, subtitle, brand mark, status list, and 640px shell contract. Browser/screenshot follow-up showed the ecommerce issue had three layout causes. First, the import grid used the page-level `md:` breakpoint inside a fixed-width PromptBar, so at the in-app Browser viewport (`735px`) the 718px PromptBar rendered the upload cards as one stacked column (`grid h=244`, cards full-width), making the frame feel tall and sparse. Second, `src/index.css` imported Tailwind and then applied an unlayered global reset `* { margin: 0; padding: 0; }`; with Tailwind v4 cascade layers, that reset overrode `px-*` and `py-*` utilities. Third, live CSSOM inspection after moving the `src/index.css` reset into `@layer base` proved `index.html` still had its own unlayered inline `* { margin: 0; padding: 0; }`, so panel/cards/chips still computed `padding: 0px` and `mb-*`/`mt-*` stayed at `0px` while buttons only looked padded because of a separate global `button, [role="button"]` touch-target rule.
- Implemented fix: the startup tests again require the branded full launch hall and restore copy. The startup component keeps token-colored title/subtitle/eyebrow rendering. Ecommerce import file cards now use a component-width-aware `auto-fit` grid (`minmax(min(100%, 196px), 1fr)`) instead of a viewport breakpoint, reduce vertical rhythm (`py-2.5`, `gap-2`, `leading-4`, 36px icons, 56px thumbnails), and keep chips plus clear/analyze actions non-wrapping. The global reset now lives inside `@layer base`, and the `index.html` bootstrap style no longer sets global margin/padding, letting Tailwind utility spacing keep precedence over reset defaults.
...
- Fresh validation evidence: focused density cascade suite passed 2/2 after the `index.html` guard was added; `npm.cmd run typecheck` passed with 133 semantic test files; `npm.cmd run build` passed and produced `dist/assets/index-BD0WEJrr.css` plus `dist/assets/index-CpTmd36I.js`; `npm.cmd run check:encoding` passed. The built CSS keeps reset rules layered, and the bootstrap HTML no longer has an unlayered global margin/padding reset.
- Browser QA evidence: Codex in-app Browser opened `http://127.0.0.1:3000/?ui-density-check=1779267073759` and exposed the ecommerce import surface. Live CSSOM inspection proved the failing cascade: `index.html`'s unlayered `*` rule was the last effective margin/padding rule for the panel/card/chip targets, while Tailwind's `.px-3`/`.py-2.5` utilities existed inside the `utilities` layer. After removing the inline reset and tuning the grid minimum to `196px`, a reload showed panel/card padding restored to `10px 12px`, `mb-2` restored to `8px`, `mt-2` restored to `8px`, the grid returned to 3 columns (`206px 206px 206px`), ecommerce panel height dropped from `315px` to `225px`, and PromptBar height dropped from `592px` to `502px` at the current in-app Browser width. Full-page screenshot capture timed out in the in-app Browser, so the claimed browser evidence is computed geometry and CSSOM, not a saved screenshot artifact.
- Production deploy evidence: `npx.cmd vercel deploy --prod -y --scope yykks-projects-727e9560` completed on 2026-05-21 and produced `https://kk-studio-g58urf6uy-yykks-projects-727e9560.vercel.app` / deployment `dpl_5yga2WPNKLvFryoM2FNjYo3e2q1e`. `vercel alias set` then successfully pointed both `https://kkai.plus` and `https://www.kkai.plus` at that deployment. Fresh `vercel inspect https://kkai.plus --scope yykks-projects-727e9560` reports the deployment as Production / Ready, created 2026-05-21 15:04:46 GMT+0800. Live `app-version.json` returns version `1.4.6`, build time `2026-05-21T07:04:58.419Z`, and the live page loads `assets/index-X2s0zxS9.css` plus `assets/index-DJnSnAkL.js`; the live HTML inline style no longer contains global `* { margin: 0; padding: 0; }`.
- Production API smoke evidence: same-origin `https://kkai.plus/healthz`, `https://kkai.plus/api/healthz`, and `https://kkai.plus/api/manifest` return `200` JSON; unauthenticated `https://kkai.plus/api/v1/auth/session` returns the expected `401 AUTH_REQUIRED` JSON; direct VPS `https://172-245-156-16.sslip.io/healthz`, `/api/manifest`, and `/api/v1/auth/session` also respond as expected. Health data shows `selfHostedCoreReady=true`, valid PostgreSQL config, and repository backends `adminConsole`, `authData`, `creditAccounts`, `creditExchangeRates`, `creditProviders`, and `workspaceLayout` all on `postgres`.
- API connection diagnosis: the API/proxy path is connected. The remaining user-visible "no available model" condition is data/configuration, not transport: `https://kkai.plus/api/v1/model-catalog/models?kind=image` returns the public image model `gemini-2.5-flash-image` / `Nano Banana`, while `https://kkai.plus/api/v1/model-catalog/active` and `/active-credit-models` return `200` with `items: []`. Code inspection confirms the active list reads `admin_credit_models where is_active = true`, so production currently has no active admin credit-provider model rows exposed to the workspace picker.
- Next step: configure or seed an active admin credit provider/model row in production `admin_credit_models` through the admin model settings flow or a controlled DB migration, then re-smoke `/api/v1/model-catalog/active-credit-models` until it returns at least one provider/model.
...
- Production deploy evidence: after commit `1c27d4f2 fix: restore prompt bar footer frost bounds`, `npx.cmd vercel deploy --prod -y --scope yykks-projects-727e9560` created deployment `dpl_8GrbT468nTemUAaqML1yn4pMLaEQ` at `https://kk-studio-c8b96hx3z-yykks-projects-727e9560.vercel.app`. The initial deploy did not move the custom domain, so `npx.cmd vercel alias set kk-studio-c8b96hx3z-yykks-projects-727e9560.vercel.app kkai.plus --scope yykks-projects-727e9560` and the same command for `www.kkai.plus` were run. Fresh `npx.cmd vercel inspect https://kkai.plus --scope yykks-projects-727e9560` now resolves `kkai.plus` to `dpl_8GrbT468nTemUAaqML1yn4pMLaEQ`.
- Production asset evidence: before the alias correction, `https://kkai.plus/` still served old `assets/index-Ci0q10SP.css` with `.prompt-bar-footer-frost:before{...inset:-6px 0 0}`. After the alias correction, `https://kkai.plus/` serves `assets/index-C4Idgjym.css` and `assets/index-BelvYSrj.js`; the live CSS rule is `.prompt-bar-footer-frost:before{...inset:0}`, `hasInset0=true`, and `hasBadInset=false`.
- Production browser QA evidence: a fresh Playwright browser context opened the deployed `https://kkai.plus/` with a synthetic temporary user session so the test went into the workspace instead of stopping at login. Desktop `1440x900` and mobile `390x844` both reported `isLogin=false`, `hasWorkspaceTextarea=true`, workspace mode buttons present, loaded `https://kkai.plus/assets/index-C4Idgjym.css`, `.theme-transitioning=0`, and stale chunk text count `0`. Mobile `.prompt-bar-footer-frost::before` computed to `inset: 0px` / `top: 0px`. Screenshots and JSON: `output/playwright/promptbar-footer-frost-fix/prod-workspace-desktop-1440x900.png`, `prod-workspace-mobile-390x844.png`, and `prod-workspace-verification.json`. The only console error was a synthetic temp-user avatar request for `/preset-default-local` returning 404; it is not related to PromptBar layout or production CSS.
...
- Fresh production evidence before this fix: `npx.cmd vercel inspect https://kkai.plus --scope yykks-projects-727e9560` still resolves `kkai.plus` to deployment `dpl_J9bcG4DvMeuE2ZrfivQMLD8NF637` (`https://kk-studio-2dege9hxw-yykks-projects-727e9560.vercel.app`), and `https://kkai.plus/app-version.json` reports `commitSha: 9aa070f87ec4cb1e494404b2eb4f2e04d197772b`. That hosted bundle does not include the local canvas geometry sanitizer, so production can still restore polluted `kk_studio_canvas_state` / `kk_canvas_view`.
- Root cause update: the earlier rollback removed the bad UI source patch, but rollback alone did not clear client-side persisted canvas state. If `localStorage["kk_canvas_view"]` kept a very small scale or `localStorage["kk_studio_canvas_state"]` kept invalid prompt/workflow dimensions and coordinates, the restored workspace could still look like tiny cards with overflowing text or a mixed-up canvas.
- Implemented fix scope: `src/context/canvasGeometrySanitizer.ts` sanitizes persisted prompt node positions, prompt dynamic width/height, image positions, group bounds, drawing points/width, and workflow utility node positions/width/height before canvas render. `src/context/canvasPersistence.ts` runs this sanitizer during `restoreCanvasStateFromLocalStorage`. `src/components/canvas/InfiniteCanvas.tsx` now rejects restored `kk_canvas_view` values below scale `0.35`, removing the polluted view and falling back to a centered scale `1` view.
- Regression coverage: `tests/unit/canvas-persistence-geometry-sanitizer.test.ts` now calls the real `restoreCanvasStateFromLocalStorage` path with corrupted persisted geometry and verifies the restored canvas is sanitized before render; it also verifies malformed canvas collections are tolerated and the canvas view restore rejects thumbnail-scale persisted views.
- Fresh local validation evidence: the real restore-path regression passed 4/4, the focused canvas regression suite passed 17/17, `npm.cmd run typecheck` passed with semantic coverage for 132 test files, `npm.cmd run build` passed and produced `dist/assets/canvas-core-Dium_GUZ.js`, `npm.cmd run governance:agent-docs` passed, `npm.cmd run check:encoding` passed, and the path-limited alternate-git `diff --check` passed with Windows LF/CRLF normalization warnings only.
- Fresh local browser evidence before production deploy: built `dist` was served from a temporary local HTTP server at `http://127.0.0.1:4186/`; polluted `kk_canvas_view` and `kk_studio_canvas_state` were injected, and reload rewrote the view to `{"x":720,"y":450,"scale":1}`. The restored prompt card box rendered at `x=560`, `y=303`, `width=320`, `height=147`, with normal text bounds inside a `294x84` text area and `.theme-transitioning=0`. Screenshot and JSON artifacts: `output/playwright/canvas-sanitizer-preview/canvas-sanitizer-local-fresh.png` and `canvas-sanitizer-local-fresh.json`. The only console errors were the known local static-preview model-catalog CORS/fetch noise, not canvas render errors.
- Production deploy evidence: after commit `2c08df4d fix: sanitize persisted canvas geometry`, `npx.cmd vercel deploy --prod -y --scope yykks-projects-727e9560` completed successfully. The remote Vercel build ran `npm run build` as `kk-studio@1.4.6`, transformed 2141 modules, produced `dist/assets/canvas-core-BRGhbfQB.js`, and created deployment `dpl_B7d8WLM3DpRJwvVKyWpq9GjiNs9Q` at `https://kk-studio-4kgevbd1t-yykks-projects-727e9560.vercel.app`.
- Production alias correction: the deploy initially left `https://kkai.plus` on old rollback deployment `dpl_J9bcG4DvMeuE2ZrfivQMLD8NF637`, so `npx.cmd vercel alias set kk-studio-4kgevbd1t-yykks-projects-727e9560.vercel.app kkai.plus --scope yykks-projects-727e9560` and the same command for `www.kkai.plus` were run. Fresh `npx.cmd vercel inspect https://kkai.plus --scope yykks-projects-727e9560` now resolves the user-facing domain to `dpl_B7d8WLM3DpRJwvVKyWpq9GjiNs9Q`.
- Production asset/version evidence: `https://kkai.plus/app-version.json` now reports build time `2026-05-10T07:16:39.078Z` and deployment target `production`. The `commitSha` field is `bd08d60d04b0f3868b0ae48e2ebb3b9bd11cc098` because Vercel reads the normal `.git` metadata in this workspace, while the actual committed fix is in the alternate git metadata as `2c08df4d`; the served HTML and JS resources are the authoritative production evidence for this deploy. `https://kkai.plus/` now serves `assets/index-C44bzq4w.js`, `assets/canvas-core-BRGhbfQB.js`, and `assets/index-Ci0q10SP.css`; the old `assets/canvas-core-CUNRGw_l.js` is no longer referenced by the user-facing domain.
- Production browser QA evidence: a fresh isolated Chromium context opened `https://kkai.plus/?qa=prod-canvas-sanitizer-*`, injected bad `kk_canvas_view` plus bad `kk_studio_canvas_state`, and refreshed without touching the user's real browser profile. The restored view became `{"x":720,"y":450,"scale":1}`; the restored prompt node position became `{x:0,y:0}`; the prompt card rendered at `x=560`, `y=303`, `width=320`, `height=147`; the inner text area was `294x84`; `.theme-transitioning=0`; stale chunk text count was `0`; console error/warning count was `0`; and request failure count was `0`. Artifacts: `output/playwright/canvas-sanitizer-production/prod-canvas-sanitizer-bad-state.png` and `prod-canvas-sanitizer-bad-state.json`.
...
- Version check: root `package.json` and `config/release-manifest.json` are aligned to `1.4.7`; `src/config/appInfo.ts` correctly reads the runtime version from `config/release-manifest.json` rather than hard-coding it.
...
- Follow-up production redeploy evidence: after commit `5b922928`, `vercel deploy . --prod -y --scope yykks-projects-727e9560` completed from the repository root. The remote build ran `npm run build` as `kk-studio@1.4.6`, transformed 2140 modules, completed in Vercel, and aliased `https://kkai.plus`. `vercel inspect https://kkai.plus --scope yykks-projects-727e9560` reports deployment `dpl_Ej3wrGiASL4FtBMZcARY81XszG34`, target `production`, status `Ready`, URL `https://kk-studio-e165sudnw-yykks-projects-727e9560.vercel.app`, created `2026-05-10 02:36:20 GMT+0800`, and aliases `https://kkai.plus` plus `https://www.kkai.plus`.
...
- Fresh production browser QA evidence: headless Chromium opened `https://kkai.plus/?qa=hosted-login-smoke` at desktop `1440x900` and mobile `390x844`. Both rendered the Chinese login page with `v1.4.6`, `lang="zh-CN"`, email/password inputs, Cloudflare Turnstile area, `staleChunkTextCount=0`, `.theme-transitioning=0`, `pageErrors=0`, and `requestFailures=0`. Screenshots and JSON are under `output/playwright/hosted-login-smoke/`. Console review found the only `kkai.plus` error was the expected signed-out `/api/v1/auth/session` `401`; the remaining console errors/warnings came from Cloudflare Turnstile Trusted Types/CSP behavior and headless WebGL/media warnings.
- Input visual QA evidence: desktop and mobile screenshots show no colored strip in the email/password fields; computed input backgrounds are transparent over the styled input shell, text and border color resolve to `rgb(16, 36, 71)`, and `boxShadow` is `none`.
...
- Remote branch reconciliation: local `main` was ahead 246 and behind 4 relative to `origin/main`. The remote-only commits were inspected and would have reintroduced the old Vercel proxy helper shape plus committed `output/` QA artifacts if merged normally. Commit `6fcdb366 chore: reconcile origin main release history` uses a deliberate `ours` merge to mark `origin/main` as integrated while preserving the current verified 1.4.6 release tree. After that merge, `origin/main` is an ancestor of local `HEAD` and the branch is no longer behind.
- Push blocker: `git push origin main` failed locally with `schannel: AcquireCredentialsHandle failed: SEC_E_NO_CREDENTIALS`; GitHub CLI is not installed and Git Credential Manager has no usable GitHub credential in this environment. The local branch is fast-forward-ready for push once GitHub credentials are available.
...
- Latest Turnstile follow-up: production login could show a Cloudflare iframe while the surrounding login panel still said the security check was `Loading`, making the login flow look stuck. `src/components/auth/TurnstileWidget.tsx` now publishes render status to `LoginScreen`, and `src/components/auth/LoginScreen.tsx` shows rendered-but-unverified Turnstile as `Verify` with `请完成 Cloudflare 安全验证后再登录。`, while widget errors now show the error badge instead of the loading badge.
- Latest hotfix candidate boundary: stage only `src/components/auth/LoginScreen.tsx`, `src/components/auth/TurnstileWidget.tsx`, `tests/unit/login-screen-auth-actions.test.ts`, `status.md`, and `validation.md`. Exclude generated `output/` and the unrelated line-ending-only `tests/unit/kk-api-client.test.ts` status noise.
- Latest validation for this Turnstile follow-up: RED first reproduced the missing parent error-state branch in `tests/unit/login-screen-auth-actions.test.ts`, then the focused login page test passed 4/4 after the fix. The broader login/Turnstile/hosted guardrail suite freshly passed 17/17, `npm.cmd run governance:agent-docs` passed, `npm.cmd run typecheck` passed, `npm.cmd run check:encoding` passed, `npm.cmd run build` passed as `kk-studio@1.4.6`, `npm.cmd pkg get version` returned `1.4.6`, and path-limited alternate-git `diff --check` passed with Windows LF/CRLF normalization warnings only.
- Latest production deployment evidence: after commit `78aa280f fix: clarify turnstile login status`, `npx.cmd vercel deploy --prod -y --scope yykks-projects-727e9560` completed successfully. The remote build ran `npm run build` as `kk-studio@1.4.6`, transformed 2140 modules, and aliased `https://kkai.plus`. `npx.cmd vercel inspect https://kkai.plus --scope yykks-projects-727e9560` reports deployment `dpl_4SXnYSMc3yeFCuLsgLEbJSR8ZnFV`, target `production`, status `Ready`, URL `https://kk-studio-o6m9dv4bi-yykks-projects-727e9560.vercel.app`, with the expected `kkai.plus` aliases and Vercel API functions.
- Latest production browser QA: the in-app browser opened `https://kkai.plus/` successfully and rendered the Chinese login page with `v1.4.6`. Turnstile first showed `待验证 / 请完成 Cloudflare 安全验证后再登录。`, then completed to `已就绪 / 安全验证已完成。`; console warning/error count was 0, and there were no Tailwind CDN or `Language zh-CN` warnings. Independent headless Playwright from this machine still hits Vercel Security Check `429`, so treat that as the known local automation limitation rather than a site failure.
...
- Production deployment evidence: after commit `17973288 fix: harden hosted login errors and input autofill`, `npx.cmd vercel deploy --prod -y --scope yykks-projects-727e9560` completed successfully, ran the remote Vercel build as `kk-studio@1.4.6`, transformed 2140 modules, and aliased `https://kkai.plus`. Fresh `npx.cmd vercel inspect https://kkai.plus --scope yykks-projects-727e9560` reports deployment `dpl_4U49MUEyEPtdjTBziC2rejxGeMMP`, target `production`, status `Ready`, URL `https://kk-studio-rja71b3e3-yykks-projects-727e9560.vercel.app`, created `2026-05-09 13:16:38 GMT+0800`, with aliases `https://kkai.plus`, `https://www.kkai.plus`, `https://kk-studio.vercel.app`, `https://kk-studio-yykks-projects-727e9560.vercel.app`, and `https://kk-studio-yinchenkang0-1635-yykks-projects-727e9560.vercel.app`. The inspected build includes Vercel runtime functions `api/healthz`, `api/auth`, `api/manifest`, `api/ecommerce-analysis`, and `api/auth/[...path]`, with additional output items hidden by Vercel inspect.

## Current 1.4.7 Release Blocker Audit

- Active user issue: align the release candidate to v1.4.7 after adding desktop snap-to-grid and collapsed manual canvas groups, while preserving hosted/VPS security, dependency audit, visible Chinese text, PromptBar UI details, local storage safety, and real browser QA gates.
- Current 1.4.7 version-alignment scope: root package metadata, lockfiles, payment-server metadata, workspace package metadata, release manifest, stable portable manifest, README/agent/development ledgers, and portable release URL scanners are being aligned to `1.4.7`. The two feature commits already in the alternate-git history are `7d4b83bb feat: add canvas snap to grid toggle` and `f5d15c44 feat: collapse manual canvas groups`.
...
- Portable publish evidence: `VITE_KK_API_BASE_URL=https://172-245-156-16.sslip.io VITE_TURNSTILE_LOCAL_BYPASS=false npm.cmd run package:portable` completed and regenerated `release/KK-Studio-Portable`; `npm.cmd run publish:portable` updated `release/publish/stable/manifest.json` for `KK-Studio-Portable-1.4.6.zip` with SHA-256 `2ba18a49403584b9d934ae18b6059d77c655c772eca6701739c8a8e9153b07c3` and size `52901506`. The portable build no longer depends on the unresolved `api.kkai.plus` DNS path.
...
- Fresh production inspect after `b6f1fd6f`: `npx.cmd vercel inspect https://kkai.plus --scope yykks-projects-727e9560` reports deployment `dpl_ACm7915SrSFiVQGNVJUDoDXMMQFA`, target `production`, status `Ready`, URL `https://kk-studio-u1atrmy8j-yykks-projects-727e9560.vercel.app`, created `2026-05-09 11:11:13 GMT+0800`, and aliases `https://kkai.plus`, `https://www.kkai.plus`, `https://kk-studio.vercel.app`, `https://kk-studio-yykks-projects-727e9560.vercel.app`, and `https://kk-studio-yinchenkang0-1635-yykks-projects-727e9560.vercel.app`. The inspected build includes the Vercel runtime functions `api/auth/[...path]`, `api/v1/[...path]`, `api/healthz`, `api/ecommerce-analysis`, and `api/v1`.
- Fresh production deployment after `0ec11cb9`: `npx.cmd vercel deploy --prod -y --scope yykks-projects-727e9560` completed successfully from the repository root, uploaded only `363B` of changed data, restored build cache from `ACm7915SrSFiVQGNVJUDoDXMMQFA`, ran `npm run build` as `kk-studio@1.4.6`, transformed 2140 modules, and aliased `https://kkai.plus`. `npx.cmd vercel inspect https://kkai.plus --scope yykks-projects-727e9560` reports deployment `dpl_BvSuUPCu4EWeBkoFcktZvDS5uJpV`, target `production`, status `Ready`, URL `https://kk-studio-a16e63rja-yykks-projects-727e9560.vercel.app`, created `2026-05-09 11:23:02 GMT+0800`, and aliases `https://kkai.plus`, `https://www.kkai.plus`, `https://kk-studio.vercel.app`, `https://kk-studio-yykks-projects-727e9560.vercel.app`, and `https://kk-studio-yinchenkang0-1635-yykks-projects-727e9560.vercel.app`. The inspected build includes the Vercel runtime functions `api/auth/[...path]`, `api/healthz`, `api/auth`, `api/ecommerce-analysis`, and `api/v1`, with additional output items hidden by Vercel inspect.
- Fresh HTTPS upstream workaround after the `api.kkai.plus` DNS blocker: the VPS can resolve `172-245-156-16.sslip.io` and `172-245-156-16.nip.io` to `172.245.156.16`, so `scripts/vps/configure-kk-vps-api-tls.sh` was copied to `/tmp` and run with `API_DOMAIN=172-245-156-16.sslip.io EXPECTED_API_IPV4=172.245.156.16`. Certbot installed successfully, issued a Let's Encrypt certificate for `172-245-156-16.sslip.io` expiring `2026-08-07`, wrote the nginx HTTPS gateway, and reloaded nginx after `nginx -t` passed. VPS-side HTTPS smoke returns `200` for `/healthz`, `200` for `/api/manifest`, expected unauthenticated `401` for `/api/v1/auth/session`, and `404` for both `/internal` and `/internal/`. Vercel Production now has `KK_VPS_API_BASE_URL` set to `https://172-245-156-16.sslip.io`, and the follow-up deploy reports deployment `dpl_JBqdQMBorigt5kTExqRrv3JosAHc`, target `production`, status `Ready`, URL `https://kk-studio-icg1ticp2-yykks-projects-727e9560.vercel.app`, created `2026-05-09 11:35:46 GMT+0800`, and aliases `https://kkai.plus` plus the expected project aliases. Direct local requests to `https://kkai.plus/api/*` are still stopped by Vercel Security Check `429`, and direct deployment URL requests are stopped by Deployment Protection `401`, so same-origin API smoke from this machine cannot prove function entry; the available evidence is Vercel Ready + Production env + VPS HTTPS smoke.
- Latest 1.4.6 production deployment evidence: after `aa14ee33`, `npx.cmd vercel deploy --prod -y --scope yykks-projects-727e9560` completed from the repository root and the remote build ran `npm run build` as `kk-studio@1.4.6`, transformed 2140 modules, and completed in Vercel. Fresh `npx.cmd vercel inspect https://kkai.plus --scope yykks-projects-727e9560` reports deployment `dpl_632NEDeDWYXHJtyjnxfgDgQfuHXo`, target `production`, status `Ready`, URL `https://kk-studio-gq1riacd7-yykks-projects-727e9560.vercel.app`, created `2026-05-09 11:49:31 GMT+0800`, and aliases `https://kkai.plus`, `https://www.kkai.plus`, `https://kk-studio.vercel.app`, `https://kk-studio-yykks-projects-727e9560.vercel.app`, and `https://kk-studio-yinchenkang0-1635-yykks-projects-727e9560.vercel.app`. `npx.cmd vercel env ls production --scope yykks-projects-727e9560` confirms Production still contains `KK_VPS_API_BASE_URL` and `VITE_KK_API_BASE_URL`; the browser continues to use same-origin `https://kkai.plus` while Vercel API functions proxy to the HTTPS VPS upstream workaround.
- Current DNS workaround resolution: a TDD regression first failed because the Vercel proxy default upstream still resolved to `api.kkai.plus`. `api/_vpsProxy.ts` now defaults to `https://172-245-156-16.sslip.io`, so Vercel functions do not fall back to the unresolved canonical API hostname if `KK_VPS_API_BASE_URL` is absent or not injected. Node HTTPS/CORS smoke confirms `https://172-245-156-16.sslip.io/api/v1/auth/session` returns the expected unauthenticated `401` JSON with `Access-Control-Allow-Origin: http://127.0.0.1:3000`, while same-machine direct `https://kkai.plus/api/*` requests can still hit Vercel Security Check `429`.
- Latest DNS-workaround production deploy evidence: after `17fae182`, `npx.cmd vercel deploy --prod -y --scope yykks-projects-727e9560` completed from the repository root. The remote build ran `npm run build` as `kk-studio@1.4.6`, transformed 2140 modules, and completed in Vercel. Fresh `npx.cmd vercel inspect https://kkai.plus --scope yykks-projects-727e9560` reports deployment `dpl_DjvHLa9pM5jZmPDYjJBBNVVF4NK4`, target `production`, status `Ready`, URL `https://kk-studio-bp55q7fxi-yykks-projects-727e9560.vercel.app`, created `2026-05-09 12:10:27 GMT+0800`, and aliases `https://kkai.plus`, `https://www.kkai.plus`, `https://kk-studio.vercel.app`, `https://kk-studio-yykks-projects-727e9560.vercel.app`, and `https://kk-studio-yinchenkang0-1635-yykks-projects-727e9560.vercel.app`. Vercel Production env still contains `KK_VPS_API_BASE_URL` and `VITE_KK_API_BASE_URL`; VPS HTTPS smoke returns `200` for `/healthz` and `/api/manifest`, expected unauthenticated `401` for `/api/v1/auth/session`, `204` for the CORS preflight from `http://127.0.0.1:3000`, and `404` for `/internal` plus `/internal/`.
...
- Fresh DNS/VPS permanent-domain evidence: the local resolver path and direct `Resolve-DnsName` checks against `1.1.1.1`, `8.8.8.8`, `autumn.ns.cloudflare.com`, and `langston.ns.cloudflare.com` still return `A api.kkai.plus -> 198.18.0.73`, not `172.245.156.16`. Local environment variables `CF_API_TOKEN`, `CLOUDFLARE_API_TOKEN`, and `CLOUDFLARE_ZONE_ID` are unset, `wrangler whoami` is unauthenticated, and `node scripts/deploy/cloudflare-upsert-api-dns.mjs` fails closed with the expected missing-token message. This is now a permanent canonical-domain hardening task, not the active hosted availability path: production hosted uses same-origin `https://kkai.plus` and Vercel functions/portable builds use the verified HTTPS VPS upstream `https://172-245-156-16.sslip.io`.
- Fresh deployment-boundary follow-up: a read-only audit found `scripts/deploy/cloudflare-upsert-api-dns.mjs` could duplicate `/client/v4` in Cloudflare API URLs and that the fallback `scripts/deploy/vercel-preview-deploy.ps1` tarball did not mirror `.vercelignore`, allowing local-only directories such as `m/`, `output/`, tests, docs, release artifacts, deploy templates, and AI ledgers into fallback uploads. Added RED/GREEN coverage in `tests/unit/hosted-release-guardrails.test.ts`, fixed the Cloudflare helper base URL, excluded `m` in `.vercelignore`, and expanded the preview tar exclusions to match the hosted production upload boundary. Focused guardrail tests now pass 9/9.
- Fresh validation after the deployment-boundary follow-up: `npm.cmd run governance:check`, `npm.cmd run audit:dependencies`, `npm.cmd run spec:check`, `npm.cmd run typecheck`, `npm.cmd run check:encoding`, the focused Hosted/VPS guardrail suite, path-limited alternate-git `diff --check`, `npm.cmd run test:unit` (1444/1444), and `npm.cmd run build` all passed.
...
- Remaining canonical-domain blocker: public DNS for `api.kkai.plus` must still be changed to `A api -> 172.245.156.16` at the authoritative DNS provider, then `scripts/vps/configure-kk-vps-api-tls.sh` should be rerun on the VPS with `API_DOMAIN=api.kkai.plus` so the permanent API domain serves its own certificate. Until then, the production hosted app uses same-origin `https://kkai.plus` for browser API calls and the Vercel proxy uses the HTTPS wildcard upstream `https://172-245-156-16.sslip.io`.
...
- Current startup simple progress UI QA: the built `dist` bundle was served through a local fixture at `http://127.0.0.1:4178/__startup-simple-fixture` and checked in dark theme. Desktop 1280x720 rendered a minimal centered loading card at 420px wide with one prompt and a 0-100 stage-driven progress fill; mobile 390x844 rendered the same surface at 354px wide. Both viewports had no horizontal or vertical overflow and measured text fit. Playwright screenshots were saved at `output/playwright/startup-simple-progress-desktop.png` and `output/playwright/startup-simple-progress-mobile.png`. Fresh unit coverage passed with `npm.cmd run test:unit` at 1466/1466. Vercel preview deploy passed at `https://kk-studio-6n5q1hmzi-yykks-projects-727e9560.vercel.app`; production `kkai.plus` was not changed in this slice.
- Current production rollback for the canvas card deployment regression: production was briefly moved to deployment `dpl_3WRjJS4YvCfLCCcwpcpTa5ArWnnH` (`commitSha: 3267afb381377580170e96b9952467952bea63d0`), which included the canvas card text-wrap/min-size changes but produced a visibly disordered hosted workspace UI. Production was rolled back with `npx.cmd vercel rollback https://kk-studio-2dege9hxw-yykks-projects-727e9560.vercel.app -y --scope yykks-projects-727e9560 --timeout 5m`; `npx.cmd vercel inspect https://kkai.plus --scope yykks-projects-727e9560` now resolves `kkai.plus` to `dpl_J9bcG4DvMeuE2ZrfivQMLD8NF637`, and `https://kkai.plus/app-version.json` reports `commitSha: 9aa070f87ec4cb1e494404b2eb4f2e04d197772b`. The local source has also withdrawn the canvas card text-wrap/min-size patch so the next local deploy cannot reintroduce that UI regression.
