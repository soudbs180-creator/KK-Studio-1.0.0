...
- Alternate-git worktree was clean at `0cc6c77c` before this user API payload unused secret-constant cleanup pass.
...
- Current focus: finish user API payload TS6133 cleanup in `src/services/api/userApiPayload.ts` by removing duplicate unused placeholder/redaction constants while preserving the live secret placeholder policy in `src/services/api/userApiCloudRecordStorage.ts`.
...
- Current commit scope: `src/services/api/userApiPayload.ts`, `tests/unit/billing-remaining-balance-contract.test.ts`, and the four ledger files. Provider routing, API/settings UI files, endpoint/auth changes, release metadata, payment business logic, storage persistence behavior, generation/PPT runtime behavior, and unrelated architecture helpers remain excluded.
- Browser QA: skipped for this M90 slice because it edits non-UI API payload constants, one source-contract test, and ledger files; it does not change JSX/CSS or browser-visible behavior.
...
## Working Tree M90 (User API Payload Secret-Constant Cleanup)
...
- The alternate-git worktree was clean at `0cc6c77c` before this user API payload unused secret-constant cleanup pass, but any staging must still be explicit path-based and reviewed before commit.
