...
- Current commit scope: none; choose a fresh next slice before editing production/runtime files.
...
- Unit gate result: `npm.cmd run test:unit` passed 1373/1373.
...
- Current fact source: the latest stable code baseline is `5dda8972 style: close settings workbench shell flattening`; exact HEAD may be a docs-only ledger sync above it and must be read from alternate git.
...
1. The current noUnused filter is clean; the route-gate helpers are wired into live entrypoints; M107-M123 moved the latest OpenAI-compatible and server helper seams into focused modules; M118 and M122 closed the latest confirmed security blockers.
...
3. Next ordinary cleanup candidate should be chosen from a fresh seam map. The safest next architecture slice remains in `OpenAICompatibleAdapter.ts` request builders/provider quirks or `PromptBar.tsx` attachment/drag/reference-image ingestion; the PromptBar path requires browser QA because it touches file-input UI behavior.
