...
- Alternate-git worktree was clean at `b31edef5` before this small LLM-adapter unused cleanup pass.
...
- Current focus: finish small LLM-adapter TS6133 cleanup by removing unused audio capability reads/imports and marking the Volcengine support parameter unused while preserving request behavior.
...
- Current commit scope: `src/services/llm/AudioCompatibleAdapter.ts`, `src/services/llm/VolcengineAdapter.ts`, `tests/unit/llm-adapter-unused-cleanup-contract.test.ts`, `tsconfig.tests.json`, and the four ledger files. Audio request body construction, duration limiting, lyrics/style/continuation/TTS fields, polling, Volcengine endpoint selection, auth headers, provider routing, model routing, API/settings UI files, endpoint/auth changes, release metadata, payment business logic, and unrelated architecture helpers remain excluded.
- Browser QA: skipped for this M85 slice because it only removes/marks unused adapter internals and updates tests/docs; it does not change JSX/CSS or browser-visible behavior.
...
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 423, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The filtered noUnused probe now shows 52 TS6133 diagnostics plus 3 TS619x diagnostics. These are refactor debt indicators, not release blockers by themselves.
