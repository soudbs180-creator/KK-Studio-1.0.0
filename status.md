...
- Alternate-git worktree was clean at `fbe26764` before the scoped Canvas image delete helper extraction.
...
- Current focus: finish and commit Stage Two M19 by moving the pure image-node deletion transform into `src/context/canvasPromptImageLinks.ts` while leaving storage deletion, Blob URL revocation, urgent-save, history, and `updateCanvas` ownership in `CanvasContext.tsx`.
- Most recent code/security scopes: Stage Two M19 image delete helper in the active working tree; Stage Two M18 workflow update helpers in `fbe26764`; Stage Two M17 prompt-image link helpers in `c46a4c49`; Stage Two M16 position update helpers in `c80ffa70`; Stage Two M15 node update helpers in `c9d39bb2`; Stage Two M14 tag helpers in `d7a9d0a7`; Stage Two M13 movement helpers in `4722acbe`; Stage Two M12 group helpers in `5994c34b`; Netlify raw-key endpoint cleanup in `0603547a`; Stage Two M11 layering helpers in `c171de38`; Stage Two M10 placement helpers in `3abdd250`; Stage Two M9 cleanup helpers in `7d8a4331`; Stage Two M8 canvas merge helpers in `b68867dd`; Stage Two M7 persisted image recovery helpers in `0a5c2339`; Stage Two M6 prompt recovery helpers in `53f80d80`; Stage Two M5 media recovery helpers in `002ee6fe`; protobuf override in `4cdbf4cf`; portable metadata refresh in `567f85aa`; Nutrient OCR server-key hardening in `0c5cadde`; PostCSS patch in `333f2551`; dead AI12 service pruning in `b6620ef2`.
- Current commit scope: `src/context/CanvasContext.tsx`, `src/context/canvasPromptImageLinks.ts`, `tests/unit/canvas-prompt-image-links-contract.test.ts`, `plans.md`, `implement.md`, `validation.md`, and `status.md`. UI, release metadata, Netlify endpoint cleanup, workflow helper code, `keyManager.ts`, `PromptBar.tsx`, and `OpenAICompatibleAdapter.ts` remain excluded from this commit.
- Browser QA: skipped for Stage Two M19 because this is a non-UI pure context/helper extraction. No visual surface, CSS, route component, or browser-visible workflow changed.
...
- Current giant tracked files in the active Stage Two M19 working tree: `src/services/auth/keyManager.ts` 5279 lines, `src/App.tsx` 4900 lines, `src/services/llm/OpenAICompatibleAdapter.ts` 4517 lines, `src/components/layout/PromptBar.tsx` 4437 lines, `src/context/CanvasContext.tsx` 3724 physical lines.
...
1. Commit Stage Two M19 Canvas image delete helper extraction as a separate architecture commit.
2. Identify the next narrow `CanvasContext.tsx` responsibility after image deletion, based on a source map and targeted contract first. Defer `migrateNodes` until async IndexedDB caching can be split safely.
