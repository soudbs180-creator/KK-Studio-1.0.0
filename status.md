...
- Alternate-git worktree was clean at `6c624909` before this user API cloud helper cleanup pass.
...
- Current focus: M95 user API cloud helper cleanup is validated and ready to commit; the next cleanup candidate must come from a fresh TS6133 cluster scan after this commit.
...
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 423, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The M95 production noUnused filter still exits 2 on 43 broader historical diagnostics, with zero `src/services/api/userApiCloudRecordStorage.ts` and zero `getErrorMessage` diagnostics. These are refactor debt indicators, not release blockers by themselves.
