...
- Alternate-git worktree was clean at `16d805cb` before the scoped Canvas merge-into helper extraction.
...
- Current giant tracked files in the active Stage Two M20 working tree: `src/services/auth/keyManager.ts` 5279 lines, `src/App.tsx` 4900 lines, `src/services/llm/OpenAICompatibleAdapter.ts` 4517 lines, `src/components/layout/PromptBar.tsx` 4437 lines, `src/context/CanvasContext.tsx` 3641 physical lines.
...
1. Commit Stage Two M20 Canvas merge-into helper extraction as a separate architecture commit.
2. Identify the next narrow `CanvasContext.tsx` responsibility after merge-into, based on a source map and targeted contract first. Defer `migrateNodes` until async IndexedDB caching can be split safely.
