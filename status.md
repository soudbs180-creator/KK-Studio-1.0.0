...
- Alternate-git worktree was clean at `b16843ee` before this M28 slice.
...
- Follow-up debt: `keyManagerEffectiveSlot.ts` still has a `determineKeyType` back-edge to `keyManager.ts`; `src/utils/modelIdNormalization.ts` still duplicates model migration/normalization helpers and should be consolidated or parity-guarded in a later narrow slice.
...
1. Commit Stage Two M28 keyManager model helper extraction after the full slice gate and path-limited diff check pass.
2. Continue keyManager with one narrow seam: either remove the remaining `determineKeyType` cycle through `keyManagerEffectiveSlot.ts` or consolidate/parity-guard duplicated `src/utils/modelIdNormalization.ts` behavior.
...
4. Continue Stage Two giant-file split in this priority order after keyManager model helpers: remaining `keyManager.ts`, `PromptBar.tsx`, `OpenAICompatibleAdapter.ts`.
...
- The alternate-git worktree was clean at `b16843ee` before this M28 slice, but any staging must still be explicit path-based and reviewed before commit.
