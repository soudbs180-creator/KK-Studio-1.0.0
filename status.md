...
- Alternate-git worktree was clean at `318e6b1e` before this user API profile import-only cleanup pass.
...
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 423, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The M93 production noUnused filter still exits 2 on 45 broader historical diagnostics, with zero `src/services/api/userApiProfileStorage.ts` matches. These are refactor debt indicators, not release blockers by themselves.
