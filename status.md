...
- Alternate-git worktree was clean at `37540efc` before this video-service unused cleanup pass.
...
- Current focus: finish video-service TS6133/TS619x cleanup by removing or marking unused imports/parameters while preserving browser fail-closed video behavior and Veo request/poll/download behavior.
...
- Current commit scope: `src/services/video/OpenAIVideoService.ts`, `src/services/video/VeoVideoService.ts`, `src/services/video/videoService.ts`, `tests/unit/video-service-unused-cleanup-contract.test.ts`, `tsconfig.tests.json`, and the four ledger files. Video request construction, API base normalization, polling, download/auth headers, progress label content, provider routing, model routing, API/settings UI files, endpoint/auth changes, release metadata, payment business logic, PPT behavior, storage behavior, and unrelated architecture helpers remain excluded.
- Browser QA: skipped for this M83 slice because it only removes/marks unused service code and updates tests/docs; it does not change JSX/CSS or browser-visible behavior.
...
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 423, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The filtered noUnused probe now shows 59 TS6133 diagnostics plus 3 TS619x diagnostics. These are refactor debt indicators, not release blockers by themselves.
...
## Active M83 Slice (Video Service Unused Cleanup)
...
1. Close the current video-service unused cleanup slice first; do not claim final completion while additional Stage Two seams and remaining TS6133/TS619x hotspots remain open.
...
1. Remaining noUnused cleanup hotspots are now outside App, ChatSidebar, PromptNode, SystemLogs, Onboarding, deleted imageCompression, deleted PixiCanvas, dormant/live canvas residual files, workflow-actions import cleanup, common ErrorBoundary cleanup, generation runtime import cleanup, CanvasContext type-import cleanup, ProjectManager prop-destructure cleanup, ecommerce taskMerger parameter cleanup, model display-name provider-parameter cleanup, and video-service cleanup. The largest clusters are `useImageGeneration.ts`, `geminiService.ts`, `ApiSettingsView.tsx`, `keyManager.ts`, and `GoogleAdapter.ts`. Handle each as a separate minimal slice with a source contract.
2. Next cleanup candidate should be chosen from a fresh TS6133 cluster scan after M83. Current lower-risk candidates include isolated storage/image helper locals or small audio/Volcengine adapter parameter cleanup. Keep the next slice separate from storage persistence, API/key settings, Gemini/provider routing, billing/payment, and keyManager work unless explicitly scoped.
