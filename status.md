...
- Alternate-git worktree was clean at `c46a4c49` before the scoped Canvas workflow update helper extraction.
...
- Current giant tracked files in the active Stage Two M18 working tree: `src/services/auth/keyManager.ts` 5279 lines, `src/App.tsx` 4900 lines, `src/services/llm/OpenAICompatibleAdapter.ts` 4517 lines, `src/components/layout/PromptBar.tsx` 4437 lines, `src/context/CanvasContext.tsx` 3733 physical lines.
...
1. Identify the next narrow `CanvasContext.tsx` responsibility after workflow updates, based on a source map and targeted contract first. Defer `migrateNodes` until async IndexedDB caching can be split safely.
2. Continue Stage Two giant-file split in this priority order after remaining CanvasContext slices: `keyManager.ts`, `PromptBar.tsx`, `OpenAICompatibleAdapter.ts`.
3. If release metadata changes again, rerun packaging/publish and the full release gate including `npm.cmd run governance:check`.
