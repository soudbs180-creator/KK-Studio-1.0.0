...
- VPS admin static deploy status: local `apps/admin/dist` is built and packaged at ignored `.tmp/kk-admin-dist-89e5dd81.tar`, but upload to `172.245.156.16` is blocked because both `.codex-tmp-vps-key2` and `.codex-tmp-vps-key` have local ACL `Access is denied`; `ssh-add -l` reports no agent, and SSH without a key returns `Permission denied (publickey,password)`. `takeown.exe /f .codex-tmp-vps-key2` also returns `Access is denied`. Do not treat this as an API connection failure; it is a local deploy credential access blocker.
- Next step: with a readable VPS SSH key or server-side deploy access, copy the built admin app to `/var/www/kk-admin` and reload Nginx; then use the admin provider UI to save the real provider key/model row and re-smoke `/api/v1/model-catalog/active-credit-models` until it returns at least one active model.
