...
- Current milestone: Milestone 3 prompt group layout extraction in progress; tenth drag-commit persistence slice is complete in the current scoped change
...
Line count change after tenth slice:
...
Current risk:
- This slice intentionally does not move prompt-group drag handler wiring, render wiring, selection behavior, or the broader canvas reset/focus lifecycle yet, so `App.tsx` still owns those orchestration controls.
...
- Auto-repair, live-position change handling, regroup predicate checks, and drag commit persistence now live in `usePromptGroupLayout.ts`; future slices should keep prompt-group changes narrow because remaining wiring still crosses render and selection behavior.
...
- Candidate next slice: audit whether prompt-group node selection/height handlers or drag handler wiring can be split safely; keep render wiring isolated unless a dedicated contract proves the boundary.
