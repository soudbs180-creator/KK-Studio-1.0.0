...
- Alternate-git worktree was clean at `bdb082d7` before this workflow-actions import cleanup pass.
...
- Current focus: finish workflow-actions TS6133 cleanup by removing the unused `WORKFLOW_TEMPLATES` import while preserving `WorkflowTemplateId`, workflow node factories, and `App.tsx` template-list ownership.
...
- Current commit scope: `src/app/useWorkflowActions.ts`, `tests/unit/workflow-actions-unused-cleanup-contract.test.ts`, `tsconfig.tests.json`, and the four ledger files. Provider routing, endpoint/auth changes, release metadata, payment business logic, PPT/runtime behavior, broad UI redesign, storage persistence behavior, keyManager, workflow template behavior, and unrelated architecture helpers remain excluded.
- Browser QA: skipped for this M76 slice because it only removes an unused import from a runtime hook and does not change JSX, CSS, workflow template definitions, or visible UI behavior.
...
## Completed In Current Slice (Workflow Actions Import Cleanup)
...
1. Close the current workflow-actions import cleanup slice first; do not claim final completion while additional Stage Two seams and remaining TS6133/TS619x hotspots remain open.
...
1. Remaining noUnused cleanup hotspots are now outside App, ChatSidebar, PromptNode, SystemLogs, Onboarding, deleted imageCompression, deleted PixiCanvas, dormant/live canvas residual files, and the current workflow-actions import cleanup. The largest clusters are `useImageGeneration.ts`, `geminiService.ts`, `ApiSettingsView.tsx`, `keyManager.ts`, `ProjectManager.tsx`, and `GoogleAdapter.ts`. Handle each as a separate minimal slice with a source contract.
2. Next cleanup candidate should be chosen from a fresh TS6133 cluster scan after M76. Keep it separate from storage persistence, API/key settings, Gemini/provider routing, billing/payment, and keyManager work unless explicitly scoped.
...
- The alternate-git worktree was clean at `bdb082d7` before this workflow-actions import cleanup pass, but any staging must still be explicit path-based and reviewed before commit.
