...
- Alternate-git worktree was clean at `4722acbe`; current uncommitted work is the scoped Canvas node tag helper extraction.
...
- Current focus: finish and commit Stage Two M14 by moving node tag transforms into `src/context/canvasTags.ts` while leaving public `setNodeTags`, `updateCanvas`, and compatibility/`lastModified` ownership in `CanvasContext.tsx`.
- Most recent code/security scopes: Stage Two M14 tag helpers in the active working tree; Stage Two M13 movement helpers in `4722acbe`; Stage Two M12 group helpers in `5994c34b`; Netlify raw-key endpoint cleanup in `0603547a`; Stage Two M11 layering helpers in `c171de38`; Stage Two M10 placement helpers in `3abdd250`; Stage Two M9 cleanup helpers in `7d8a4331`; Stage Two M8 canvas merge helpers in `b68867dd`; Stage Two M7 persisted image recovery helpers in `0a5c2339`; Stage Two M6 prompt recovery helpers in `53f80d80`; Stage Two M5 media recovery helpers in `002ee6fe`; protobuf override in `4cdbf4cf`; portable metadata refresh in `567f85aa`; Nutrient OCR server-key hardening in `0c5cadde`; PostCSS patch in `333f2551`; dead AI12 service pruning in `b6620ef2`.
- Current commit scope: `src/context/CanvasContext.tsx`, `src/context/canvasTags.ts`, `tests/unit/canvas-tags-contract.test.ts`, `tsconfig.tests.json`, `plans.md`, `implement.md`, `validation.md`, and `status.md`. UI, release metadata, Netlify endpoint cleanup, movement/group helper code, `keyManager.ts`, `PromptBar.tsx`, and `OpenAICompatibleAdapter.ts` remain excluded from this commit.
- Browser QA: skipped for Stage Two M14 because this is a non-UI pure context/helper extraction. No visual surface, CSS, route component, or browser-visible workflow changed.
...
- Current giant tracked files in the active Stage Two M14 working tree: `src/services/auth/keyManager.ts` 5279 lines, `src/App.tsx` 4900 lines, `src/services/llm/OpenAICompatibleAdapter.ts` 4517 lines, `src/components/layout/PromptBar.tsx` 4437 lines, `src/context/CanvasContext.tsx` 4012 physical lines.
...
## Stage Two M14 Canvas Tags Extraction
...
1. Commit Stage Two M14 Canvas tags helper extraction as a separate architecture commit.
2. After M14 commit, start the next narrow `CanvasContext.tsx` responsibility from the reviewed M15 candidate: node update helpers around image dimensions, image updates, and prompt/image batch updates.
