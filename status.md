...
- Alternate-git worktree was clean at `eeb377d5` before this dormant canvas residual cleanup pass, except for the untracked generated `tsconfig.node.tsbuildinfo` file from typecheck.
...
- Current focus: finish dormant canvas residual TS6133 cleanup by removing only unused destructured values/state values from `Canvas.tsx` and `PendingNode.tsx`, while preserving their props interfaces and leaving live `InfiniteCanvas.tsx` untouched.
...
- Current commit scope: `src/components/canvas/Canvas.tsx`, `src/components/canvas/PendingNode.tsx`, `tests/unit/canvas-dormant-unused-cleanup-contract.test.ts`, and the four ledger files. Provider routing, endpoint/auth changes, release metadata, payment business logic, PPT/runtime behavior, broad UI redesign, storage/auth/proxy/billing files, live `InfiniteCanvas.tsx`, and unrelated architecture helpers remain excluded.
- Browser QA: skipped for this M71 slice because it only removes unread props destructures/state values from dormant support files; the Clay source contract covers `PendingNode` frosted-surface invariants and live `InfiniteCanvas.tsx` is untouched.
...
- Current giant tracked files after the dormant canvas residual cleanup working tree: `src/index.css` 13553 physical lines, `src/App.tsx` 4812, `src/services/auth/keyManager.ts` 4338, `src/services/llm/OpenAICompatibleAdapter.ts` 4337, `src/components/layout/PromptBar.tsx` 3965, `src/components/settings/ApiSettingsView.tsx` 3385, `src/components/layout/ChatSidebar.tsx` 2743, `src/app/useGenerationRuntime.ts` 2604, `src/context/CanvasContext.tsx` 2518, `src/components/canvas/PromptNodeComponent.tsx` 2241, `src/services/storage/fileSystemService.ts` 1577, `src/components/Onboarding/OnboardingManager.tsx` 262, `src/components/Onboarding/OnboardingOverlay.tsx` 200, `src/components/Onboarding/useOnboardingProgress.ts` 137, and `src/components/Onboarding/AchievementToast.tsx` 60. Current SystemLogs line counts: `SystemLogsView.tsx` 421 and `SystemLogsView.localized.tsx` 450.
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 421, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The filtered noUnused probe now shows 91 remaining diagnostics overall. These are refactor debt indicators, not release blockers by themselves.
...
## Completed In Current Slice (Dormant Canvas Residual Cleanup)
...
- Fresh TS6133/TS619x probe: `npx.cmd tsc --noEmit --noUnusedLocals true --noUnusedParameters true` has zero `src/components/canvas/Canvas` and `src/components/canvas/PendingNode` matches. The broader repository still has 91 TS6133/TS619x diagnostics led by non-canvas-residual hotspots.
...
1. Close the current dormant canvas residual cleanup slice first; do not claim final completion while additional Stage Two seams and remaining TS6133 hotspots remain open.
...
1. Remaining noUnused cleanup hotspots are now outside App, ChatSidebar, PromptNode, SystemLogs, Onboarding, deleted imageCompression, deleted PixiCanvas, and the current dormant canvas residual files; the largest clusters are `fileSystemService.ts`, `ApiSettingsView.tsx`, `geminiService.ts`, `useImageGeneration.ts`, `keyManager.ts`, `ProjectManager.tsx`, and `GoogleAdapter.ts`. Handle each as a separate minimal slice with a source contract.
2. Next cleanup candidate should be chosen from a fresh TS6133 cluster scan after M71. Keep it separate from storage persistence, API/key settings, Gemini/provider routing, billing/payment, and keyManager work unless explicitly scoped.
