...
- Alternate-git worktree was clean at `63386046` before this cost service import-only cleanup pass.
...
- Current giant tracked files after the M91 working tree: `src/index.css` 13552 physical lines, `src/App.tsx` 4812, `src/services/auth/keyManager.ts` 4338, `src/services/llm/OpenAICompatibleAdapter.ts` 4337, `src/components/layout/PromptBar.tsx` 3989, `src/components/settings/ApiSettingsView.tsx` 3385, `src/components/layout/ChatSidebar.tsx` 2743, `src/app/useGenerationRuntime.ts` 2603, `src/context/CanvasContext.tsx` 2516, `src/components/canvas/PromptNodeComponent.tsx` 2244, `src/services/model/modelCapabilities.ts` 1806, `src/services/storage/fileSystemService.ts` 1577, `src/services/billing/costService.ts` 792, `src/services/chat/chatService.ts` 179, `src/app/useWorkflowActions.ts` 498, `src/components/settings/ProjectManager.tsx` 842, `src/components/Onboarding/OnboardingManager.tsx` 262, `src/components/Onboarding/OnboardingOverlay.tsx` 200, `src/components/Onboarding/useOnboardingProgress.ts` 137, and `src/components/Onboarding/AchievementToast.tsx` 60. Current SystemLogs line counts: `SystemLogsView.tsx` 421 and `SystemLogsView.localized.tsx` 450.
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 423, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The M91 production noUnused filter still exits 2 on 47 broader historical diagnostics, with zero `src/services/billing/costService.ts` matches. These are refactor debt indicators, not release blockers by themselves.
...
## Working Tree M91 (Cost Service Import-Only Cleanup)
...
1. Remaining noUnused cleanup hotspots are now outside App, ChatSidebar, PromptNode, SystemLogs, Onboarding, deleted imageCompression, deleted PixiCanvas, dormant/live canvas residual files, workflow-actions import cleanup, common ErrorBoundary cleanup, generation runtime import cleanup, CanvasContext type-import cleanup, ProjectManager prop-destructure cleanup, ecommerce taskMerger parameter cleanup, model display-name provider-parameter cleanup, video-service cleanup, image priority/LOD cleanup, small audio/Volcengine adapter cleanup, chat-service cleanup, generation-runtime contract test cleanup, prompt optimizer duplicate-tab cleanup, KK API client DTO cleanup, user API payload secret cleanup, and cost service import cleanup. The largest production clusters are `useImageGeneration.ts`, `geminiService.ts`, `ApiSettingsView.tsx`, `keyManager.ts`, and `GoogleAdapter.ts`. Handle each as a separate minimal slice with a source contract.
2. Next cleanup candidate should be chosen from a fresh TS6133 cluster scan after M91. Current lower-risk candidate from subagent review is the single unused private `buildInvocationError` helper in `src/services/model/secureModelProxy.ts`; do not include the route-gate helpers in the same slice because existing contracts assert them. Keep the next slice separate from storage persistence, API/key settings, Gemini/provider routing, billing/payment behavior, and keyManager work unless explicitly scoped.
...
- The alternate-git worktree was clean at `63386046` before this cost service import-only cleanup pass, but any staging must still be explicit path-based and reviewed before commit.
