...
- Turnstile verification result: live bundle probing confirmed the Turnstile site key and enabled flag are present in served assets. `GET /`, `GET /healthz`, and admin `/login` return 200; a fake Turnstile token returns `403 TURNSTILE_FAILED`, proving the server-side verifier is active. A no-token password-login probe still returns `401 AUTH_REQUIRED`, which confirms the login route is reachable and no longer 404/502.
