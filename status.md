...
- Fresh DNS/VPS blocker evidence: the local resolver path and direct `Resolve-DnsName` checks against `1.1.1.1`, `8.8.8.8`, `autumn.ns.cloudflare.com`, and `langston.ns.cloudflare.com` return `A api.kkai.plus -> 198.18.0.73`, while Node DoH checks against `https://cloudflare-dns.com/dns-query` and `https://dns.google/resolve` return no A answer and only the Cloudflare SOA. Both evidence paths agree that `api.kkai.plus` is not resolving to the required VPS address `172.245.156.16`. Local environment variables `CF_API_TOKEN`, `CLOUDFLARE_API_TOKEN`, and `CLOUDFLARE_ZONE_ID` are unset, and `node scripts/deploy/cloudflare-upsert-api-dns.mjs` fails closed with the expected missing-token message. Fresh VPS HTTP smoke still returns `200` for `http://172.245.156.16/healthz`, `200` for `/api/manifest`, `401 AUTH_REQUIRED` for `/api/v1/auth/session`, and `404` for both `/internal` and `/internal/`. `https://api.kkai.plus/healthz` remains unavailable, so the full-line hosted/VPS release is still blocked until DNS is updated and the TLS helper completes.
