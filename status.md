...
- Alternate-git worktree was clean at `c29effe5` before this ProjectManager prop-destructure cleanup pass.
...
- Current focus: finish ProjectManager TS6133 cleanup by removing unused mobile prompt optimization props from component destructuring while preserving the public props interface.
...
- Current commit scope: `src/components/settings/ProjectManager.tsx`, `tests/unit/project-manager-unused-cleanup-contract.test.ts`, `tsconfig.tests.json`, and the four ledger files. ProjectManager JSX/control behavior, visual styling, dropdown behavior, workflow UI, canvas operations, provider routing, endpoint/auth changes, release metadata, payment business logic, PPT behavior, storage behavior, keyManager, and unrelated architecture helpers remain excluded.
- Browser QA: skipped for this M80 slice because it only removes unused destructured props, preserves the public props interface, does not change JSX/CSS/control behavior, and passes Clay/theme/workspace source contracts.
...
- Current giant tracked files after the ProjectManager prop-destructure cleanup working tree: `src/index.css` 13553 physical lines, `src/App.tsx` 4812, `src/services/auth/keyManager.ts` 4338, `src/services/llm/OpenAICompatibleAdapter.ts` 4337, `src/components/layout/PromptBar.tsx` 3965, `src/components/settings/ApiSettingsView.tsx` 3385, `src/components/layout/ChatSidebar.tsx` 2743, `src/app/useGenerationRuntime.ts` 2603, `src/context/CanvasContext.tsx` 2516, `src/components/canvas/PromptNodeComponent.tsx` 2241, `src/services/storage/fileSystemService.ts` 1577, `src/app/useWorkflowActions.ts` 498, `src/components/settings/ProjectManager.tsx` 840, `src/components/Onboarding/OnboardingManager.tsx` 262, `src/components/Onboarding/OnboardingOverlay.tsx` 200, `src/components/Onboarding/useOnboardingProgress.ts` 137, and `src/components/Onboarding/AchievementToast.tsx` 60. Current SystemLogs line counts: `SystemLogsView.tsx` 421 and `SystemLogsView.localized.tsx` 450.
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 421, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The filtered noUnused probe now shows 63 TS6133 diagnostics plus 4 TS619x diagnostics. These are refactor debt indicators, not release blockers by themselves.
...
## Completed In Current Slice (ProjectManager Prop Destructure Cleanup)
...
1. Close the current ProjectManager prop-destructure cleanup slice first; do not claim final completion while additional Stage Two seams and remaining TS6133/TS619x hotspots remain open.
...
1. Remaining noUnused cleanup hotspots are now outside App, ChatSidebar, PromptNode, SystemLogs, Onboarding, deleted imageCompression, deleted PixiCanvas, dormant/live canvas residual files, workflow-actions import cleanup, common ErrorBoundary cleanup, generation runtime import cleanup, CanvasContext type-import cleanup, and ProjectManager prop-destructure cleanup. The largest clusters are `useImageGeneration.ts`, `geminiService.ts`, `ApiSettingsView.tsx`, `keyManager.ts`, and `GoogleAdapter.ts`. Handle each as a separate minimal slice with a source contract.
2. Next cleanup candidate should be chosen from a fresh TS6133 cluster scan after M80. Current lowest-risk candidates are `taskMerger.ts` unused parameter cleanup, `modelCapabilities.ts` unused public parameter rename, or video-service import/parameter cleanup. Keep the next slice separate from storage persistence, API/key settings, Gemini/provider routing, billing/payment, and keyManager work unless explicitly scoped.
