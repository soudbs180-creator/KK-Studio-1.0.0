...
- Alternate-git worktree was clean at `8f878b3a` before this ledger alignment pass.
...
## Current OpenAI-Compatible Unreachable Image Fallback Cleanup
...
1. M58 is the active cleanup slice; after it lands, continue with the next smallest quality/architecture slice only after a fresh seam map, not by blind deletion.
2. Recommended next slice from read-only seam review: compiler/source-proven unused locals/imports narrowed to `src/components/layout/PromptBar.tsx` and `src/components/image/ImageCard2.tsx`; do not touch `src/App.tsx` unless a fresh diagnostic produces App-specific proof.
3. Remaining OpenAI-compatible adapter seam: provider image routing dispatch is still inside `OpenAICompatibleAdapter.ts`; map it before extracting because current tests intentionally guard `resolveImageSurface` usage and endpoint fallbacks. Do not change endpoint selection, auth, fetch behavior, or fallback ordering without a dedicated behavior test.
...
- The alternate-git worktree was clean at `8f878b3a` before this ledger-only alignment pass, but any staging must still be explicit path-based and reviewed before commit.
