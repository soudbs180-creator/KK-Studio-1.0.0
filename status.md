...
- Alternate-git worktree was clean at `3dab3056` before the M112 extraction pass.
...
Fresh remaining-work assessment after the M112 working-tree validation:

- Current fact source: the latest committed code slice before M112 is `3dab3056`, and M112 is the current validated commit scope.
- Current gate spot-checks: M112 targeted gate passed 21/21; `npx.cmd tsc --noEmit --noUnusedLocals true --noUnusedParameters true --pretty false` passed; `npm.cmd run architecture:check` passed with the existing allowlisted exceptions; `npm.cmd run typecheck`, `npm.cmd run test:unit`, `npm.cmd run build`, `npm.cmd run governance:agent-docs`, and `npm.cmd run check:encoding` passed.
- Largest real source files still above 2k lines: `src/App.tsx` 4813; `src/services/auth/keyManager.ts` 4311; `src/services/llm/OpenAICompatibleAdapter.ts` 4009; `src/components/layout/PromptBar.tsx` 3990; `src/components/settings/ApiSettingsView.tsx` 3348; `src/components/layout/ChatSidebar.tsx` 2744; `src/app/useGenerationRuntime.ts` 2604; `src/context/CanvasContext.tsx` 2517; `src/components/canvas/PromptNodeComponent.tsx` 2245; `apps/api/src/modules/model-proxy/application/local-user-route-proxy-service.ts` 2232; `apps/api/src/server.ts` 2118; `src/hooks/useImageGeneration.ts` 2032.
- Current broad debt counts across `src`, `apps`, `packages`, `tests`, and `scripts`: broad `any` token matches 533; TS suppressions 133; `console.log` 358.
...
1. The current noUnused filter is clean; the route-gate helpers are wired into live entrypoints; M107 moved OpenAI-compatible post-surface image dispatch planning into a pure helper; M108 moved image payload URL/base64 extraction into a pure helper; M109 moved OpenAI image sizing/profile logic into a pure helper; M110 moved generic task payload parsing into a pure helper; M111 moved generic task result assembly into a pure helper; and M112 moved local user-route auth/header/query-key and route-surface helpers into a pure server helper.
