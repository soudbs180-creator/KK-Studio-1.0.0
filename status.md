...
- Follow-up VPS hardening: the deploy script now installs `deploy/nginx/kk-vps-gateway.conf`, disables the old enabled `kk-api.conf` default site that was capturing `http://172.245.156.16/`, and runs `nginx -t` before reload. The API default Turnstile verifier now uses `TURNSTILE_SECRET_KEY` against Cloudflare siteverify, while self-hosted operators can explicitly set `KK_AUTH_REQUIRE_TURNSTILE=false` when they have not provisioned Turnstile yet.
- Fresh validation passed: targeted auth/VPS suite passed 44/44; focused API startup Turnstile/CORS/legacy-login gate passed 3/3; `npm.cmd run typecheck` passed; `npm.cmd run build` passed; `npm.cmd run admin:build` passed; `npm.cmd run governance:agent-docs` passed; `npm.cmd run check:encoding` passed.
- Deployment note: this fix changes frontend bundles, API code, nginx, and deploy scripts. The live VPS will still show the old `HTTP_404`/default API root behavior until the updated repo is deployed, nginx config is installed/reloaded, and `kk-api` is restarted. Browser site data may also need clearing if an old `temp_user_session_v1` exists. For production CAPTCHA, configure both `VITE_TURNSTILE_SITE_KEY` and `TURNSTILE_SECRET_KEY`; without those, use the explicit no-CAPTCHA self-host setting only as a temporary operator choice.
