...
- Alternate-git worktree was clean at `6028e59a` before this keyManager unused cleanup pass.
...
- Current focus: M105 keyManager unused cleanup is in final verification and path-limited staging/commit.
...
- Current commit scope: `src/services/auth/keyManager.ts`, `tests/unit/key-manager-dead-code-pruning-contract.test.ts`, `tests/unit/key-manager-model-helpers-contract.test.ts`, `tests/unit/google-official-gemini-protocol-guards.test.ts`, and the four ledger files. Key storage, provider credential values, cloud payload shape, storage persistence behavior, provider routing behavior, release metadata, UI surfaces, generation/PPT runtime behavior, and unrelated architecture helpers remain excluded.
- Browser QA: skipped for M105 because this is a non-UI service/source-contract cleanup with no JSX, CSS, route rendering, or browser-visible behavior change.
...
1. Remaining noUnused cleanup after M105 is 2 diagnostics: `src/services/model/secureModelProxy.ts` retains `shouldUseLocalSystemProxy` and `shouldUseLocalUserRouteApi`. These source-contracted route-gate helpers are intentional exceptions until the next slice maps route-gate behavior; do not remove them blindly.
2. Next cleanup candidate should be chosen from a fresh TS6133 cluster scan after the M105 commit. Keep the next slice separate from keyManager secrets/cloud sync, provider routing, storage persistence, release metadata, and UI behavior unless explicitly scoped.
