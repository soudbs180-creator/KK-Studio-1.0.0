...
- Current giant tracked files at `7cbd7346`: `src/services/auth/keyManager.ts` 5279 lines, `src/App.tsx` 4900 lines, `src/services/llm/OpenAICompatibleAdapter.ts` 4517 lines, `src/components/layout/PromptBar.tsx` 4466 lines, `src/context/CanvasContext.tsx` 2563 physical lines.
...
1. Commit this ledger alignment so the docs match `7cbd7346`.
2. Identify whether a remaining narrow `CanvasContext.tsx` seam is safer than moving on; defer `migrateNodes`, IndexedDB/local-folder movement, and persistence orchestration until async caching can be split safely.
