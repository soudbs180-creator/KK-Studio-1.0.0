...
- Alternate-git worktree was clean at `c80ffa70`; current uncommitted work is the scoped Canvas prompt-image relationship helper extraction.
...
- Current giant tracked files in the active Stage Two M17 working tree: `src/services/auth/keyManager.ts` 5279 lines, `src/App.tsx` 4900 lines, `src/services/llm/OpenAICompatibleAdapter.ts` 4517 lines, `src/components/layout/PromptBar.tsx` 4437 lines, `src/context/CanvasContext.tsx` 3852 physical lines.
...
## Stage Two M17 Canvas Prompt-Image Links Extraction
...
1. Commit Stage Two M17 Canvas prompt-image link helper extraction as a separate architecture commit.
2. Identify the next narrow `CanvasContext.tsx` responsibility after prompt-image links, based on a source map and targeted contract first. Defer `migrateNodes` until async IndexedDB caching can be split safely.
