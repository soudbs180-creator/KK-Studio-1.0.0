...
- Active lane in this thread: single-line Stage Two giant-file split plus finalization audit. Stage One M6 and Stage One Backfill are complete; Stage Two M1-M55 are committed through `6902b79b`; the recent unused-code cleanup line is `b9baa445` (PromptBar/ImageCard), `b6767e85` (App), `fafecef9` (OpenAI-compatible adapter), `783fddeb` (LLMService), `92ee7a4f` (pure utility), `3108a29f` (ChatSidebar), `0efba271` (PromptNode), and `0797bf95` (SystemLogs).
- Current slice override: `0797bf95` committed the SystemLogs unused cleanup; the active working-tree slice is `src/components/Onboarding/AchievementToast.tsx` compiler-source unused cleanup.
- Clay UI audit closure landed in `9e7ae2b5` and is no longer the active lane.
- Current branch: `main`.
- Plain `.git` still reports `4c448660` and is a stale historical view. The writable full Git metadata copy at `node_modules/.codex-git-full` is the only development fact source; the latest committed baseline before the current slice is `0797bf95 refactor: prune system logs unused code`. Use `git --git-dir=node_modules/.codex-git-full --work-tree=.` for status/staging/commits in this session.
...
- Alternate-git worktree was clean at `0797bf95` before this Onboarding cleanup pass, except for the untracked generated `tsconfig.node.tsbuildinfo` file from typecheck.
...
- Current focus: finish the compiler-source unused cleanup seam narrowed to `src/components/Onboarding/AchievementToast.tsx`; that file now has zero filtered `noUnusedLocals/noUnusedParameters` matches.
...
- Current commit scope: `src/components/Onboarding/AchievementToast.tsx`, `tests/unit/onboarding-unused-cleanup-contract.test.ts`, `tsconfig.tests.json`, and the four ledger files. Provider routing, endpoint/auth changes, release metadata, payment business logic, PPT/runtime behavior, broad UI redesign, storage/auth/proxy/billing files, and unrelated architecture helpers remain excluded.
- Browser QA: production-browser smoke passed for this visual-no-op source cleanup; the default unauthenticated route loads the login screen, while the achievement toast itself is not reachable without completing onboarding tasks.
...
- Current giant tracked files after the Onboarding cleanup working tree: `src/index.css` 13553 physical lines, `src/App.tsx` 4812, `src/services/auth/keyManager.ts` 4338, `src/services/llm/OpenAICompatibleAdapter.ts` 4337, `src/components/layout/PromptBar.tsx` 3965, `src/components/settings/ApiSettingsView.tsx` 3385, `src/components/layout/ChatSidebar.tsx` 2743, `src/app/useGenerationRuntime.ts` 2604, `src/context/CanvasContext.tsx` 2518, `src/components/canvas/PromptNodeComponent.tsx` 2241, `src/services/storage/fileSystemService.ts` 1577, and `src/components/Onboarding/AchievementToast.tsx` 60. Current SystemLogs line counts: `SystemLogsView.tsx` 421 and `SystemLogsView.localized.tsx` 450.
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 433, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 247. The filtered noUnused probe now shows 108 remaining diagnostics overall. These are refactor debt indicators, not release blockers by themselves.
...
## Completed In Current Slice (Onboarding Unused Cleanup)
...
1. Close the current Onboarding unused cleanup slice first; do not claim final completion while additional Stage Two seams and remaining TS6133 hotspots remain open.
...
1. Remaining noUnused cleanup hotspots are now outside App, ChatSidebar, PromptNode, SystemLogs, and the current AchievementToast cleanup; the largest clusters are `fileSystemService.ts`, `ApiSettingsView.tsx`, `geminiService.ts`, `useImageGeneration.ts`, `keyManager.ts`, `ProjectManager.tsx`, and `GoogleAdapter.ts`. Handle each as a separate minimal slice with a source contract.
2. Low-risk follow-up candidates from the latest read-only explorer pass are Onboarding residual cleanup, dormant canvas support cleanup, and pure image utility cleanup. Keep these separate from storage persistence, API/key settings, Gemini/provider routing, billing/payment, and keyManager work.
