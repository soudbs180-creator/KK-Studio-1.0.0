...
- Alternate-git worktree was clean at `95c5fc16` before this NewAPI management unused cleanup pass.
...
- Current focus: M96 NewAPI management unused cleanup has full validation evidence and is ready for path-limited staging/commit.
...
- Current commit scope: `src/services/billing/newApiManagementService.ts`, `tests/unit/billing-service-unused-cleanup-contract.test.ts`, `tsconfig.tests.json`, and the four ledger files. NewAPI endpoints, auth/header behavior, request/error handling, channel cache key/data shape, provider routing, API/settings UI files, release metadata, keyManager secret storage, storage persistence behavior, generation/PPT runtime behavior, and unrelated architecture helpers remain excluded.
- Browser QA: skipped for M96 because this is a non-UI billing service cleanup with no JSX, CSS, route rendering, or browser-visible behavior change.
...
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 423, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The M96 production noUnused filter still exits non-zero on 41 broader historical diagnostics, with zero `src/services/billing/newApiManagementService.ts` diagnostics. These are refactor debt indicators, not release blockers by themselves.
