...
- Alternate-git worktree was clean at `5994c34b`; current uncommitted work is the scoped Canvas selected-node movement helper extraction.
...
- Current giant tracked files in the active Stage Two M13 working tree: `src/services/auth/keyManager.ts` 5279 lines, `src/App.tsx` 4900 lines, `src/services/llm/OpenAICompatibleAdapter.ts` 4517 lines, `src/components/layout/PromptBar.tsx` 4437 lines, `src/context/CanvasContext.tsx` 4015 physical lines.
...
## Stage Two M13 Canvas Movement Extraction
...
1. Commit Stage Two M12 Canvas group management helper extraction as a separate architecture commit.
2. Identify the next narrow `CanvasContext.tsx` responsibility after group management, based on a source map and targeted contract first. The likely next seam remains selected-node movement batching/application, but it is larger and must be split only after a fresh source scan.
