...
- Alternate-git worktree was clean at `c0c37736` before the scoped Canvas arrange-selection helper extraction.
...
- Current giant tracked files in the active Stage Two M22 working tree: `src/services/auth/keyManager.ts` 5279 lines, `src/App.tsx` 4900 lines, `src/services/llm/OpenAICompatibleAdapter.ts` 4517 lines, `src/components/layout/PromptBar.tsx` 4437 lines, `src/context/CanvasContext.tsx` 3571 physical lines.
...
1. Commit Stage Two M22 Canvas arrange-selection helper extraction as a separate helper/layout commit.
2. Identify the next narrow `CanvasContext.tsx` responsibility after the M22 commit, based on a source map and targeted contract first. Defer `migrateNodes` until async IndexedDB caching can be split safely.
