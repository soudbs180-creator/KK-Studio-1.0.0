...
- Alternate-git worktree was clean at `c4526e6b` before this ecommerce taskMerger parameter cleanup pass.
...
- Current focus: finish ecommerce taskMerger TS6133 cleanup by renaming the unused private `buildTemplateCopySeed` template parameter while preserving copy/style/layout/inherit merge behavior.
...
- Current commit scope: `src/services/ecommerce/taskMerger.ts`, `tests/unit/ecommerce-task-services.test.ts`, and the four ledger files. Ecommerce behavior changes, render task generation, runtime wiring, ProjectManager/UI files, provider routing, endpoint/auth changes, release metadata, payment business logic, PPT behavior, storage behavior, keyManager, and unrelated architecture helpers remain excluded.
- Browser QA: skipped for this M81 slice because it only renames a private unused parameter in an ecommerce service and does not change JSX/CSS or browser-visible behavior.
...
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 421, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The filtered noUnused probe now shows 62 TS6133 diagnostics plus 4 TS619x diagnostics. These are refactor debt indicators, not release blockers by themselves.
...
## Completed In Current Slice (Ecommerce Task Merger Parameter Cleanup)
...
1. Close the current ecommerce taskMerger parameter cleanup slice first; do not claim final completion while additional Stage Two seams and remaining TS6133/TS619x hotspots remain open.
...
1. Remaining noUnused cleanup hotspots are now outside App, ChatSidebar, PromptNode, SystemLogs, Onboarding, deleted imageCompression, deleted PixiCanvas, dormant/live canvas residual files, workflow-actions import cleanup, common ErrorBoundary cleanup, generation runtime import cleanup, CanvasContext type-import cleanup, ProjectManager prop-destructure cleanup, and ecommerce taskMerger parameter cleanup. The largest clusters are `useImageGeneration.ts`, `geminiService.ts`, `ApiSettingsView.tsx`, `keyManager.ts`, and `GoogleAdapter.ts`. Handle each as a separate minimal slice with a source contract.
2. Next cleanup candidate should be chosen from a fresh TS6133 cluster scan after M81. Current lowest-risk candidates are `modelCapabilities.ts` unused public parameter rename or video-service import/parameter cleanup. Keep the next slice separate from storage persistence, API/key settings, Gemini/provider routing, billing/payment, and keyManager work unless explicitly scoped.
