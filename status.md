...
- Alternate-git worktree was clean at `324b42a6` before this image-service unused cleanup pass.
...
- Current focus: finish image-service TS6133 cleanup by removing unused priority-loader and LOD locals while preserving queue sorting/execution, LOD lookup, cache mutation, and fallback behavior.
...
- Current commit scope: `src/services/image/imagePriorityLoader.ts`, `src/services/image/lodService.ts`, `tests/unit/image-service-unused-cleanup-contract.test.ts`, `tsconfig.tests.json`, and the four ledger files. Queue ordering changes, task execution changes, lazy-image behavior changes, LOD thresholds, storage lookup order, cache mutation, memory-manager calls, fallback URL behavior, UI image rendering, provider routing, endpoint/auth changes, release metadata, payment business logic, storage persistence policy, and unrelated architecture helpers remain excluded.
- Browser QA: skipped for this M84 slice because it only removes unused service internals and updates tests/docs; it does not change JSX/CSS or browser-visible image rendering behavior.
...
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 423, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The filtered noUnused probe now shows 55 TS6133 diagnostics plus 3 TS619x diagnostics. These are refactor debt indicators, not release blockers by themselves.
...
## Active M84 Slice (Image Service Unused Cleanup)
...
1. Close the current image-service unused cleanup slice first; do not claim final completion while additional Stage Two seams and remaining TS6133/TS619x hotspots remain open.
...
1. Remaining noUnused cleanup hotspots are now outside App, ChatSidebar, PromptNode, SystemLogs, Onboarding, deleted imageCompression, deleted PixiCanvas, dormant/live canvas residual files, workflow-actions import cleanup, common ErrorBoundary cleanup, generation runtime import cleanup, CanvasContext type-import cleanup, ProjectManager prop-destructure cleanup, ecommerce taskMerger parameter cleanup, model display-name provider-parameter cleanup, video-service cleanup, and image priority/LOD cleanup. The largest clusters are `useImageGeneration.ts`, `geminiService.ts`, `ApiSettingsView.tsx`, `keyManager.ts`, and `GoogleAdapter.ts`. Handle each as a separate minimal slice with a source contract.
2. Next cleanup candidate should be chosen from a fresh TS6133 cluster scan after M84. Current lower-risk candidates include small audio/Volcengine adapter parameter cleanup or isolated storage helper locals. Keep the next slice separate from storage persistence, API/key settings, Gemini/provider routing, billing/payment, and keyManager work unless explicitly scoped.
