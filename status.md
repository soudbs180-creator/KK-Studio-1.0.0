...
- Alternate-git worktree was clean at `943af671` before the scoped Canvas unused-code cleanup.
...
- Current giant tracked files in the active Stage Two M21 working tree: `src/services/auth/keyManager.ts` 5279 lines, `src/App.tsx` 4900 lines, `src/services/llm/OpenAICompatibleAdapter.ts` 4517 lines, `src/components/layout/PromptBar.tsx` 4437 lines, `src/context/CanvasContext.tsx` 3633 physical lines.
...
1. Commit Stage Two M21 Canvas unused-code cleanup as a separate cleanup commit.
2. Identify the next narrow `CanvasContext.tsx` responsibility after cleanup, based on a source map and targeted contract first. Defer `migrateNodes` until async IndexedDB caching can be split safely.
