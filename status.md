...
- Current production parity check for the canvas card overflow fix: local preview `http://127.0.0.1:4173/` served rebuilt assets `assets/index-u_AvA3Y2.js` and `assets/canvas-core-CY0Nrr1E.js`, which contain the card text-wrap/min-size fix fingerprints (`min-w-0 max-w-full break-words`, `[overflow-wrap:anywhere]`, `Math.max(260`, and `overflow-x-hidden overflow-y-auto`). Production `https://kkai.plus/` currently serves `assets/index-C92hbb3n.js` and `assets/canvas-core-CUNRGw_l.js`; neither production JS asset contains those fix fingerprints. `https://kkai.plus/app-version.json` reports `commitSha: 9aa070f87ec4cb1e494404b2eb4f2e04d197772b` and `buildTime: 2026-05-09T20:15:06.564Z`, while local `dist/app-version.json` reports `buildTime: 2026-05-10T04:00:09.690Z` and `commitSha: null`. Conclusion: local build includes the UI fix, but current production does not; deploying from the old normal `.git`/remote state would keep the UI mismatch.
- Current canvas card overflow fix browser QA: Codex in-app Browser opened the rebuilt preview at `http://127.0.0.1:4173/` after `npm.cmd run build`. DOM/title evidence confirmed the workspace shell loaded as `KK Studio - AI Image Workspace`, the prompt textbox was present, stale chunk text was absent, and `.theme-transitioning` was absent. In-app screenshot capture timed out via `Page.captureScreenshot`, so this slice uses DOM/browser-log evidence rather than pixel evidence. Browser logs showed 3 existing `[AdminModelService] Failed to fetch` errors from the account/admin model chunk; these are unrelated to the canvas card text wrapping path.
- Current canvas card overflow fix implementation: `PromptNodeComponent` now applies shared break-word/`overflow-wrap:anywhere` protection and hides horizontal overflow for prompt text; `WorkflowUtilityCard` now clamps persisted card width/height to minimum dimensions and applies the same wrapping protection to subtitle/info rows. The regression is locked in `tests/unit/canvas-visual-regression.test.ts`.
