...
- Production state after code fix: live `/api/v1/model-catalog/active` and `/active-credit-models` still return empty because no real production admin provider row/key has been saved yet. This commit unblocks the admin configuration path; it does not inject production secrets or mutate production DB rows.
- Next step: finish governance/encoding/diff validation, stage the included files through alternate git, commit the slice, then either deploy the admin app to the VPS with existing operations access or use the production admin UI to save the real provider key/model row.
