...
- Current focus: finish and commit Stage Two M35 by extracting the duplicated ASCII API-key sanitizer from `src/services/auth/keyManager.ts` into `src/services/auth/keyManagerCredentialSanitizer.ts`.
- Most recent code/security scopes: Stage Two M35 credential sanitizer extraction in the active working tree; Stage Two M34 route ID helper extraction in `a598312d`; Stage Two M33 provider usage helper extraction in `dd3ad358`; Stage Two M32 provider linked-slot matching helper extraction in `615b7969`; Stage Two M31 provider runtime-state merge helper extraction in `56debf21`; Stage Two M30 model normalization facade consolidation in `08eb89d8`; Stage Two M29 keyManager key type split in `3ce7ae59`; Stage Two M28 keyManager model helper split in `a174b557`; Stage Two M27 prompt-node update helper in `b16843ee`; Stage Two M26 auto-arrange helper in `7cbd7346`; Netlify raw-key endpoint cleanup in `0603547a`; protobuf override in `4cdbf4cf`; portable metadata refresh in `567f85aa`; Nutrient OCR server-key hardening in `0c5cadde`; PostCSS patch in `333f2551`; dead AI12 service pruning in `b6620ef2`.
...
- Browser QA: skipped for Stage Two M35 because this is a non-UI pure service/helper extraction with no visual surface, CSS, route component, or browser-visible workflow changed.
...
- Current giant tracked files after the active Stage Two M35 working tree: `src/services/auth/keyManager.ts` 4871 lines, `src/App.tsx` 4385 lines, `src/services/llm/OpenAICompatibleAdapter.ts` 3980 lines, `src/components/layout/PromptBar.tsx` 4075 lines, `src/context/CanvasContext.tsx` 2198 physical lines. `src/services/auth/keyManagerCredentialSanitizer.ts` is 3 lines, `src/services/auth/keyManagerRouteIds.ts` is 94 lines, `src/services/auth/keyManagerProviderUsage.ts` is 67 lines, `src/services/auth/keyManagerProviderLinks.ts` is 154 lines, `src/services/auth/keyManagerProviders.ts` is 102 lines, `src/services/auth/keyManagerModelHelpers.ts` is 168 lines, `src/services/auth/keyManagerKeyType.ts` is 9 lines, and `src/utils/modelIdNormalization.ts` is 6 lines.
...
1. After the scoped Stage Two M35 commit lands, start the next highest-value keyManager seam from the reviewed candidate list: centralize channel-config API-key non-exposure checks or extract a very small browser-direct diagnostics guard. Keep either slice narrow and verify with key boundary, provider runtime, architecture, security, typecheck, unit, build, and encoding gates.
2. Keep the M35 commit scope limited to `src/services/auth/keyManager.ts`, `src/services/auth/keyManagerCredentialSanitizer.ts`, `tests/unit/key-manager-credential-sanitizer-contract.test.ts`, `tsconfig.tests.json`, and the four ledger files.
...
- The alternate-git worktree was clean at `a598312d` before this M35 slice, but any staging must still be explicit path-based and reviewed before commit.
