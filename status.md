...
- Alternate-git worktree was clean at `c465146b` before this storage adapter unused cleanup pass.
...
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 423, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The M98 production noUnused filter still exits non-zero on 38 broader historical diagnostics, with zero `src/services/storage/storageAdapter.ts` diagnostics. These are refactor debt indicators, not release blockers by themselves.
