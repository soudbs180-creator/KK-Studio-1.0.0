...
- Deployment note: browser site data may still need clearing if an old `temp_user_session_v1` exists. The VPS was restored with `VITE_TURNSTILE_ENABLED=false` and `KK_AUTH_REQUIRE_TURNSTILE=false` because no Cloudflare Turnstile server secret was present; for production CAPTCHA, configure both `VITE_TURNSTILE_SITE_KEY` and `TURNSTILE_SECRET_KEY`, then re-enable Turnstile.
