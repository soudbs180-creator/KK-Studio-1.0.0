...
- Alternate-git worktree was clean at `d5e6a809` before this billing service unused cleanup pass.
...
- Current focus: M97 billing service unused cleanup has full validation evidence and is ready for path-limited staging/commit.
...
- Current commit scope: `src/services/billing/rechargeSubmissionService.ts`, `tests/unit/billing-service-unused-cleanup-contract.test.ts`, `tsconfig.tests.json`, and the four ledger files. Billing channel fallback behavior, QR display mapping, provider routing, API/settings UI files, release metadata, keyManager secret storage, storage persistence behavior, generation/PPT runtime behavior, and unrelated architecture helpers remain excluded.
- Browser QA: skipped for M97 because this is a non-UI billing service cleanup with no JSX, CSS, route rendering, or browser-visible behavior change.
...
- Current tracked TS/TSX debt scan by alternate-git `git grep` over `*.ts` / `*.tsx`: direct `as any` matches 156, explicit any-type pattern matches 423, `@ts-ignore` / `@ts-expect-error` matches 133, and `console.log` matches 245. The M97 production noUnused filter still exits non-zero on 40 broader historical diagnostics, with zero `src/services/billing/rechargeSubmissionService.ts` diagnostics. These are refactor debt indicators, not release blockers by themselves.
