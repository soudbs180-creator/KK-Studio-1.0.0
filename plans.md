...
- Completed M63 working-tree slice: removed source-proven unused locals from `src/app/promptGroupRenderLayout.ts` and unused private helpers/constants from `src/utils/modelSorting.ts`, with a small App call-site cleanup for the removed layout argument. The full noUnused probe now has zero `promptGroupRenderLayout.ts`, `modelSorting.ts`, and `App.tsx` matches and 135 remaining diagnostics overall.
- Next slice after M63 should be a separate proof for `ChatSidebar.tsx`, `PromptNodeComponent.tsx`, `fileSystemService.ts`, or another remaining noUnused hotspot; do not mix pure utility cleanup with keyManager secret storage, provider routing redesign, or visible UI behavior changes.
