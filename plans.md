...
- Active M85 working-tree slice: remove or mark compiler-proven unused locals in `AudioCompatibleAdapter.ts` and `VolcengineAdapter.ts`, while preserving audio request body/duration/lyrics/continuation behavior and Volcengine chat/image request behavior.
- Next slice after M85 should be selected from a fresh low-risk TS6133 cluster scan; do not mix LLM-adapter unused cleanup with provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, billing/proxy routes, or destructive log actions.
