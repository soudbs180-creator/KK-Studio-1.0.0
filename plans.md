...
- `src/services/llm/OpenAICompatibleAdapter.ts`: 4170 physical lines after the M109 sizing/profile helper extraction; `src/services/llm/openAICompatibleImageDispatch.ts`: 91 physical lines; `src/services/llm/openAICompatibleImagePayload.ts`: 141 physical lines; `src/services/llm/openAICompatibleImageSizing.ts`: 81 physical lines.
...
- Completed M109 in the current line: extracted OpenAI-compatible image profile, count, and size helpers into `src/services/llm/openAICompatibleImageSizing.ts`, preserving endpoint selection, auth, fetch behavior, task polling, compatibility fallback ordering, edit endpoint routing, and provider routing ownership.
- Next slice after M109 should be selected from a fresh seam map; do not mix OpenAI image helper follow-up work with endpoint/auth changes, fetch behavior changes, fallback-order changes, keyManager secrets/cloud sync, provider routing, storage persistence, release metadata, UI behavior, or broad debt cleanup.
