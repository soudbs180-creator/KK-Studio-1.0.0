...
- Active M77 working-tree slice: rename the unused `localizeBoundaryErrorText` language parameter in `src/components/common/ErrorBoundary.tsx` to `_language`, while preserving the call shape, captured-error localization, frosted error surface, and reload behavior. The full noUnused probe now has 70 TS6133 diagnostics plus 4 TS619x diagnostics.
- Next slice after M77 should be a separate proof for the next lowest-risk TS6133 cluster; do not mix common error-boundary cleanup with visible UI redesign, error handling behavior, storage persistence, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
