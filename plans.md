...
- Stage Two M58 `OpenAICompatibleAdapter` unreachable image fallback cleanup is the current slice: it removes post-throw automatic fallback code that could not execute after billing-safe compatibility errors, without changing provider routing, endpoint selection, auth, fetch behavior, UI, or release metadata.
...
- `src/App.tsx`: 4900 physical lines after the latest finalization/security cleanup line.
...
- `src/services/llm/OpenAICompatibleAdapter.ts`: 4405 physical lines after the current unreachable image fallback cleanup.
...
- Current M58 slice: remove source-proven unreachable image fallback code after `buildImageCompatibilityModeError` throws in `OpenAICompatibleAdapter.ts`, preserving the billing-safe fail-closed policy and adding source-contract coverage to prevent the old automatic fallback snippets from returning.
- Next slice after M58 must start with a fresh seam map instead of blind cleanup. Current candidates are compiler-proven unused locals/imports in `PromptBar.tsx` and `ImageCard2.tsx`, provider image-routing dispatch inside `OpenAICompatibleAdapter.ts`, the server route auth/header dedupe seam, a fresh `keyManager.ts` seam map, or a smaller `CanvasContext.tsx` helper seam.
