...
- Active M94 working-tree slice: remove the stale duplicate `billingFeatureEnabled` alias from `src/components/modals/UserProfileModal.tsx` and update `tests/unit/local-runtime-consistency-contract.test.ts` to assert the live `billingUiEnabled` gate instead.
- Next slice after M94 should be selected from a fresh low-risk TS6133 cluster scan; do not mix the billing alias cleanup with BillingContext changes, recharge services, balance/refund logic, provider routing behavior, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, storage persistence, or destructive log actions.
