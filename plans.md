...
The plain `.git` metadata currently still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. The development fact source is the writable full Git metadata copy at `node_modules/.codex-git-full`; the alternate-git HEAD before the current M110 working-tree slice was `cb0d23c9 docs: record remaining refactor assessment`. Use only `git --git-dir=node_modules/.codex-git-full --work-tree=.` for status, staging, diffs, and commits in this session.
...
Commit boundary going forward: UI fixes, runtime/PPT/ecommerce fixes, release metadata, final audit fixes, and Stage Two architecture splits must be staged separately. Key storage, permission checks, encryption helpers, provider credential management, cloud sync, provider persistence, shared pricing cache construction, runtime routing, fetch/header behavior, remote model fetch behavior, 12AI runtime URL resolution, and channel config behavior remain excluded unless a future scoped milestone explicitly maps one of those seams.
...
- `src/services/llm/OpenAICompatibleAdapter.ts`: 4051 physical lines after the current M110 task payload helper extraction; `src/services/llm/openAICompatibleImageDispatch.ts`: 91 physical lines; `src/services/llm/openAICompatibleImagePayload.ts`: 141 physical lines; `src/services/llm/openAICompatibleImageSizing.ts`: 81 physical lines; `src/services/llm/openAICompatibleTaskPayload.ts`: 151 physical lines.
...
- Completed M110 in the current working-tree slice: extracted OpenAI-compatible generic task payload parsing into `src/services/llm/openAICompatibleTaskPayload.ts`, preserving endpoint selection, auth, fetch behavior, polling request URLs, backoff, provider branch selection, result metadata assembly, and fallback ordering.
- Next slice after M110 should be selected from a fresh seam map; do not mix OpenAI task result follow-up work with endpoint/auth changes, fetch behavior changes, fallback-order changes, keyManager secrets/cloud sync, provider routing, storage persistence, release metadata, UI behavior, or broad debt cleanup.
