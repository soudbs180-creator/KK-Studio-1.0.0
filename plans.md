...
- Active M82 working-tree slice: rename the compiler-proven unused public-compatible `getModelDisplayName` provider parameter to `_provider`, while preserving the third-argument call shape and custom-label/model-name behavior. The full noUnused probe now has 61 TS6133 diagnostics plus 4 TS619x diagnostics.
- Next slice after M82 should be selected from a fresh low-risk TS6133 cluster scan; do not mix model display-name cleanup with model/provider routing behavior changes, API/key settings, Gemini routing, UI redesign, storage persistence, keyManager secret storage, billing/proxy routes, or destructive log actions.
