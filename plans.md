...
- Active M97 working-tree slice: remove the uncalled private `normalizeRechargePaymentChannelConfig` helper from `src/services/billing/rechargeSubmissionService.ts` while preserving the default payment-channel config builder, `qrDisplay` mapping, and recharge channel list fallback behavior.
- Next slice after M97 should be selected from a fresh low-risk TS6133 cluster scan; do not mix the billing cleanup with channel endpoint changes, provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, storage persistence, or destructive log actions.
