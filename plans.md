...
- Completed M62 working-tree slice: removed compiler/source-proven unused `LLMService.ts` imports, stale direct browser-call helper paths, unused adapter fields, and unread public parameters while preserving local user-route proxy first, secure proxy fallback, billing metadata, task-status routing, and browser-direct blocking contracts. The full noUnused probe now has zero `LLMService.ts` matches and 140 remaining diagnostics overall.
- Next slice after M62 should be a separate proof for `ChatSidebar.tsx`, `PromptNodeComponent.tsx`, `fileSystemService.ts`, or another remaining noUnused hotspot; do not mix LLMService cleanup with keyManager secret storage, provider routing redesign, or UI behavior changes.
