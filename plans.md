...
- Active M87 working-tree slice: remove compiler-proven unused local source slices from `tests/unit/generation-runtime-contract.test.ts`, while preserving the generation runtime boundary assertions and avoiding production-code changes.
- Next slice after M87 should be selected from a fresh low-risk TS6133 cluster scan; do not mix test-only cleanup with provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, billing/proxy routes, storage persistence, or destructive log actions.
