...
- `src/services/llm/OpenAICompatibleAdapter.ts`: 4247 physical lines after the M108 payload helper extraction; `src/services/llm/openAICompatibleImageDispatch.ts`: 91 physical lines; `src/services/llm/openAICompatibleImagePayload.ts`: 141 physical lines.
...
- Completed M108 in the current line: extracted OpenAI-compatible image payload URL/base64 parsing into `src/services/llm/openAICompatibleImagePayload.ts`, preserving endpoint selection, auth, fetch behavior, task polling, compatibility fallback ordering, and provider routing ownership.
- Next slice after M108 should be selected from a fresh seam map; do not mix OpenAI image helper follow-up work with endpoint/auth changes, fetch behavior changes, fallback-order changes, keyManager secrets/cloud sync, provider routing, storage persistence, release metadata, UI behavior, or broad debt cleanup.
