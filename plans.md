...
- Active M95 working-tree slice: remove the uncalled private `getErrorMessage` helper from `src/services/api/userApiCloudRecordStorage.ts`, while preserving typed auth payload loading, local API fallback, cache behavior, payload compaction, and secret redaction.
- Next slice after M95 should be selected from a fresh low-risk TS6133 cluster scan; do not mix the user API cloud helper cleanup with provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, billing/payment behavior, storage persistence, or destructive log actions.
