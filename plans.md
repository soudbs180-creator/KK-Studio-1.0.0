...
- Active M90 working-tree slice: remove duplicate unused secret placeholder constants from `src/services/api/userApiPayload.ts`, while preserving the live placeholder/redaction policy in `src/services/api/userApiCloudRecordStorage.ts`.
- Next slice after M90 should be selected from a fresh low-risk TS6133 cluster scan; do not mix user API payload cleanup with provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, billing/payment behavior, storage persistence, or destructive log actions.
