...
- Active M96 working-tree slice: remove the compiler-proven unused `notify` import from `src/services/billing/newApiManagementService.ts` and keep the `getAllChannels()` preflight as an awaited side-effect call without the unread `channels` binding.
- Next slice after M96 should be selected from a fresh low-risk TS6133 cluster scan; do not mix the NewAPI management cleanup with channel endpoint changes, provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, billing/payment behavior, storage persistence, or destructive log actions.
