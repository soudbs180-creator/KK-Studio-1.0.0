...
- Active M73 working-tree slice: rename unused compatibility stub parameters in `src/services/storage/fileSystemService.ts` to `_handle` and `_isVideo`, preserving every public method, call signature order, no-op tag/settings behavior, and App call sites. The full noUnused probe now has 78 TS6133 diagnostics plus 4 TS619x diagnostics.
- Next slice after M73 should be a separate proof for the next lowest-risk TS6133 cluster; do not mix storage stub cleanup with real storage persistence, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
