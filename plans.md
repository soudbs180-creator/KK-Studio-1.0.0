...
- Completed M104 working-tree slice: removed compiler-proven unused imports/constants/helpers from `src/components/settings/ApiSettingsView.tsx`, preserving API settings route visibility, readonly snapshot behavior, BYOK persistence, source encoding guards, and settings workbench UI contracts.
- Next slice after M104 should be selected from a fresh low-risk TS6133 cluster scan; do not mix the API settings cleanup with provider routing behavior changes, model capability policy changes, keyManager secret storage, storage persistence redesign, local folder behavior redesign, or destructive log actions.
