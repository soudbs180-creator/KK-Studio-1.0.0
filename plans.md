...
- `src/services/llm/OpenAICompatibleAdapter.ts`: 4339 physical lines after the M107 dispatch helper extraction; `src/services/llm/openAICompatibleImageDispatch.ts`: 91 physical lines.
...
- Completed M107 working-tree slice: extracted the post-surface `OpenAICompatibleAdapter.ts` image dispatch plan into `src/services/llm/openAICompatibleImageDispatch.ts`, preserving Wuyin/AceData short-circuits, `resolveImageSurface` ownership, endpoint/auth/fetch behavior, provider-specific method calls, and billing-safe fallback throws.
- Next slice after M107 should be selected from a fresh seam map; do not mix OpenAI image dispatch follow-up work with endpoint/auth changes, fetch behavior changes, fallback-order changes, keyManager secrets/cloud sync, provider routing, storage persistence, release metadata, UI behavior, or broad debt cleanup.
