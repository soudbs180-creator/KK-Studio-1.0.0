...
- The source-proven unused cleanup line now includes `b9baa445` (PromptBar/ImageCard), `b6767e85` (App), `fafecef9` (OpenAI-compatible adapter), `783fddeb` (LLMService), `92ee7a4f` (pure utility), `3108a29f` (ChatSidebar), `0efba271` (PromptNode), `0797bf95` (SystemLogs), and `f453cd9a` (AchievementToast). The active working-tree slice is Onboarding residual cleanup.
...
- `src/context/CanvasContext.tsx`: 2518 physical lines after Stage Two M27 prompt-node update extraction and later cleanup, down from 5218 text lines at the start of Stage Two.
...
- `src/services/llm/OpenAICompatibleAdapter.ts`: 4337 physical lines after the current unused cleanup line.
...
- Active M68 working-tree slice: remove compiler-proven unused `OnboardingProgress`, `updateProgress`, no-op task parameter, `ChevronLeft`, `Keyboard`, and the unread `getTasksByPhase` parameter from Onboarding residual files. The filtered noUnused probe now has zero `src/components/Onboarding` matches and 102 remaining diagnostics overall.
- Next slice after M68 should be a separate proof for dormant canvas support cleanup, pure image utility cleanup, `fileSystemService.ts`, `ApiSettingsView.tsx`, `geminiService.ts`, `useImageGeneration.ts`, or another remaining noUnused hotspot. Prefer the low-risk dormant-canvas/pure-image candidates first; do not mix Onboarding cleanup with storage behavior, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
