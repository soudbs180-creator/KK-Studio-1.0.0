...
- Active M106 working-tree slice: wire the `secureModelProxy.ts` route-gate helpers into the live local user-route and local system proxy entrypoints while preserving the current routing behavior and the no-browser-direct / no-stale-fallback contracts.
- Next slice after M106 should be selected from a fresh TS6133 cluster scan or the next largest-file split; do not mix route-gate cleanup with keyManager secrets/cloud sync, provider routing, storage persistence, release metadata, UI behavior, or broad debt cleanup.
