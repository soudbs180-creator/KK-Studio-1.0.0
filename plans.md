...
- Active M72 working-tree slice: remove compiler-proven unused `AlertTriangle`, `ShieldCheck`, and `Wallet` icon imports from the legacy plain `DashboardView.tsx`, while preserving the live localized dashboard route and billing remaining-balance source contract. The full noUnused probe now has 88 TS6133 diagnostics plus 4 TS619x diagnostics.
- Next slice after M72 should be a separate proof for the next lowest-risk TS6133 cluster; do not mix dashboard cleanup with storage behavior, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
