...
- Active M103 working-tree slice: remove compiler-proven unused imports/destructures/locals from `src/hooks/useImageGeneration.ts`, preserving generation recovery, billing, storage persistence, sync bridge recovery, and pending-task state transitions.
- Next slice after M103 should be selected from a fresh low-risk TS6133 cluster scan; do not mix the image generation hook cleanup with UI/settings behavior, provider routing behavior changes, model capability policy changes, API/key settings, keyManager secret storage, storage persistence redesign, local folder behavior redesign, or destructive log actions.
