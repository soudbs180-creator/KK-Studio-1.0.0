...
- The source-proven unused cleanup line now includes `b9baa445` (PromptBar/ImageCard), `b6767e85` (App), `fafecef9` (OpenAI-compatible adapter), `783fddeb` (LLMService), `92ee7a4f` (pure utility), `3108a29f` (ChatSidebar), and `0efba271` (PromptNode). The active working-tree slice is `SystemLogsView`.
...
- Active M66 working-tree slice: remove compiler/source-proven unused `Activity` import and unused `importantLogs` derived values from `src/components/settings/views/SystemLogsView.tsx` and `src/components/settings/views/SystemLogsView.localized.tsx`; added `tests/unit/system-logs-unused-cleanup-contract.test.ts`. The full noUnused probe now has zero `SystemLogsView` matches and 109 remaining diagnostics overall.
- Next slice after M66 should be a separate proof for `fileSystemService.ts`, `ApiSettingsView.tsx`, `geminiService.ts`, `useImageGeneration.ts`, or another remaining noUnused hotspot; do not mix SystemLogs cleanup with storage behavior, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
