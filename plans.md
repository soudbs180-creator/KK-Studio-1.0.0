...
- Active M81 working-tree slice: rename the compiler-proven unused private `buildTemplateCopySeed` template parameter to `_seriesTemplate`, while preserving call arity and ecommerce copy/style/layout/inherit merge behavior. The full noUnused probe now has 62 TS6133 diagnostics plus 4 TS619x diagnostics.
- Next slice after M81 should be selected from a fresh low-risk TS6133 cluster scan; do not mix ecommerce taskMerger parameter cleanup with ecommerce behavior changes, UI redesign, storage persistence, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
