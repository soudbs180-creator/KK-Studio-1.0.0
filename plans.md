...
The plain `.git` metadata currently still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. The development fact source is the writable full Git metadata copy at `node_modules/.codex-git-full`; the latest clean baseline before the current slice is `8b40e892 refactor: extract key manager channel capabilities helper`. Use only `git --git-dir=node_modules/.codex-git-full --work-tree=.` for status, staging, diffs, and commits in this session.
...
- Stage Two M48 `keyManager` API type detector helper split is the current slice: it moves only the pure `detectApiType` string classifier into `src/services/auth/keyManagerApiType.ts`.
...
Commit boundary going forward: UI fixes, runtime/PPT/ecommerce fixes, release metadata, final audit fixes, and Stage Two architecture splits must be staged separately. The active Stage Two slice moves only `detectApiType` from `keyManager.ts` into `keyManagerApiType.ts`; key storage, permission checks, encryption helpers, provider credential management, cloud sync, provider persistence, shared pricing cache construction, runtime routing, fetch/header behavior, model-list filtering call sites, remote model fetch behavior, 12AI runtime URL resolution, and channel config behavior remain excluded.
...
- `src/services/auth/keyManager.ts`: 4662 lines in the active M48 API type detector helper split.
...
- Active M48 slice: extract only the pure `detectApiType` helper into `src/services/auth/keyManagerApiType.ts`, preserving historical case-sensitive substring matching and `keyManager.ts` compatibility re-export.
...
- Continue `src/services/auth/keyManager.ts` after M48 only after a fresh seam map. Defer key storage, permission checks, encryption helpers, provider credential management, cloud sync, provider persistence, shared pricing cache construction, runtime routing, remote model fetch behavior, and localStorage policy until smaller seams are mapped.
