...
- Active M78 working-tree slice: remove the compiler-proven unused `resolveModelDisplayName` import from `src/app/useGenerationRuntime.ts`, while preserving the deps boundary and every runtime call through the injected dependency. The full noUnused probe now has 69 TS6133 diagnostics plus 4 TS619x diagnostics.
- Next slice after M78 should be selected from a fresh low-risk TS6133 cluster scan; do not mix generation runtime import cleanup with visible UI redesign, error handling behavior, storage persistence, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
