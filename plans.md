...
- Active M80 working-tree slice: remove compiler-proven unused mobile prompt optimization props from the `ProjectManager.tsx` destructuring list, while preserving the public `ProjectManagerProps` declarations and JSX/control behavior. The full noUnused probe now has 63 TS6133 diagnostics plus 4 TS619x diagnostics.
- Next slice after M80 should be selected from a fresh low-risk TS6133 cluster scan; do not mix ProjectManager prop-destructure cleanup with UI redesign, storage persistence, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
