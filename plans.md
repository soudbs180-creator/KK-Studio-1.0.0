...
- Active M86 working-tree slice: remove compiler-proven unused imports/locals from `src/services/chat/chatService.ts`, while preserving chat request URL/header construction, keyManager success/failure reporting, saved-message/session behavior, and thrown API error semantics.
- Next slice after M86 should be selected from a fresh low-risk TS6133 cluster scan; do not mix chat-service cleanup with provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, billing/proxy routes, storage persistence, or destructive log actions.
