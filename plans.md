...
- Current M121 slice: move pure Wuyin route/base-url/reference-image/task-status helpers from `OpenAICompatibleAdapter.ts` into `src/services/llm/openAICompatibleWuyinRoute.ts`, preserving endpoint selection, auth, fetch behavior, provider routing, fallback ordering, billing, UI, and release metadata.
- Next priority after M121: security hardening for local user-route task signing secret fallback. Do not continue lower-priority UI/debt cleanup before this hard-coded fallback is mapped and fixed.
- Next architecture slice after the security hardening should be selected from a fresh seam map or quality scan; do not mix auth/header behavior, endpoint call-site behavior, fetch behavior, fallback-order changes, keyManager secrets/cloud sync, provider routing, storage persistence, release metadata, unrelated UI behavior, or broad debt cleanup.
