...
- Active M91 working-tree slice: remove import-only unused `ModelType` and `getRefImageTokenEstimate` from `src/services/billing/costService.ts`, while preserving cost estimation behavior and the live `getModelPricing` / `getImageTokenEstimate` calculation path.
- Next slice after M91 should be selected from a fresh low-risk TS6133 cluster scan; do not mix cost service import cleanup with provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, billing/payment behavior, storage persistence, or destructive log actions.
