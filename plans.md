...
- Active M116 working-tree slice: close ecommerce visibility/localization UI debt by keeping generated ecommerce task cards visible on canvas, hiding only `a-plus-group` helper nodes, and localizing ecommerce/mobile post-build controls. This is a UI slice and must include browser QA before commit.
- Next slice after M116 should be selected from a fresh seam map or quality scan; do not mix auth/header behavior, endpoint call-site behavior, fetch behavior, fallback-order changes, keyManager secrets/cloud sync, provider routing, storage persistence, release metadata, UI behavior, or broad debt cleanup.
