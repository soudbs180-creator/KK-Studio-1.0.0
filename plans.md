...
- Active M71 working-tree slice: keep `Canvas.tsx` and `PendingNode.tsx` props interfaces stable while removing only compiler-proven unused destructured values and unread state values. The full noUnused probe now has 91 remaining diagnostics overall.
- Next slice after M71 should be a separate proof for the next lowest-risk TS6133 cluster; do not mix dormant canvas cleanup with storage behavior, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
