...
- The source-proven unused cleanup line now includes `b9baa445` (PromptBar/ImageCard), `b6767e85` (App), `fafecef9` (OpenAI-compatible adapter), `783fddeb` (LLMService), `92ee7a4f` (pure utility), `3108a29f` (ChatSidebar), `0efba271` (PromptNode), and `0797bf95` (SystemLogs). The active working-tree slice is `AchievementToast`.
...
- Active M67 working-tree slice: remove the compiler-proven unused `Star` import from `src/components/Onboarding/AchievementToast.tsx`; added `tests/unit/onboarding-unused-cleanup-contract.test.ts`. The filtered noUnused probe now has zero `AchievementToast.tsx` matches.
- Next slice after M67 should be a separate proof for Onboarding residual cleanup, dormant canvas support cleanup, pure image utility cleanup, `fileSystemService.ts`, `ApiSettingsView.tsx`, `geminiService.ts`, `useImageGeneration.ts`, or another remaining noUnused hotspot. Prefer the low-risk Onboarding/dormant-canvas/pure-image candidates first; do not mix Onboarding cleanup with storage behavior, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
