...
- Active M99 working-tree slice: rename the compiler-proven unused `prompt` parameter in `src/services/storage/storagePreference.ts` to `_prompt`, preserving the public `saveOriginalToLocalFolder` arity, local-folder write behavior, and merge timestamp call shape.
- Next slice after M99 should be selected from a fresh low-risk TS6133 cluster scan; do not mix the storage preference cleanup with channel endpoint changes, provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, storage persistence redesign, local folder behavior redesign, or destructive log actions.
