...
The plain `.git` metadata currently still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. The development fact source is the writable full Git metadata copy at `node_modules/.codex-git-full`; the latest committed slice before the current security working tree is `c0c96808 refactor: extract gemini image sizing helper`, with exact commits tracked by alternate git. Use only `git --git-dir=node_modules/.codex-git-full --work-tree=.` for status, staging, diffs, and commits in this session.
...
- The source-proven unused cleanup line now includes the M60-M106 cleanup commits through secure proxy route-gate wiring, plus later M114-M117 and the `5aaccf50` ecommerce analysis fallback hotfix. The active line is the M118 legacy payment-server security blocker fix found by finalization audit.
...
- Current M118 security slice: hard-fail legacy WeChat Pay webhook handling when required WeChat Pay credentials are missing, remove placeholder cert/private-key fallbacks, and derive legacy return/notify defaults from the current request origin instead of production `kkai.plus` URLs.
