...
The plain `.git` metadata currently still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. The development fact source is the writable full Git metadata copy at `node_modules/.codex-git-full`; the latest working-tree code line is M112 local user-route auth helper extraction, with exact commits tracked by alternate git. Use only `git --git-dir=node_modules/.codex-git-full --work-tree=.` for status, staging, diffs, and commits in this session.
...
- `apps/api/src/modules/model-proxy/application/local-user-route-proxy-service.ts`: 2232 physical lines after the M112 auth helper extraction; `apps/api/src/modules/model-proxy/application/local-user-route-auth.ts`: 370 physical lines.
...
- Current M112 working-tree slice: extracted local user-route auth/header/query-key, route strategy, route format, and image-surface pure helpers into `apps/api/src/modules/model-proxy/application/local-user-route-auth.ts`, preserving endpoint selection, fetch execution, task operation routing, credential storage/retrieval, provider branch execution, fallback ordering, billing metadata, release metadata, and UI behavior.
- Next slice after M112 should be selected from a fresh seam map; do not mix endpoint selection, fetch behavior, fallback-order changes, keyManager secrets/cloud sync, provider routing, storage persistence, release metadata, UI behavior, or broad debt cleanup.
