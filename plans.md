...
- Active M70 working-tree slice: delete the orphaned `src/components/canvas/PixiCanvas.tsx` dormant renderer after source/import proof that no production code imports it; add `tests/unit/canvas-dormant-unused-cleanup-contract.test.ts` so Pixi canvas entrypoints do not return. The full noUnused probe now has 95 remaining diagnostics overall.
- Next slice after M70 should be a separate proof for remaining dormant canvas support cleanup such as `src/components/canvas/Canvas.tsx` orphan cleanup or `PendingNode.tsx` residual cleanup with the Clay contract preserved, then `fileSystemService.ts`, `ApiSettingsView.tsx`, `geminiService.ts`, `useImageGeneration.ts`, or another remaining noUnused hotspot. Do not mix dormant canvas cleanup with storage behavior, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
