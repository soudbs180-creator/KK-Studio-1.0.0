...
- Active M98 working-tree slice: remove the unused OPFS `compressIfNeeded` import and unused Promise `reject` executor parameter from `src/services/storage/storageAdapter.ts`, preserving import/read/delete behavior and image dimension fallback semantics.
- Next slice after M98 should be selected from a fresh low-risk TS6133 cluster scan; do not mix the storage adapter cleanup with channel endpoint changes, provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, storage persistence redesign, or destructive log actions.
