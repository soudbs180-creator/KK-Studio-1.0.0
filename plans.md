...
- Active M92 working-tree slice: remove the uncalled private `buildInvocationError` helper from `src/services/model/secureModelProxy.ts`, while preserving the two source-contracted local route gate helpers.
- Next slice after M92 should be selected from a fresh low-risk TS6133 cluster scan; do not mix secure model proxy helper cleanup with provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, billing/payment behavior, storage persistence, or destructive log actions.
