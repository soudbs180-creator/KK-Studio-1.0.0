...
- Active M84 working-tree slice: remove compiler-proven unused image priority/LOD locals across `imagePriorityLoader.ts` and `lodService.ts`, while preserving priority sorting, queue execution, LOD thresholds, storage lookup, cache updates, and fallback URL behavior.
- Next slice after M84 should be selected from a fresh low-risk TS6133 cluster scan; do not mix image-service cleanup with storage persistence policy changes, UI image rendering changes, model/provider routing behavior changes, API/key settings, Gemini routing, keyManager secret storage, billing/proxy routes, or destructive log actions.
