...
- Active M83 working-tree slice: remove or mark compiler-proven unused video-service locals across `OpenAIVideoService.ts`, `VeoVideoService.ts`, and `videoService.ts`, while preserving browser fail-closed behavior, Veo request/URL construction, progress labels, polling, downloading, and returned mode.
- Next slice after M83 should be selected from a fresh low-risk TS6133 cluster scan; do not mix video-service cleanup with model/provider routing behavior changes, API/key settings, Gemini routing, UI redesign, storage persistence, keyManager secret storage, billing/proxy routes, or destructive log actions.
