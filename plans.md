...
- Active M100 working-tree slice: remove the compiler-proven unused `db` local from `src/services/storage/imageStorage.ts` `cleanupOriginals()`, preserving batch image pagination, compression, save, and error behavior.
- Next slice after M100 should be selected from a fresh low-risk TS6133 cluster scan; do not mix the image storage cleanup with channel endpoint changes, provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, storage persistence redesign, local folder behavior redesign, or destructive log actions.
