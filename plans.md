...
- Active M79 working-tree slice: remove the compiler-proven unused `CanvasContextType` and `SubCardLayout` type imports from `src/context/CanvasContext.tsx`, while preserving the existing type re-export compatibility boundary. The full noUnused probe now has 67 TS6133 diagnostics plus 4 TS619x diagnostics.
- Next slice after M79 should be selected from a fresh low-risk TS6133 cluster scan; do not mix CanvasContext type-import cleanup with persistence, hydration, cloud sync, local folder logic, visible UI redesign, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
