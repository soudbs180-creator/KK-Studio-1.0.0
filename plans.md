...
- Active M101 working-tree slice: remove import-only compiler-proven unused symbols from `src/services/llm/GoogleAdapter.ts`, preserving Google/Gemini chat, image, audio, and video request behavior plus inline video generation type references.
- Next slice after M101 should be selected from a fresh low-risk TS6133 cluster scan; do not mix the Google adapter import cleanup with channel endpoint changes, provider routing behavior changes, model capability policy changes, API/key settings, Gemini service behavior, keyManager secret storage, storage persistence redesign, local folder behavior redesign, or destructive log actions.
