...
Completed prompt optimizer cache/logging redaction milestone:
- Cache keys now use non-reversible fingerprints for prompt and reference-image content instead of raw prompt/reference substrings.
- The legacy `kk_prompt_optimizer_cache_v4` namespace is cleared when the v5 cache is read, preventing old prompt-bearing keys from surviving silently.
- Persisted optimizer cache results redact `raw_prompt_original` and `params.subject` while preserving functional optimized prompt cache hits.
...
- Stage Two M127 `OpenAICompatibleAdapter` image reference cleanup is completed in `1f0ce6bd`: it moves repeated reference-image formatting into `src/services/llm/openAICompatibleImageReferences.ts` and deletes unreachable commented legacy delegate bodies, while leaving endpoint selection, auth/header behavior, fetch execution, polling, billing, fallback ordering, UI, and release metadata unchanged.
- Stage Two M128 dead Gemini response cache cleanup is completed in `7d65c686`: it deletes the source-proven unreferenced `src/services/storage/cache.ts` module and removes prompt-content diagnostics from the surviving `CanvasContext.addPromptNode` log, while leaving active storage persistence, image storage, file-system service behavior, UI, release metadata, provider routing, and auth/header behavior unchanged.
- Stage Two M129 keyManager update diagnostic redaction is completed in `740042c1`: it moves the `updateKey` console diagnostic payload into `src/services/auth/keyManagerUpdateDiagnostics.ts` so raw `key` / `apiKey` update values are not logged, while leaving key persistence, provider persistence, cloud sync, model discovery, route selection, auth/header behavior, UI, and release metadata unchanged.
- Stage Two M130 OpenAI-compatible diagnostics prompt redaction is completed: it redacts prompt-like fields from JSON/multipart diagnostics previews and makes OpenAI-compatible python snippets use redacted previews or fixed prompt placeholders, while leaving endpoint selection, auth/header behavior, fetch execution, polling, billing, fallback ordering, UI, and release metadata unchanged.
- Stage Two M131 prompt optimizer cache/logging redaction is the current narrow slice: it fingerprints prompt/reference content in optimizer cache keys, drops legacy prompt-bearing cache v4 entries, redacts persisted raw prompt cache fields, and summarizes optimizer failures before logging or user notification, while leaving provider selection, endpoint/auth behavior, billing, storage ownership, UI, and route fallback semantics unchanged.
- The source-proven unused cleanup line now includes the M60-M106 cleanup commits through secure proxy route-gate wiring, plus later M114-M119, the post-M120 UI closure commits `da4ffc79` / `485a6bef` / `1ca080eb`, the `5aaccf50` ecommerce analysis fallback hotfix, M128 dead-cache cleanup, M129 key diagnostic redaction, M130 prompt diagnostics redaction, and M131 prompt optimizer cache/logging redaction. The active line is now finalization audit plus fresh seam-map giant-file splitting after the M131 cleanup slice.
...
- `src/context/CanvasContext.tsx`: 2517 physical lines after Stage Two CanvasContext extractions and the M128 prompt-log redaction, down from 5218 text lines at the start of Stage Two.
...
- `src/services/auth/keyManager.ts`: 3595 physical lines after the M129 update diagnostic redaction.
...
- `src/services/auth/keyManagerUpdateDiagnostics.ts`: 30 lines in the M129 update diagnostic redaction boundary.
...
- `src/services/llm/OpenAICompatibleAdapter.ts`: 2536 physical lines after the current M130 prompt diagnostics redaction; `src/services/llm/openAICompatible12AIAsyncRoute.ts`: 100 physical lines; `src/services/llm/openAICompatibleAceDataRoute.ts`: 153 physical lines; `src/services/llm/openAICompatibleChatPayload.ts`: 66 physical lines; `src/services/llm/openAICompatibleGoogleExtraBody.ts`: 62 physical lines; `src/services/llm/openAICompatibleImageDispatch.ts`: 91 physical lines; `src/services/llm/openAICompatibleImagePayload.ts`: 254 physical lines after M126 URL/MIME hardening; `src/services/llm/openAICompatibleImageReferences.ts`: 57 physical lines after M127; `src/services/llm/openAICompatibleImageSizing.ts`: 99 physical lines; `src/services/llm/openAICompatibleTaskPayload.ts`: 219 physical lines.
...
- `src/services/llm/openAICompatibleDiagnostics.ts`: 95 physical lines in the current M130 diagnostics prompt redaction boundary.
...
- Completed M127 in `1f0ce6bd`: extracted repeated OpenAI-compatible reference-image formatting into `src/services/llm/openAICompatibleImageReferences.ts` and removed unreachable commented legacy delegate bodies from `OpenAICompatibleAdapter.ts`, preserving endpoint selection, auth/header behavior, fetch execution, polling, billing, fallback ordering, UI, and release metadata.
- Completed M128 in `7d65c686`: deleted the source-proven unreferenced `src/services/storage/cache.ts` Gemini response cache and removed prompt text from the surviving `CanvasContext.addPromptNode` diagnostic, preserving active storage persistence, image storage, file-system service behavior, UI, release metadata, provider routing, and auth/header behavior.
