...
- Active M75 working-tree slice: remove unused `onAutoArrange` and `id` destructured values from live `src/components/canvas/InfiniteCanvas.tsx`, while preserving `InfiniteCanvasProps`, canvas behavior, and live canvas visual contracts. The full noUnused probe now has 72 TS6133 diagnostics plus 4 TS619x diagnostics.
- Next slice after M75 should be a separate proof for the next lowest-risk TS6133 cluster; do not mix live canvas cleanup with real canvas behavior changes, storage persistence, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
