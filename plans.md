...
- Active M105 working-tree slice: remove compiler-proven unused bindings from `src/services/auth/keyManager.ts`, preserving compatibility re-exports, key storage, provider persistence, cloud payload shape, and route selection behavior.
- Next slice after M105 should be selected from the remaining `secureModelProxy.ts` source-contracted route-gate helpers or another fresh low-risk TS6133 cluster scan; do not mix route-gate cleanup with keyManager secrets/cloud sync, provider routing, storage persistence, release metadata, UI behavior, or broad debt cleanup.
