...
- Active M89 working-tree slice: remove the unused `AdminRechargeSubmissionDto` direct import from `packages/contracts/src/client/kk-api-client.ts`, while preserving the public billing DTO export and the recharge-submission client response DTOs.
- Next slice after M89 should be selected from a fresh low-risk TS6133 cluster scan; do not mix contract-client cleanup with provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, billing/payment behavior, storage persistence, or destructive log actions.
