...
- Active M88 working-tree slice: remove the duplicate unused `DEFAULT_TABS` constant from `src/services/llm/promptOptimizerService.ts`, while preserving the `HUMAN_DEFAULT_TABS` UI payload wiring and prompt optimizer route behavior.
- Next slice after M88 should be selected from a fresh low-risk TS6133 cluster scan; do not mix prompt optimizer cleanup with provider routing behavior changes, model capability policy changes, API/key settings, Gemini routing, keyManager secret storage, billing/proxy routes, storage persistence, or destructive log actions.
