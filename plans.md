...
- Current M115 working-tree slice: extracted pure remote model discovery response parsing and canonical dedupe into `src/services/auth/keyManagerRemoteModelDiscovery.ts`, preserving fetch execution, endpoint selection, auth/header behavior, provider persistence, cloud sync, key storage, route selection, model resolution, and UI behavior.
- Next slice after M115 should be selected from a fresh seam map; do not mix auth/header behavior, endpoint call-site behavior, fetch behavior, fallback-order changes, keyManager secrets/cloud sync, provider routing, storage persistence, release metadata, UI behavior, or broad debt cleanup.
