...
- Active M69 working-tree slice: delete the orphaned `src/services/image/imageCompression.ts` module after source/import proof that no production code imports it; extend `tests/unit/pure-utility-unused-cleanup-contract.test.ts` so the module and imports do not return. The full noUnused probe now has 101 remaining diagnostics overall.
- Next slice after M69 should be a separate proof for dormant canvas support cleanup, `fileSystemService.ts`, `ApiSettingsView.tsx`, `geminiService.ts`, `useImageGeneration.ts`, or another remaining noUnused hotspot. Prefer dormant-canvas before high-risk storage/API/key/provider candidates; do not mix pure utility cleanup with storage behavior, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
