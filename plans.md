...
- Active M74 working-tree slice: remove import-only unused types/imports from `src/components/image/GlobalLightbox.tsx`, `src/hooks/useImageQuality.ts`, and `src/services/model/modelRegistry.ts`, while preserving lightbox partial-redraw, image-quality hook behavior, and model registry exports. The full noUnused probe now has 74 TS6133 diagnostics plus 4 TS619x diagnostics.
- Next slice after M74 should be a separate proof for the next lowest-risk TS6133 cluster; do not mix import-only cleanup with real UI behavior, storage persistence, keyManager secret storage, provider routing redesign, billing/proxy routes, or destructive log actions.
