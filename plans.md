...
- Active M102 working-tree slice: remove compiler-proven unused imports/helpers from `src/services/llm/geminiService.ts`, preserving Gemini request behavior, error classification, secure-proxy error handling, `llmService.generateImage(llmOptions)`, `calculateCost(...)`, and the `_negativePrompt` call shape.
- Next slice after M102 should be selected from a fresh low-risk TS6133 cluster scan; do not mix the Gemini service cleanup with channel endpoint changes, provider routing behavior changes, model capability policy changes, API/key settings, keyManager secret storage, storage persistence redesign, local folder behavior redesign, or destructive log actions.
