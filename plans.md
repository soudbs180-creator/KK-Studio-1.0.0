...
The plain `.git` metadata currently still reports baseline commit `4c448660 Refactor Clay UI and PPT runtime boundaries` and may show stale dirty state. The development fact source is the writable full Git metadata copy at `node_modules/.codex-git-full`; the latest code baseline before the current M125 OpenAI-compatible helper slice is `53338975 refactor: extract acedata route helper`, after the latest code slices `74dbdbf1 refactor: extract wuyin route helper`, `dcf38e87 fix: harden local user-route task signing`, `9e4b409e refactor: extract local user-route task token helper`, `5dda8972 style: close settings workbench shell flattening`, and `53338975 refactor: extract acedata route helper`. A docs-only ledger sync commit may sit above that code baseline; use alternate-git `log -1` for the exact HEAD. Use only `git --git-dir=node_modules/.codex-git-full --work-tree=.` for status, staging, diffs, and commits in this session.
...
- Stage Two M125 `OpenAICompatibleAdapter` 12AI async and chat-image helper extraction is completed in the current slice: it moves pure 12AI async route/base-url/size/quality/reference helpers into `src/services/llm/openAICompatible12AIAsyncRoute.ts` and moves structured chat-image candidate selection into `src/services/llm/openAICompatibleImagePayload.ts`, while leaving endpoint selection, auth/header behavior, fetch execution, polling, billing, fallback ordering, UI, and release metadata unchanged.
- The source-proven unused cleanup line now includes the M60-M106 cleanup commits through secure proxy route-gate wiring, plus later M114-M119, the post-M120 UI closure commits `da4ffc79` / `485a6bef` / `1ca080eb`, and the `5aaccf50` ecommerce analysis fallback hotfix. The active line is now finalization audit plus fresh seam-map giant-file splitting after the M125 helper slice; the next selected follow-up is M126 chat structured image URL/MIME security hardening.
...
- `src/services/llm/OpenAICompatibleAdapter.ts`: 2789 physical lines after M125 12AI async and chat-image helper extraction; `src/services/llm/openAICompatible12AIAsyncRoute.ts`: 100 physical lines; `src/services/llm/openAICompatibleAceDataRoute.ts`: 153 physical lines; `src/services/llm/openAICompatibleChatPayload.ts`: 66 physical lines; `src/services/llm/openAICompatibleGoogleExtraBody.ts`: 62 physical lines; `src/services/llm/openAICompatibleImageDispatch.ts`: 91 physical lines; `src/services/llm/openAICompatibleImagePayload.ts`: 206 physical lines; `src/services/llm/openAICompatibleImageSizing.ts`: 99 physical lines; `src/services/llm/openAICompatibleTaskPayload.ts`: 219 physical lines.
