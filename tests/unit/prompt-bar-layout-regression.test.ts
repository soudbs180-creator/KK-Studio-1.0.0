...
test('prompt bar keeps footer wrapping while allowing full desktop control labels', () => {
  ...
  assert.match(promptBarSource, /className=\{`flex min-w-fit flex-shrink-0 items-center justify-center gap-1 px-2 h-full rounded-md transition-all text-\[11px\] font-medium whitespace-nowrap \$\{config\.enableGrounding/);
  assert.match(promptBarSource, /className=\{`flex min-w-fit flex-shrink-0 items-center justify-center gap-1 px-2 h-full rounded-md transition-all text-\[11px\] font-medium whitespace-nowrap \$\{config\.enableImageSearch/);
  assert.match(promptBarSource, /<span className="whitespace-nowrap">\{config\.aspectRatio === AspectRatio\.AUTO \? '自适应' : config\.aspectRatio\} · \{config\.imageSize\}<\/span>/);
  assert.match(promptBarSource, /<span className="whitespace-nowrap">谷歌搜索<\/span>/);
  assert.match(promptBarSource, /<span className="whitespace-nowrap">图片搜索<\/span>/);
