...
test("server-side user-route config resolution forces GPT Best back to bearer-header auth", () => {
  ...
  assert.match(diagnosticsSource, /function shouldForceHeaderAuthForProvider\(provider: string, baseUrl: string\): boolean \{/);
  assert.match(
    diagnosticsSource,
    /if \(shouldForceHeaderAuthForProvider\(routeConfig\.provider, routeConfig\.baseUrl\)\) \{\s*return "header";\s*\}/,
  );

  assert.match(localProxySource, /function shouldForceHeaderAuthForProvider\(provider: string \| undefined, baseUrl: string \| undefined\): boolean \{/);
  assert.match(
    localProxySource,
    /if \(shouldForceHeaderAuthForProvider\(routeConfig\.provider, routeConfig\.baseUrl\)\) \{\s*return "header";\s*\}/,
  );

  assert.match(secureProxySource, /function shouldForceHeaderAuthForProvider\(provider: string, baseUrl: string\): boolean \{/);
  assert.match(
    secureProxySource,
    /authMethod: shouldForceHeaderAuthForProvider\(provider, baseUrl\)\s*\?\s*'header'\s*:\s*matchedRecord\.authMethod === 'query' \? 'query' : 'header'/,
  );
});
