...
test("landing page keeps KK Studio brand while using the current landing structure", () => {
  ...
  assert.match(combinedCss, /\.kk-landing-continuous-stage\s*\{/);
  assert.match(combinedCss, /--kk-landing-page-gradient:/);
  assert.match(combinedCss, /min-height:\s*100%;/);
  assert.doesNotMatch(combinedCss, /\.kk-landing-gradient-stage\s*\{[\s\S]*height:\s*240vh;/);
  ...
  assert.match(combinedCss, /\.kk-landing-footer\s*\{[\s\S]*grid-template-columns:\s*minmax\(0,\s*0\.42fr\)\s*minmax\(320px,\s*0\.58fr\);/);
  assert.match(combinedCss, /\.kk-landing-footer__flower\s*\{[\s\S]*position:\s*relative;[\s\S]*aspect-ratio:\s*16 \/ 10;/);
  assert.doesNotMatch(combinedCss, /\.kk-landing-footer__flower\s*\{[\s\S]*right:\s*min\(-12vw,\s*-90px\);/);
  assert.match(combinedCss, /\.kk-landing-work-card\s*\{/);
  assert.match(combinedCss, /touch-action:\s*pan-y;/);
  assert.doesNotMatch(combinedCss, /newgenre_static|--ng-|\.ng-/);
...
test("login card uses neutral KK reference styling instead of the rejected blue treatment", () => {
  ...
  assert.match(combinedAuthCss, /\.auth-page--dark \.auth-panel\s*\{[\s\S]*background:\s*rgba\(255,\s*255,\s*255,\s*0\.45\)/);
  assert.match(combinedAuthCss, /\.auth-page--dark \.auth-btn-main\s*\{[\s\S]*background:\s*#0c1018 !important;/);
