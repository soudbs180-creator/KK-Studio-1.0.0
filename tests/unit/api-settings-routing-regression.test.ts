...
test('ApiSettingsView keeps editor visibility route-driven and returns through API management state', () => {
  ...
  assert.match(source, /const showOfficialEditor = activeEditorMode === 'official' \|\| showInlineOfficialCreate;/);
  assert.match(source, /const showProviderEditor = activeEditorMode === 'third-party' \|\| showInlineProviderCreate;/);
...
test('ApiSettingsView clears stale provider models when the provider connection changes or a refresh returns an empty list', () => {
  ...
  assert.match(source, /models: existingProvider\?\.models \|\| \[\],/);
  assert.match(source, /supportedModels: check\.ok && check\.models\.length > 0 \? check\.models : slot\.supportedModels,/);
  assert.match(source, /models: check\.ok && check\.models\.length > 0 \? check\.models : provider\.models,/);
});
