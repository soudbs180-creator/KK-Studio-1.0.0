-- Align the legacy authenticate_admin RPC with the current admin_sessions flow.
-- This preserves backward compatibility for old clients without keeping a
-- parallel, non-persisted admin token mechanism alive.
...
BEGIN
    ...
    SELECT COALESCE(requires_password_change, true)
    ...
    v_token := 'adm_' || encode(gen_random_bytes(32), 'hex');
...
COMMENT ON FUNCTION public.authenticate_admin(text) IS
    'Legacy compatibility RPC. Validates the admin second-factor password and now issues a real public.admin_sessions token.';
