...
const saveCreditProviderSchema = z.object({
  ...
  models: z.array(z.object({
    ...
    endpointType: z.string().min(1).max(80).default('openai'),
    ...
    weight: z.coerce.number().int().min(0).max(100000).default(0),
...
router.put('/v1/admin/credit-providers/:providerId', adminAuth(2), async (req, res) => {
  ...
  if (!parsed.success) {
    return res.status(400).json({ error: 'Invalid credit provider payload.' });
  }
    ...
    for (const model of parsed.data.models) {
      await client.query(
        `INSERT INTO public.admin_credit_models (
          ...
          endpoint_type, credit_cost, priority, weight, is_active, max_calls_limit, color,
          color_secondary, text_color, advanced_enabled, mix_with_same_model, quality_pricing,
          auto_pause_on_limit, provider_kind, updated_at
        ) VALUES (
