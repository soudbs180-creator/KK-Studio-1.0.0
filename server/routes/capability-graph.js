...
function sendError(response, error) {
  ...
  response.status(statusCode).json(wrapError(safeError, { surface: 'capability-graph' }));
}
...
function createCapabilityGraphRouter(overrides = {}) {
  ...
  router.use(createAuthMiddleware(overrides.verifyJWT || verifyJWT));
  router.use(createFlagMiddleware(overrides.isEnabled || isImageProviderSliceEnabled));
