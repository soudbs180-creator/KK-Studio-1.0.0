import { type ChatOptions, type ImageGenerationOptions, type ImageGenerationResult, type VideoGenerationOptions, type VideoGenerationResult, type AudioGenerationOptions, type AudioGenerationResult, type ProviderConfig } from './LLMAdapter';
...
import { logWarning, logError } from '../system/systemLogService';
...
export interface GenerateImageResult {
  ...
  model?: string;
  imageSize?: ImageSize;
  effectiveModel?: string; // 实际使用的模型
  effectiveSize?: ImageSize; // 实际使用的尺寸
  aspectRatio?: AspectRatio; // 宽高比
  dimensions?: { width: number; height: number }; // AUTO 模式下的精确尺寸
  provider?: string; // 供应商内部 ID
  providerName?: string; // 用户定义的供应商名称
  modelName?: string; // 友好模型名称
  keySlotId?: string;
  requestPath?: string;
...
export class GenerationService {
    ...
    public async generateImageRaw(options: ImageGenerationOptions, onTaskId?: (id: string) => void): Promise<import('./LLMAdapter').ImageGenerationResult> {
        let lastError: any;
        const maxAttempts = 1;
