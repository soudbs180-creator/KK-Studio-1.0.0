  ...
  return (
    ...
    >
      ...
      >
        ...
        <button type="button" title="Delete workflow" className="h-8 w-8 text-zinc-500 hover:text-red-300" onPointerDown={(event) => event.stopPropagation()} onClick={onDelete}>
      ...
      <div className="flex h-[309px] flex-col overflow-y-auto px-3 py-2">
        {data.steps.map((step, index) => (
          <div key={step.id} className="grid grid-cols-[24px_minmax(0,1fr)_72px] items-center gap-2 border-b border-white/5 py-2">
            <input
              type="checkbox"
            ...
            <div className="flex items-center justify-end">
              <button type="button" title="Move up" className="h-7 w-6 text-zinc-500 hover:text-zinc-200" onClick={() => updateSteps(moveStep(data.steps, index, -1))}><ArrowUp className="h-3.5 w-3.5" /></button>
              <button type="button" title="Move down" className="h-7 w-6 text-zinc-500 hover:text-zinc-200" onClick={() => updateSteps(moveStep(data.steps, index, 1))}><ArrowDown className="h-3.5 w-3.5" /></button>
              <button type="button" title="Remove step" className="h-7 w-6 text-zinc-500 hover:text-red-300" onClick={() => updateSteps(data.steps.filter((item) => item.id !== step.id))}><Trash2 className="h-3.5 w-3.5" /></button>
            </div>
          </div>
        ))}
        <button
          type="button"
          className="mt-2 flex h-8 items-center justify-center gap-1 text-xs text-zinc-400 hover:text-zinc-100"
