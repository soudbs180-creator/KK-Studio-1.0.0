...
- `llmBrain.ts` / `localBrain.ts` 已消费该 authority-free Session context；无 binding 时 LLM 保持原 system + current-user 两消息形状。权威 Context Snapshot 画布摘要、多轮指代、语义事件 replay 与跨设备执行接管仍是后续任务。
