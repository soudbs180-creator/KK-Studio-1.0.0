---
name: pptx-ooxml-toolkit
description: "PPTX OOXML 与 PowerPoint 包结构后端，适用于处理 PPTX 内部结构、布局、讲者备注、批注、HTML 转 PPTX 流程和复杂演示文稿编辑。仅在 `slides-workbench` 选中 OOXML 路线后，或用户明确需要 PPTX 底层结构时使用。"
license: Proprietary. LICENSE.txt has complete terms
---

# PPTX 创建、编辑与分析

这是一个后端技能。除非你已经确定需要 OOXML 和包结构级别的处理，否则请先从 `slides-workbench` 开始。

## 概览

`.pptx` 文件本质上是一个包含 XML 文件和资源的 ZIP 包。这个技能适用于基于文本提取、原始 XML 访问或 html2pptx 流程来创建、编辑和分析 PowerPoint 演示文稿。
