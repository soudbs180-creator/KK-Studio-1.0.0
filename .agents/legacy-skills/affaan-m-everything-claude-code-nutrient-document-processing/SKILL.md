---
name: nutrient-api-document-processing
description: Nutrient API 文档处理后端，适用于 PDF、Office、HTML 和图像文件的云端转换、OCR、提取、编辑、签署与表单填写。仅在 `document-workbench` 选中 Nutrient 路线后，或用户明确要求 Nutrient DWS/API 时使用。
---

# 文档处理

这是 `document-workbench` 选择后的后端技能。只有在明确需要 Nutrient API 时再继续使用这里的细节。
