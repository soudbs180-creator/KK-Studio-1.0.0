---
name: python-pptx-manipulation
description: 基于 python-pptx 的 PowerPoint 编程处理后端，适用于创建、编辑和批量修改 PPTX。仅在 `slides-workbench` 选中 python-pptx 路线后，或用户明确要求 python-pptx 时使用。
...
这是一个后端技能。除非你已经确定要走 `python-pptx` 路线，否则请先从 `slides-workbench` 开始。

## 概览

这个技能基于 **python-pptx** 实现 Microsoft PowerPoint（`.pptx`）的程序化创建、编辑和修改。你可以不用手工打开 PowerPoint，就生成包含文本、形状、图片、图表和表格的专业幻灯片。
