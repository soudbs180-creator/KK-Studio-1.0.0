---
name: paddleocr-ocr
description: PaddleOCR 本地 OCR 后端，适用于图片和扫描文档的多语言文字识别。仅在 `document-workbench` 选中 OCR 路线后，或用户明确要求 PaddleOCR 或本地 OCR 时使用。
...
这是一个后端技能。除非你已经确定要走 PaddleOCR 的本地识别路线，否则请先从 `document-workbench` 开始。

## 概览

这个技能基于 **PaddleOCR** 从图片和扫描文档中智能提取文字。PaddleOCR 支持 100 多种语言，适用于照片、截图、扫描 PDF 和手写文档等场景，并能提供较高识别准确率。
