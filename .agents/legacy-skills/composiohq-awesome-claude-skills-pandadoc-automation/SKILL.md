...
description: "使用 PandaDoc 自动化文档工作流，包括从文件创建文档、管理联系人、整理文件夹、配置 webhook、创建模板，以及通过 Composio PandaDoc 集成跟踪文档状态。"
requires:
  mcp:
    - rube
---
...
直接在 Claude Code 中管理 **PandaDoc** 文档工作流。你可以从上传文件创建文档、管理收件人与联系人、整理文件夹、配置事件 webhook、创建模板，并在终端里跟踪文档状态。

**工具文档：** [composio.dev/toolkits/pandadoc](https://composio.dev/toolkits/pandadoc)

---

## 设置

1. 把 Composio MCP 服务添加到配置中：
   ```
   https://rube.app/mcp
   ```
2. 按提示连接你的 PandaDoc 账号，代理会提供一个 OAuth 链接用于登录授权。
3. 确保你的 PandaDoc 工作区具备所需功能对应的套餐，例如电子签名、模板和 webhook。
