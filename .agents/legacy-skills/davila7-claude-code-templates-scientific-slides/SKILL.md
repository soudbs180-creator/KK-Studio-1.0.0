---
name: research-talk-slides
description: "科研汇报与学术演讲幻灯片后端，适用于演示结构、叙事节奏、时间控制、设计原则和视觉检查。仅在 `slides-workbench` 选中科研演讲路线后，或用户明确要求科研演示指导时使用。"
allowed-tools: [Read, Write, Edit, Bash]
---

# 科研演示文稿

这是一个后端技能。除非你已经确定任务就是科研汇报或学术演讲的展示策略，否则请先从 `slides-workbench` 开始。

## 概览

科研演示是传播研究成果、分享发现并与学术或专业观众沟通的重要媒介。这个技能提供从结构规划、内容组织到视觉设计和演讲准备的完整指导。
