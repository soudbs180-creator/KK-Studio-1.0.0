...
- 窄卡片页脚、移动端输入栏与生成结果排布继续收口，提升小屏可读性与多结果场景下的信息密度
...
- **卡片缩略外壳模式**：Prompt 卡与图片卡在中远景视图下可降级为紧凑卡片或缩略外壳，弱化文字细节并优先使用缩略图质量，兼顾可读性与帧率
- **工作流兼容底座**：Canvas 数据结构新增可选 `workflow` 字段，并补齐 `canvas -> workflow` 同步逻辑；当前特性默认关闭，不改变既有卡片式交互
- **图像成本口径修正**：生成结果与移动聊天流统一通过 `resolveImageCost` 解析费用，优先采用供应商价格快照，回退到显式成本、历史成本或估算值
- **参考图与尺寸计费补齐**：生成结果保存参考图来源标识，费用估算补入参考图数量，并校正 `4K` / `4k` / `4096x4096` 等尺寸别名的价格换算
- **窄卡片与移动端排布优化**：图片卡底部信息区根据宽度自动切换密度，移动端输入栏收窄并精简网络选项，生成结果间距也随卡片宽度同步优化

#### 当前建议
- 对外说明优先聚焦“画布性能分级 + 工作流兼容底座 + 成本口径修正”这三条主线
- `workflow` 当前仍是兼容层能力，默认关闭，不代表界面已经切换为通用节点工作流
- 历史更新日志继续保留用于追溯，不再代表当前界面、性能与计费展示基线
